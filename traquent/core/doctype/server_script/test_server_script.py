...
class TestServerScript(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		traquent.db.truncate("Server Script")
		traquent.get_doc("User", "Administrator").add_roles("Script Manager")
		for script in scripts:
			script_doc = traquent.get_doc(doctype="Server Script")
	...
	def tearDownClass(cls):
		traquent.db.commit()
		traquent.db.truncate("Server Script")
		traquent.cache.delete_value("server_script_map")

	def setUp(self):
		traquent.cache.delete_value("server_script_map")

	def test_doctype_event(self):
		todo = traquent.get_doc(doctype="ToDo", description="hello").insert()
		self.assertEqual(todo.status, "Open")

		todo = traquent.get_doc(doctype="ToDo", description="test todo").insert()
		self.assertEqual(todo.status, "Closed")

		self.assertRaises(
			traquent.ValidationError, traquent.get_doc(doctype="ToDo", description="validate me").insert
		)

		role = traquent.get_doc(doctype="Role", role_name="_Test Role 9").insert(ignore_if_duplicate=True)
	...
	def test_api(self):
		response = requests.post(get_site_url(traquent.local.site) + "/api/method/test_server_script")
		self.assertEqual(response.status_code, 200)
		self.assertEqual("hello", response.json()["message"])

	def test_api_return(self):
		self.assertEqual(traquent.get_doc("Server Script", "test_return_value").execute_method(), "hello")

	def test_permission_query(self):
		if traquent.conf.db_type == "mariadb":
			self.assertTrue("where (1 = 1)" in traquent.db.get_list("ToDo", run=False))
		else:
			self.assertTrue("where (1 = '1')" in traquent.db.get_list("ToDo", run=False))
		self.assertTrue(isinstance(traquent.db.get_list("ToDo"), list))

	def test_attribute_error(self):
		"""Raise AttributeError if method not found in Namespace"""
		note = traquent.get_doc({"doctype": "Note", "title": "Test Note: Server Script"})
	...
	def test_syntax_validation(self):
		...
		with self.assertRaises(traquent.ValidationError) as se:
			traquent.get_doc(doctype="Server Script", **server_script).insert()
	...
	def test_commit_in_doctype_event(self):
		server_script = traquent.get_doc("Server Script", "test_todo_commit")
		server_script.disabled = 0
		server_script.save()

		self.assertRaises(AttributeError, traquent.get_doc(doctype="ToDo", description="test me").insert)

		server_script.disabled = 1
		server_script.save()

	def test_add_index_in_doctype_event(self):
		server_script = traquent.get_doc("Server Script", "test_add_index")
		server_script.disabled = 0
		server_script.save()
	...
	def test_restricted_qb(self):
		todo = traquent.get_doc(doctype="ToDo", description="QbScriptTestNote")
		todo.insert()

		script = traquent.get_doc(
...
"""
	...
	def test_server_script_scheduled(self):
		scheduled_script = traquent.get_doc(
			doctype="Server Script",
		...
		sync_jobs()
		self.assertTrue(traquent.db.exists("Scheduled Job Type", {"server_script": scheduled_script.name}))

		cron_job_name = traquent.db.get_value("Scheduled Job Type", {"server_script": cron_script.name})
		self.assertTrue(cron_job_name)

		cron_job = traquent.get_doc("Scheduled Job Type", cron_job_name)
		...
		updated_cron_job_name = traquent.db.get_value("Scheduled Job Type", {"server_script": cron_script.name})
		updated_cron_job = traquent.get_doc("Scheduled Job Type", updated_cron_job_name)
		self.assertEqual(updated_cron_job.next_execution.day, 2)

	def test_server_script_state_changes(self):
		script: ServerScript = traquent.get_doc(
			doctype="Server Script",
		...
		job: ScheduledJobType = traquent.get_doc("Scheduled Job Type", {"server_script": script.name})
