...
if TYPE_CHECKING:
	...
	from traquent.workflow.doctype.workflow.workflow import Workflow


class WorkflowStateError(traquent.ValidationError):
	pass


class WorkflowTransitionError(traquent.ValidationError):
	pass


class WorkflowPermissionError(traquent.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = traquent.cache.hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = traquent.db.get_value("Workflow", {"document_type": doctype, "is_active": 1}, "name")
		traquent.cache.hset("workflow", doctype, workflow_name or "")
...
def get_transitions(
	...
	if not current_state:
		if raise_exception:
			...
			traquent.throw(_("Workflow State not set"), WorkflowStateError)
	...
	return dict(
		traquent=traquent._dict(
			...
			utils=traquent._dict(
				now_datetime=traquent.utils.now_datetime,
				add_to_date=traquent.utils.add_to_date,
				get_datetime=traquent.utils.get_datetime,
...
def apply_workflow(doc, action):
	...
	if not transition:
		traquent.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		traquent.throw(_("Self approval is not allowed"))
	...
	if doc.docstatus.is_draft() and new_docstatus == DocStatus.draft():
		...
		traquent.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = traquent.bold(current_state)
		bold_next = traquent.bold(next_state)
...
def get_workflow(doctype) -> "Workflow":
	return traquent.get_cached_doc("Workflow", get_workflow_name(doctype))
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	if len(docnames) < 20:
		...
		traquent.msgprint(_("Bulk {0} is enqueued in background.").format(action), alert=True)
		traquent.enqueue(
		...
		traquent.throw(_("Bulk approval only support up to 500 documents."), title=_("Too Many Documents"))
...
def _bulk_workflow_action(docnames, doctype, action):
	...
	for idx, docname in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(traquent.get_doc(doctype, docname), action)
			traquent.db.commit()
		except Exception as e:
			if not traquent.message_log:
			...
			if not message_dict:
				if traquent.message_log:
					messages = traquent.get_message_log()
					for message in messages:
						traquent.message_log.pop()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{traquent.utils.get_link_to_form(doctype, doc)}</summary>"
	...
	try:
		for i, doc in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(traquent.session.user, doc, t)
			]
			if not actions:
				return []
...
def show_progress(docnames, message, i, description):
	n = len(docnames)
	if n >= 5:
		traquent.publish_progress(float(i) * 100 / n, title=message, description=description)


def set_workflow_state_on_action(doc, workflow_name, action):
	workflow = traquent.get_doc("Workflow", workflow_name)
