...
from traquent.model.naming import parse_naming_series
from traquent.utils.data import evaluate_filters
...
class DocumentNamingRule(Document):
	...
	if TYPE_CHECKING:
		from traquent.core.doctype.document_naming_rule_condition.document_naming_rule_condition import (
	...
	def clear_doctype_map(self):
		traquent.cache_manager.clear_doctype_map(self.doctype, self.document_type)
	...
	def validate_fields_in_conditions(self):
		if self.has_value_changed("document_type"):
			docfields = [x.fieldname for x in traquent.get_meta(self.document_type).fields]
			for condition in self.conditions:
				if condition.field not in docfields:
					traquent.throw(
						_("{0} is not a field of doctype {1}").format(
							traquent.bold(condition.field), traquent.bold(self.document_type)
						)
					)
	...
	def apply(self, doc):
		...
		counter = traquent.db.get_value(self.doctype, self.name, "counter", for_update=True) or 0
		naming_series = parse_naming_series(self.prefix, doc=doc)

		doc.name = naming_series + ("%0" + str(self.prefix_digits) + "d") % (counter + 1)
		traquent.db.set_value(self.doctype, self.name, "counter", counter + 1)
