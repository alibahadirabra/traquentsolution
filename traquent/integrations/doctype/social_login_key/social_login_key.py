...
class BaseUrlNotSetError(traquent.ValidationError):
	pass


class AuthorizeUrlNotSetError(traquent.ValidationError):
	pass


class AccessTokenUrlNotSetError(traquent.ValidationError):
	pass


class RedirectUrlNotSetError(traquent.ValidationError):
	pass


class ClientIDNotSetError(traquent.ValidationError):
	pass


class ClientSecretNotSetError(traquent.ValidationError):
	pass
...
class SocialLoginKey(Document):
	...
	def autoname(self):
		self.name = traquent.scrub(self.provider_name)

	def validate(self):
		self.set_icon()
		if self.custom_base_url and not self.base_url:
			traquent.throw(_("Please enter Base URL"), exc=BaseUrlNotSetError)
		if not self.authorize_url:
			traquent.throw(_("Please enter Authorize URL"), exc=AuthorizeUrlNotSetError)
		if not self.access_token_url:
			traquent.throw(_("Please enter Access Token URL"), exc=AccessTokenUrlNotSetError)
		if not self.redirect_url:
			traquent.throw(_("Please enter Redirect URL"), exc=RedirectUrlNotSetError)
		if self.enable_social_login and not self.client_id:
			traquent.throw(_("Please enter Client ID before social login is enabled"), exc=ClientIDNotSetError)
...
def provider_allows_signup(provider: str) -> bool:
	from traquent.website.utils import is_signup_disabled

	sign_up_config = traquent.db.get_value("Social Login Key", provider, "sign_ups")
