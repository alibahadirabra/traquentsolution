...
from traquent.utils import cint, cstr, now_datetime
...
class InvalidNamingSeriesError(traquent.ValidationError):
	pass


class InvalidUUIDValue(traquent.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			traquent.throw(
				_("Invalid naming series {}: dot (.) missing").format(traquent.bold(self.series)),
		...
		if not NAMING_SERIES_PATTERN.match(self.series):
			traquent.throw(
				...
				).format(traquent.bold(self.series)),
				exc=InvalidNamingSeriesError,
			)
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			traquent.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		Series = traquent.qb.DocType("Series")
		prefix = self.get_prefix()

		# Initialize if not present in DB
		if traquent.db.get_value("Series", prefix, "name", order_by="name") is None:
			traquent.qb.into(Series).insert(prefix, 0).columns("name", "current").run()

		(traquent.qb.update(Series).set(Series.current, cint(new_count)).where(Series.name == prefix)).run()

	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(traquent.db.get_value("Series", prefix, "current", order_by="name"))
...
def set_new_name(doc):
	...
	if autoname.lower() not in ("prompt", "uuid") and not traquent.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = traquent.db.get_next_sequence_val(doc.doctype)
		return
	...
	if meta.autoname == "UUID":
		if not doc.name:
			...
			try:
				...
				traquent.throw(_("Invalid value specified for UUID: {}").format(doc.name), InvalidUUIDValue)
		return
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			traquent.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_naming_from_document_naming_rule(doc):
	...
	"""
	from traquent.model.base_document import DOCTYPES_FOR_DOCTYPE
	...
	document_naming_rules = traquent.cache_manager.get_doctype_map(
	...
	for d in document_naming_rules:
		traquent.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		traquent.throw(traquent._("Naming Series mandatory"))
...
def parse_naming_series(
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = traquent.get_attr(method[0])(doc, e)
		else:
			part = e
...
def has_custom_parser(e):
	...
	return traquent.get_hooks("naming_series_variables", {}).get(e)
...
def getseries(key, digits):
	...
	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		traquent.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		traquent.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (traquent.qb.from_(series).where(series.name == prefix).for_update().select("current")).run()

	if current and current[0][0] == count:
		traquent.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype: str) -> str | None:
	"""get default value for `naming_series` property"""
	naming_series_options = traquent.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str):
	if not name:
		traquent.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			traquent.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		traquent.throw(_("Invalid name type (integer) for varchar name column"), traquent.NameError)

	if name.startswith("New " + doctype):
		traquent.throw(
			_("There were some errors setting the name, please contact the administrator"), traquent.NameError
		)
	name = name.strip()

	if not traquent.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		traquent.throw(_("Name of {0} cannot be {1}").format(doctype, name), traquent.NameError)

	special_characters = "<>"
	if re.findall(f"[{special_characters}]+", name):
		message = ", ".join(f"'{c}'" for c in special_characters)
		traquent.throw(_("Name cannot contain special characters like {0}").format(message), traquent.NameError)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = traquent.db.exists(doctype, filters)

	regex = f"^{re.escape(value)}{separator}\\d+$"

	if exists:
		last = traquent.db.sql(
			f"""SELECT `{fieldname}` FROM `tab{doctype}`
			WHERE `{fieldname}` {traquent.db.REGEX_CHARACTER} %s
...
def _set_amended_name(doc):
	...
	if traquent.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		traquent.throw(_("Please set the document name"))
