...
if any((os.getenv("CI"), traquent.conf.developer_mode, traquent.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from traquent.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from traquent.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = traquent.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		traquent.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if traquent.db.exists("Token Cache", token_cache_name):
			token_cache = traquent.get_doc("Token Cache", token_cache_name)
...
def callback(code=None, state=None):
	...
	if traquent.session.user == "Guest":
		traquent.local.response["type"] = "redirect"
		traquent.local.response["location"] = "/login?" + urlencode({"redirect-to": traquent.request.url})
		return

	path = traquent.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		traquent.throw(_("Invalid Parameters."))

	connected_app = traquent.get_doc("Connected App", path[3])
	token_cache = traquent.get_doc("Token Cache", connected_app.name + "-" + traquent.session.user)

	if state != token_cache.state:
		traquent.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
