...
class PortalSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.website.doctype.portal_menu_item.portal_menu_item import PortalMenuItem
	...
	def sync_menu(self):
		...
		dirty = False
		for item in traquent.get_hooks("standard_portal_menu_items"):
			if item.get("role") and not traquent.db.exists("Role", item.get("role")):
				traquent.get_doc({"doctype": "Role", "role_name": item.get("role"), "desk_access": 0}).insert()
	...
	def remove_deleted_doctype_items(self):
		existing_doctypes = set(traquent.get_list("DocType", pluck="name"))
	...
	def validate(self):
		if traquent.request and self.default_portal_home:
