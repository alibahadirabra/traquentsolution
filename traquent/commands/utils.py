...
from traquent.coverage import CodeCoverage
from traquent.exceptions import SiteNotSpecifiedError
from traquent.utils import cint, update_progress_bar
...
def watch(apps=None):
	...
	from traquent.build import watch
...
def destroy_all_sessions(context: CliCtxObj, reason=None):
	...
	for site in context.sites:
		try:
			traquent.init(site)
			traquent.connect()
			traquent.sessions.clear_all_sessions(reason)
			traquent.db.commit()
		finally:
			traquent.destroy()
	if not context.sites:
...
def show_config(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		configuration = traquent.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(traquent.as_json(sites_config))
...
def reset_perms(context: CliCtxObj):
	...
	from traquent.permissions import reset_perms

	for site in context.sites:
		try:
			traquent.init(site)
			traquent.connect()
			for d in traquent.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				traquent.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			traquent.destroy()
	if not context.sites:
...
def execute(context: CliCtxObj, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = traquent.get_attr(method)(*fn_args, **fn_kwargs)
			except Exception:
...
def export_doc(context: CliCtxObj, doctype, docname):
	...
	for site in context.sites:
		try:
			traquent.init(site)
			traquent.connect()
			traquent.modules.export_doc(doctype, docname)
		finally:
			traquent.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context: CliCtxObj, doctype, path, name=None):
	...
	from traquent.core.doctype.data_import.data_import import export_json
...
def export_csv(context: CliCtxObj, doctype, path):
	...
	from traquent.core.doctype.data_import.data_import import export_csv
...
def data_import(
	...
	from traquent.core.doctype.data_import.data_import import import_file
...
def bulk_rename(context: CliCtxObj, doctype, path):
	...
	from traquent.model.rename_doc import bulk_rename
	from traquent.utils.csvutils import read_csv_content
...
def postgres(context: CliCtxObj, extra_args):
	...
	site = get_site(context)
	traquent.init(site)
	traquent.conf.db_type = "postgres"
	_enter_console(extra_args=extra_args)


def _enter_console(extra_args=None):
	from traquent.database import get_command
	from traquent.utils import get_site_path
	...
	bin, args, bin_name = get_command(
		...
		user=traquent.conf.db_user,
		password=traquent.conf.db_password,
...
def jupyter(context: CliCtxObj):
	...
	jupyter_notebooks_path = os.path.abspath(traquent.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(traquent.get_site_path(".."))
...
def store_logs(terminal: "InteractiveShellEmbed") -> None:
	...
	traquent.log_level = 20  # info
	with suppress(Exception):
		logger = traquent.logger("ipython")
...
def console(context: CliCtxObj, autoreload=False):
	...
	all_apps = traquent.get_installed_apps()
	failed_to_import = []
...
def transform_database(context: CliCtxObj, table, engine, row_format, failfast):
	...
	if table == "all":
		...
		queried_tables = (
			...
			.where(
				...
				& (information_schema.tables.table_schema == traquent.conf.db_name)
			)
			.run()
		)
	...
	for current, table in enumerate(tables):
		...
		try:
			traquent.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
		...
		try:
			...
			if args:
				...
				if args.startswith("/api/method"):
					traquent.local.form_dict.cmd = args.split("?", 1)[0].split("/")[-1]
			elif path:
			...
			traquent.handler.execute_cmd(traquent.form_dict.cmd)

			print(traquent.response)
		finally:
			traquent.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def make_app(destination, app_name, no_git=False):
	...
	from traquent.utils.boilerplate import make_boilerplate
...
def create_patch():
	...
	from traquent.utils.boilerplate import PatchCreator
...
def get_version(output):
	...
	from traquent.utils.change_log import get_app_branch
	from traquent.utils.commands import render_table

	traquent.init("")
	data = []

	for app in sorted(traquent.get_all_apps()):
		module = traquent.get_module(app)
		app_hooks = traquent.get_module(app + ".hooks")

		app_info = traquent._dict()

		try:
			app_info.commit = Repo(traquent.get_app_source_path(app)).head.object.hexsha[:7]
...
def rebuild_global_search(context: CliCtxObj, static_pages=False):
	...
	from traquent.utils.global_search import (
