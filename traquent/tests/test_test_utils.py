...
class TestTestUtils(IntegrationTestCase):
	...
	def test_document_assertions(self):
		currency = traquent.new_doc("Currency")
	...
	def test_thread_locals(self):
		traquent.flags.temp_flag_to_be_discarded = True

	def test_temp_setting_changes(self):
		current_setting = traquent.get_system_settings("logout_on_password_reset")

		with IntegrationTestCase.change_settings(
			"System Settings", {"logout_on_password_reset": int(not current_setting)}
		):
			updated_settings = traquent.get_system_settings("logout_on_password_reset")
			self.assertNotEqual(current_setting, updated_settings)

		restored_settings = traquent.get_system_settings("logout_on_password_reset")
...
def tearDownModule():
	...
	assert "temp_flag_to_be_discarded" not in traquent.flags
	assert not traquent.db.exists("Currency", "STONKS")
