...
class TestWebTemplate(IntegrationTestCase):
	def test_render_web_template_with_values(self):
		doc = traquent.get_doc("Web Template", "Hero with Right Image")
		values = {
			"title": "Test Hero",
	...
	def test_custom_stylesheet(self):
		...
		traquent.get_doc("Website Theme", "Standard").set_as_default()

	def create_web_page(self):
		if not traquent.db.exists("Web Page", "test-web-template"):
			traquent.get_doc(
				{
					"doctype": "Web Page",
	...
	def create_website_theme(self):
		if not traquent.db.exists("Website Theme", "Custom"):
			theme = traquent.get_doc({"doctype": "Website Theme", "theme": "Custom"}).insert()
		else:
			theme = traquent.get_doc("Website Theme", "Custom")
		return theme
