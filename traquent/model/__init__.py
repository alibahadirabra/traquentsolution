...
def delete_fields(args_dict, delete=0):
	...
	* If single, deletes record from `tabSingles`
	...
	for dt in args_dict:
		...
		if traquent.db.get_value("DocType", dt, "issingle"):
			traquent.db.delete(
				"Singles",
				{
					"doctype": dt,
			...
			existing_fields = traquent.db.describe(dt)
...
def get_permitted_fields(
	...
	*,
	...
	if permission_type is None:
		permission_type = "select" if traquent.only_has_select_perm(doctype, user=user) else "read"
