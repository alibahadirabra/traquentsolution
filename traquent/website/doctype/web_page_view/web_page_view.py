...
class WebPageView(Document):
	...
	def clear_old_logs(days=180):
		...
		table = traquent.qb.DocType("Web Page View")
...
def make_view_log(
	...
	# real path
	path = traquent.request.headers.get("Referer")

	if not traquent.utils.is_site_link(path):
		return

	path = urlparse(path).path

	request_dict = traquent.request.__dict__
	...
	is_unique = visitor_id and not bool(traquent.db.exists("Web Page View", {"visitor_id": visitor_id}))

	view = traquent.new_doc("Web Page View")
	view.path = path
...
def is_tracking_enabled():
	return traquent.db.get_single_value("Website Settings", "enable_view_tracking")
