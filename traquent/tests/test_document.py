...
import traquent
from traquent.app import make_form_dict
from traquent.core.doctype.doctype.test_doctype import new_doctype
from traquent.desk.doctype.note.note import Note
from traquent.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from traquent.tests import IntegrationTestCase
from traquent.utils import cint, now_datetime, set_request
...
class TestDocument(IntegrationTestCase):
	def test_get_return_empty_list_for_table_field_if_none(self):
		d = traquent.get_doc({"doctype": "User"})
	...
	def test_load_single(self):
		d = traquent.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(traquent.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_website_route_default(self):
		default = traquent.generate_hash()
		...
		doc = traquent.get_doc({"doctype": parent, "child_table": [{"some_fieldname": "xasd"}]}).insert()
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(traquent.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(traquent.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_discard_transitions(self):
		...
		d.submit()
		self.assertRaises(traquent.ValidationError, d.discard)
		d.reload()
	...
	def test_save_on_discard_throws(self):
		from traquent.desk.doctype.event.event import Event
		...
		self.assertRaises(traquent.ValidationError, d3.discard)
	...
	def test_mandatory(self):
		...
		traquent.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(traquent.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(traquent.db.get_value("User", d.name), d.name)

	def test_text_editor_field(self):
		try:
			traquent.get_doc(doctype="Activity Log", subject="test", message='<img src="test.png" />').insert()
		except traquent.MandatoryError:
			self.fail("Text Editor false positive mandatory error")

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = traquent.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(traquent.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = traquent.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = traquent.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		...
		self.assertRaises(traquent.PermissionError, self.test_insert)
		traquent.set_user("Administrator")

	def test_permission_single(self):
		traquent.set_user("Guest")
		d = traquent.get_doc("Website Settings", "Website Settings")
		self.assertRaises(traquent.PermissionError, d.save)
		traquent.set_user("Administrator")

	def test_link_validation(self):
		traquent.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(traquent.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(traquent.ValidationError, d.validate)
		self.assertRaises(traquent.ValidationError, d.run_method, "validate")
		self.assertRaises(traquent.ValidationError, d.save)
	...
	def test_new_doc_with_fields(self):
		user = traquent.new_doc("User", first_name="wizard")
		self.assertEqual(user.first_name, "wizard")

	def test_update_after_submit(self):
		d = self.test_insert()
		d.starts_on = "2014-09-09"
		self.assertRaises(traquent.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(traquent.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = traquent.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(traquent.db.get_value("Series", prefix, "current", order_by="name"))
	...
	def test_get_formatted(self):
		...
		traquent.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = traquent.get_doc({"doctype": "Test Formatted", "currency": 100000})
	...
	def test_virtual_fields(self):
		...
		traquent.db.delete("Custom Field", {"dt": "Note", "fieldname": "age"})
		...
		with patch_note():
			doc = traquent.get_last_doc("Note")
	...
	def test_doc_events(self):
		...
		for doctype, doc_hooks in traquent.get_doc_hooks().items():
			for _, hooks in doc_hooks.items():
				for hook in hooks:
					try:
						traquent.get_attr(hook)
	...
	def test_error_on_saving_new_doc_with_name(self):
		...
		self.assertRaises(traquent.DoesNotExistError, doc.save)
	...
	def test_validate_from_to_dates(self):
		...
		self.assertRaises(
			traquent.exceptions.InvalidDates, doc.validate_from_to_dates, "start_date", "end_date"
		)
	...
	def test_db_set_singles(self):
		...
		c.db_set(key, val)
		changed_val = traquent.db.get_single_value(c.doctype, key)
...
class TestDocumentWebView(IntegrationTestCase):
	def get(self, path, user="Guest"):
		...
		make_form_dict(traquent.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = traquent.get_doc("Document Share Key", {"key": document_key})
	...
	def test_base_class_set_correctly_on_has_web_view_change(self):
		...
		from traquent.modules.utils import get_doc_path, scrub

		traquent.flags.allow_doctype_export = True

		traquent.delete_doc_if_exists("DocType", "Test WebViewDocType", force=1)
	...
	def test_bulk_inserts(self):
		from traquent.model.document import bulk_insert

		doctype = "Role Profile"
		child_field = "roles"
		child_doctype = traquent.get_meta(doctype).get_field(child_field).options
		...
		def doc_generator():
			for _ in range(21):
				...
				doc.role_profile = traquent.generate_hash()
		...
		all_docs = set(traquent.get_all(doctype, pluck="name"))
		all_child_docs = set(
			traquent.get_all(
