...
from traquent import STANDARD_USERS, _, msgprint, throw
from traquent.apps import get_default_path
from traquent.auth import MAX_PASSWORD_SIZE
from traquent.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from traquent.utils.data import sha256_hash
from traquent.utils.password import check_password, get_password_reset_limit
from traquent.utils.password import update_password as _update_password
from traquent.utils.user import get_system_managers
from traquent.website.utils import get_home_page, is_signup_disabled
...
class User(Document):
	...
	if TYPE_CHECKING:
		from traquent.core.doctype.block_module.block_module import BlockModule
		from traquent.core.doctype.defaultvalue.defaultvalue import DefaultValue
		from traquent.core.doctype.has_role.has_role import HasRole
		from traquent.core.doctype.user_email.user_email import UserEmail
		from traquent.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from traquent.core.doctype.user_social_login.user_social_login import UserSocialLogin
	...
	def onload(self):
		from traquent.utils.modules import get_modules_from_all_apps
	...
	def after_insert(self):
		...
		traquent.cache.delete_key("users_for_mentions")
		traquent.cache.delete_key("enabled_users")
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = traquent.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	from traquent.deprecation_dumpster import validate_roles
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = traquent.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			traquent.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		traquent.clear_cache(user=self.name)
		...
		if self.time_zone:
			traquent.defaults.set_default("time_zone", self.time_zone, self.name)

		if self.has_value_changed("language"):
			locale_keys = ("date_format", "time_format", "number_format", "first_day_of_the_week")
			if self.language:
				language = traquent.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						traquent.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					traquent.defaults.clear_default(key, parent=self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == traquent.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			traquent.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(traquent.local, "login_manager", None):
			traquent.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not traquent.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = traquent.get_cached_doc("User Type", self.user_type)
	...
	def password_reset_mail(self, link):
		reset_password_template = traquent.db.get_system_setting("reset_password_template")
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = traquent.get_hooks("welcome_email")
		if method:
			subject = traquent.get_attr(method[-1])()
		if not subject:
			site_name = traquent.db.get_default("site_name") or traquent.get_conf().get("site_name")
			if site_name:
				subject = _("Welcome to {0}").format(site_name)
			else:
				subject = _("Complete Registration")

		welcome_email_template = traquent.db.get_system_setting("welcome_email_template")
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		from traquent.utils import get_url
		from traquent.utils.user import get_user_fullname

		created_by = get_user_fullname(traquent.session["user"])
		if created_by == "Guest":
		...
		sender = (
			traquent.session.user not in STANDARD_USERS and get_formatted_email(traquent.session.user) or None
		)

		if custom_template:
			from traquent.email.doctype.email_template.email_template import get_email_template
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(traquent.local, "login_manager", None):
			traquent.local.login_manager.logout(user=self.name)

		# delete todos
		traquent.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			traquent.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		traquent.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		traquent.db.delete("DocShare", {"user": self.name})
		# delete messages
		...
		traquent.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		traquent.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		traquent.db.delete("User Permission", {"user": self.name})

		# Delete OAuth data
		traquent.db.delete("OAuth Authorization Code", {"user": self.name})
		traquent.db.delete("Token Cache", {"user": self.name})

		# Delete EPS data
		traquent.db.delete("Energy Point Log", {"user": self.name})

		# Remove user link from Workflow Action
		traquent.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		traquent.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = traquent.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = traquent.get_doc("Note", note_id)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			traquent.delete_doc("Notification Settings", old_name, ignore_permissions=True)

		traquent.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = traquent.db.get_tables()
		for tab in tables:
			desc = traquent.db.get_table_columns_description(tab)
		...
		if traquent.db.exists("Notification Settings", old_name):
			traquent.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		traquent.db.set_value("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in traquent.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = traquent.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				traquent.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(traquent.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(traquent.scrub("{} {}".format(self.first_name, self.last_name or "")))

		if username:
			traquent.msgprint(_("Suggested Username: {0}").format(username))

		return username

	def username_exists(self, username=None):
		return traquent.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			traquent.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(traquent.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(traquent.get_system_settings("allow_login_using_user_name"))
		...
		users = traquent.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
	...
	def check_roles_added(self):
		...
		traquent.msgprint(
			_("Newly created user {0} has no roles enabled.").format(traquent.bold(self.name)),
...
def get_all_roles():
	...
	roles = traquent.get_all(
		"Role",
		filters={
			"name": ("not in", traquent.permissions.AUTOMATIC_ROLES),
			"disabled": 0,
		},
...
def update_password(
	...
	if len(new_password) > MAX_PASSWORD_SIZE:
		traquent.throw(_("Password size exceeded the maximum allowed size."))
	...
	if res.get("message"):
		traquent.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or traquent.get_system_settings("logout_on_password_reset")
	_update_password(user, new_password, logout_all_sessions=cint(logout_all_sessions))

	user_doc, redirect_url = reset_user_data(user)

	# get redirect url from cache
	redirect_to = traquent.cache.hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		traquent.cache.hdel("redirect_after_login", user)

	traquent.local.login_manager.login_as(user)

	traquent.db.set_value("User", user, "last_password_reset_date", today())
	traquent.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	...
	enable_password_policy = traquent.get_system_settings("enable_password_policy")

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = traquent.db.get_value(
			"User", traquent.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(traquent.get_system_settings("minimum_password_score"))
...
def ask_pass_update():
	...
	from traquent.utils import set_default

	password_list = traquent.get_all(
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				traquent.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		traquent.local.login_manager.check_password(traquent.session.user, old_password)
...
def reset_user_data(user):
	user_doc = traquent.get_doc("User", user)
...
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	...
	user = traquent.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if traquent.db.get_creation_count("User", 60) > 300:
		...
		from traquent.utils import random_string
		...
		default_role = traquent.db.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			traquent.cache.hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user: str) -> str:
	try:
		user: User = traquent.get_doc("User", user)
		...
		traquent.local.response["http_status_code"] = 404
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters and filters.get("ignore_user_type") and traquent.session.data.user_type == "System User":
		user_type_condition = ""
    ...
    """.format(
			...
			standard_users=", ".join(traquent.db.escape(u) for u in STANDARD_USERS),
			key=searchfield,
...
def get_website_users():
	...
	return traquent.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(traquent.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and traquent.local.conf.notify_admin_access_to_system_manager
	):
		site = '<a href="{0}" target="_blank">{0}</a>'.format(traquent.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = traquent.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	traquent.throw(msg=" ".join([warning, *suggestions]), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		traquent.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if traquent.db.get_creation_count("User", 60) > traquent.local.conf.get("throttle_user_limit", 60):
...
def create_contact(user, ignore_links=False, ignore_mandatory=False):
	...
	if not contact_name:
		...
		except traquent.DuplicateEntryError:
			pass
	else:
		try:
			contact = traquent.get_doc("Contact", contact_name)
		...
		except traquent.TimestampMismatchError:
			raise traquent.RetryBackgroundJobError
...
def generate_keys(user: str):
	...
	user_details: User = traquent.get_doc("User", user)
	api_secret = traquent.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = traquent.generate_hash(length=15)
...
def get_enabled_users():
	def _get_enabled_users():
		enabled_users = traquent.get_all("User", filters={"enabled": "1"}, pluck="name")
...
def impersonate(user: str, reason: str):
	...
	impersonator = traquent.session.user
	traquent.get_doc(
		{
	...
	traquent.local.login_manager.impersonate(user)
