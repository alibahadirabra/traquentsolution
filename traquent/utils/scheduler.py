...
import traquent
from traquent.utils import cint, get_bench_path, get_datetime, get_sites, now_datetime
from traquent.utils.background_jobs import set_niceness
...
def start_scheduler() -> NoReturn:
	...
	try:
		...
		traquent.logger("scheduler").debug("Scheduler already running")
		return
...
def enqueue_events_for_all_sites() -> None:
	...
	for site in sites:
		try:
			...
			traquent.logger("scheduler").debug(f"Failed to enqueue events for site: {site}", exc_info=True)


def enqueue_events_for_site(site: str) -> None:
	def log_exc():
		traquent.logger("scheduler").error(f"Exception in Enqueue Events for Site {site}", exc_info=True)
	...
	try:
		...
		traquent.logger("scheduler").debug(f"Queued events for site {site}")
	except Exception as e:
		if traquent.db.is_access_denied(e):
			traquent.logger("scheduler").debug(f"Access denied for site {site}")
		log_exc()
...
def enqueue_events() -> list[str] | None:
	if schedule_jobs_based_on_activity():
		enqueued_jobs = []
		all_jobs = traquent.get_all("Scheduled Job Type", filters={"stopped": 0}, fields="*")
		random.shuffle(all_jobs)
		for job_type in all_jobs:
			job_type = traquent.get_doc(doctype="Scheduled Job Type", **job_type)
			try:
				if job_type.enqueue():
					enqueued_jobs.append(job_type.method)
			except CroniterBadCronError:
				traquent.logger("scheduler").error(
					f"Invalid Job on {traquent.local.site} - {job_type.name}", exc_info=True
				)
...
def is_scheduler_inactive(verbose=True) -> bool:
	if traquent.local.conf.maintenance_mode:
		if verbose:
			cprint(f"{traquent.local.site}: Maintenance mode is ON")
		return True
...
def is_scheduler_disabled(verbose=True) -> bool:
	...
	scheduler_disabled = not traquent.utils.cint(
		traquent.db.get_single_value("System Settings", "enable_scheduler")
	)
	if scheduler_disabled:
		if verbose:
			cprint(f"{traquent.local.site}: SystemSettings.enable_scheduler is UNSET")
	return scheduler_disabled


def toggle_scheduler(enable):
	traquent.db.set_single_value("System Settings", "enable_scheduler", int(enable))
...
def is_dormant(check_time=None):
	...
	since = (traquent.get_system_settings("dormant_days") or 4) * 86400
...
def _get_last_creation_timestamp(doctype):
	timestamp = traquent.db.get_value(doctype, filters={}, fieldname="creation", order_by="creation desc")
	if timestamp:
...
def activate_scheduler():
	...
	if traquent.local.conf.maintenance_mode:
		traquent.throw(traquent._("Scheduler can not be re-enabled when maintenance mode is active."))

	if is_scheduler_disabled():
		enable_scheduler()
	if traquent.conf.pause_scheduler:
