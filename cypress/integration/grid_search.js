...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.its("traquent")
			.then((traquent) => {
				return traquent.xcall(
					"traquent.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("traquent")
			.then((traquent) => {
				traquent.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.traquent-control[data-fieldname="child_table_1"]').as("table");
