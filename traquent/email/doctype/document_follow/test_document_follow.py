...
import traquent
import traquent.desk.form.document_follow as document_follow
from traquent.desk.form.assign_to import add
from traquent.desk.form.document_follow import get_document_followed_by_user
from traquent.desk.form.utils import add_comment
from traquent.desk.like import toggle_like
from traquent.query_builder import DocType
from traquent.query_builder.functions import Cast_
from traquent.share import add as share
...
class TestDocumentFollow(IntegrationTestCase):
	...
	def tearDown(self):
		...
		traquent.db.delete("Document Follow")
	...
	doc = traquent.get_doc(
		{
			...
			"doc.starts_on": traquent.utils.now(),
			"doc.ends_on": traquent.utils.add_days(traquent.utils.now(), 5),
...
def get_user(document_follow=None):
	...
	if traquent.db.exists("User", "test@docsub.com"):
		doc = traquent.delete_doc("User", "test@docsub.com")
	doc = traquent.new_doc("User")
