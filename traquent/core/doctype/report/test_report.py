...
import traquent
from traquent.core.doctype.user_permission.test_user_permission import create_user
from traquent.custom.doctype.customize_form.customize_form import reset_customization
from traquent.desk.query_report import add_total_row, run, save_report
from traquent.desk.reportview import delete_report
from traquent.desk.reportview import save_report as _save_report
...
class TestReport(IntegrationTestCase):
	...
	def test_report_builder(self):
		if traquent.db.exists("Report", "User Activity Report"):
			traquent.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json")) as f:
			traquent.get_doc(json.loads(f.read())).insert()

		report = traquent.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = traquent.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			traquent.set_user("test_report_owner@example.com")
			self.assertRaises(traquent.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = traquent.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(user=traquent.session.user)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = traquent.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		traquent.set_user("test@example.com")
		traquent.db.delete("Has Role", {"parent": traquent.session.user, "role": "Test Has Role"})
		traquent.db.commit()
		if not traquent.db.exists("Role", "Test Has Role"):
			traquent.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

		if not traquent.db.exists("Report", "Test Report"):
			report = traquent.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = traquent.get_doc("Report", "Test Report")
	...
	def test_report_custom_permissions(self):
		traquent.set_user("test@example.com")
		traquent.db.delete("Custom Role", {"report": "Test Custom Role Report"})
		traquent.db.commit()  # nosemgrep
		if not traquent.db.exists("Report", "Test Custom Role Report"):
			report = traquent.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = traquent.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if traquent.db.exists("Report", "User Activity Report Without Sort"):
			traquent.delete_doc("Report", "User Activity Report Without Sort")
		with open(os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json")) as f:
			traquent.get_doc(json.loads(f.read())).insert()

		report = traquent.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not traquent.db.exists("Report", report_name):
			report = traquent.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if traquent.db.exists("Report", report_name):
			traquent.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = traquent.get_all(doctype="Report", limit=1)
		...
		doc = traquent.get_doc("Report", report_name)
		with self.assertRaises(traquent.exceptions.ValidationError):
	...
	def test_cte_in_query_report(self):
		...
		if traquent.db.db_type == "mariadb":
			...
			self.assertRaises(traquent.PermissionError, report.execute_query_report, filters={})
