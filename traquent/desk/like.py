...
def _toggle_like(doctype, name, add, user=None):
	...
	try:
		liked_by = traquent.db.get_value(doctype, name, "_liked_by")
		...
		if add == "Yes":
			if user not in liked_by:
				...
				if traquent.get_cached_value("User", user, "follow_liked_documents"):
		...
		if traquent.get_meta(doctype).issingle:
			traquent.db.set_single_value(doctype, "_liked_by", json.dumps(liked_by), update_modified=False)
		else:
			traquent.db.set_value(doctype, name, "_liked_by", json.dumps(liked_by), update_modified=False)
