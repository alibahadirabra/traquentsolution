...
class TestModuleProfile(IntegrationTestCase):
	def test_make_new_module_profile(self):
		if not traquent.db.get_value("Module Profile", "_Test Module Profile"):
			traquent.get_doc(
				{
		...
		if not traquent.db.get_value("User", "test-for-module_profile@example.com"):
			new_user = traquent.get_doc(
				{"doctype": "User", "email": "test-for-module_profile@example.com", "first_name": "Test User"}
			).insert()
		else:
			new_user = traquent.get_doc("User", "test-for-module_profile@example.com")
