...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = traquent.scrub(app_name)
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	traquent.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, traquent.scrub(hooks.app_title)),
		with_init=True,
	)
	traquent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	traquent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	traquent.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	traquent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	traquent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	traquent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	traquent.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(traquent.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(traquent.as_unicode(patches_template.format(**hooks)))
	...
	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(traquent.as_unicode(readme_template.format(**hooks)))

	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(traquent.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = traquent.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(traquent.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = traquent.scrub(self.docstring) + ".py"
