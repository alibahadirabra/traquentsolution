...
traquent.views.FileView = class FileView extends traquent.views.ListView {
	static load_last_view() {
		const route = traquent.get_route();
		if (route.length === 2) {
			const view_user_settings = traquent.get_user_settings("File", "File");
			traquent.set_route(
				"List",
				"File",
				view_user_settings.last_folder || traquent.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			traquent.views.FileView.grid_view = !traquent.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (traquent.views.FileView.grid_view) {
			Object.assign(args, {
		...
		return [
			{
				label: __("Home"),
				action: () => {
					traquent.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					traquent.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								traquent.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							traquent.call({
								method: "traquent.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new traquent.ui.FileUploader({
						...
						on_success: (file) => {
							traquent.show_alert(__("Unzipping files..."));
							traquent
								.call("traquent.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										traquent.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				traquent.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => traquent.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/traquent.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => traquent.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (traquent.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
		...
		d.subject_html = `
			${traquent.utils.icon(icon_class)}
			<span>${title}</span>
	...
	before_render() {
		super.before_render();
		traquent.model.user_settings.save("File", "grid_view", traquent.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !traquent.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !traquent.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === traquent.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = traquent.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? traquent.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${traquent.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${traquent.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					traquent.file_manager.paste(folder_name);
					traquent.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!traquent.file_manager.can_paste ||
			traquent.file_manager.old_folder === this.current_folder;
...
traquent.views.FileView.grid_view = traquent.get_user_settings("File").grid_view || false;
