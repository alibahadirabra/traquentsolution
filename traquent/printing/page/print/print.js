traquent.pages["print"].on_page_load = function (wrapper) {
	traquent.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new traquent.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = traquent.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!traquent.route_options || !traquent.route_options.frm) {
			traquent.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = traquent.get_doc(doctype, docname);
				traquent.model.with_doctype(doctype, () => {
					frm.meta = traquent.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = traquent.route_options.frm.doctype
				? traquent.route_options.frm
				: traquent.route_options.frm.frm;
			traquent.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

traquent.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${traquent.render_template("print_skeleton_loading")}
		...
		this.print_settings = traquent.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = traquent.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (traquent.model.can_create("Print Format")) {
	...
	show(frm) {
		...
		// print designer link
		if (Object.keys(traquent.boot.versions).includes("print_designer")) {
			...
			this.page.add_inner_message(`
			<a style="line-height: 2.4" href="https://traquentcloud.com/marketplace/apps/print_designer?utm_source=framework-desk&utm_medium=print-view&utm_campaign=try-link">
				${__("Try the new Print Designer")}
			</a>
			`);
		}
	...
	set_breadcrumbs() {
		traquent.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		traquent
			.xcall("traquent.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			traquent.set_route("Form", "Print Format", print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				traquent.set_route("print-format-builder-beta", print_format.name);
			} else {
				traquent.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		traquent.prompt(
			...
			(data) => {
				...
				traquent.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			traquent.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || traquent.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = traquent.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = traquent.urllib.get_base_url();
		let print_css = traquent.assets.bundled_asset(
			"print.bundle.css",
			traquent.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", traquent.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
	...
	go_to_form_view() {
		...
		traquent.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					traquent.ui.form
						...
						.then(traquent.ui.form.qz_success)
						.catch((err) => {
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			traquent.call({
				method: "traquent.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		traquent.call({
			method: "traquent.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	async is_wkhtmltopdf_valid() {
		const is_valid = await traquent.xcall("traquent.utils.pdf.is_wkhtmltopdf_valid");
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/traquent.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
			...
			this.render_page("/api/method/traquent.utils.print_format.download_pdf?");
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = traquent.call({
			method: "traquent.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		traquent.call({
			method: "traquent.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	set_style(style) {
		traquent.dom.set_style(style || traquent.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		traquent.ui.form.qz_get_printer_list().then((data) => {
						...
						fields: [
							{
								...
								fieldname: "print_format",
								default: 0,
								options: traquent.meta.get_print_formats(this.frm.doctype),
								read_only: 0,
								in_list_view: 1,
								label: __("Print Format"),
							},
			...
			if (!(this.printer_list && this.printer_list.length)) {
				traquent.throw(__("No Printer is Available."));
			}
		});
	}
};
