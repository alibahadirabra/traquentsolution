...
import traquent
from traquent.geo.doctype.country.country import (
	get_countries_and_currencies,
	import_country_and_currency,
)
from traquent.geo.doctype.currency.currency import enable_default_currencies
from traquent.tests import IntegrationTestCase, UnitTestCase


def get_table_snapshot(doctype):
	data = traquent.db.sql(f"select * from `tab{doctype}` order by name", as_dict=True)
...
class TestCountry(IntegrationTestCase):
	def test_bulk_insert_correctness(self):
		def clear_tables():
			traquent.db.delete("Currency")
			traquent.db.delete("Country")
