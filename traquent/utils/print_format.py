...
def download_multi_pdf_async(
	...
	traquent.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if task_id:
					traquent.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		if task_id is None:
			traquent.local.response.filename = "{doctype}.pdf".format(
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						traquent.publish_realtime(task_id=task_id, message="Failed")
					traquent.log_error(
		...
		if task_id is None:
			traquent.local.response.filename = f"{name}.pdf"
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			traquent.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			traquent.local.response.filecontent = merged_pdf.getvalue()
...
def download_pdf(doctype, name, format=None, doc=None, no_letterhead=0, language=None, letterhead=None):
	...
	with print_language(language):
		pdf_file = traquent.get_print(
			doctype, name, format, doc=doc, as_pdf=True, letterhead=letterhead, no_letterhead=no_letterhead
		)

	traquent.local.response.filename = "{name}.pdf".format(name=name.replace(" ", "-").replace("/", "-"))
	traquent.local.response.filecontent = pdf_file
	traquent.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	traquent.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	traquent.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = traquent.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		traquent.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		if (
			...
			traquent.throw(_("PDF generation failed"))
	except cups.IPPError:
		traquent.throw(_("Printing failed"))
