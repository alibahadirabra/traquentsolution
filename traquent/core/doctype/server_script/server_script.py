...
class ServerScript(Document):
	...
	def on_trash(self):
		...
		if self.script_type == "Scheduler Event":
			for job in self.scheduled_jobs:
				scheduled_job_type: "ScheduledJobType" = traquent.get_doc("Scheduled Job Type", job.name)
	...
	def sync_scheduled_job_type(self):
		...
		def get_scheduled_job() -> "ScheduledJobType":
			if scheduled_script := traquent.db.get_value("Scheduled Job Type", {"server_script": self.name}):
				return traquent.get_doc("Scheduled Job Type", scheduled_script)
			else:
				return traquent.get_doc({"doctype": "Scheduled Job Type", "server_script": self.name})
		...
		get_scheduled_job().update(
			{
				"method": traquent.scrub(f"{self.name}-{self.event_frequency}"),
		...
		traquent.msgprint(_("Scheduled execution for script {0} has updated").format(self.name), alert=True)
...
def get_autocompletion_items():
	...
	items = traquent.cache.get_value("server_script_autocompletion_items")
	if not items:
		items = get_keys(get_safe_globals())
		items = [{"value": d[0], "score": d[1]} for d in items]
		traquent.cache.set_value("server_script_autocompletion_items", items)
	return items
...
def execute_api_server_script(script: ServerScript, *args, **kwargs):
	...
	if traquent.session.user == "Guest" and not script.allow_guest:
