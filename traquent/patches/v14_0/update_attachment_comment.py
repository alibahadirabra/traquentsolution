...
def execute():
	...
	for name, content in traquent.get_all(
		...
		traquent.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in traquent.get_all(
		...
		if content and content.startswith("Removed "):
			traquent.db.set_value("Comment", name, "content", content[8:], update_modified=False)
