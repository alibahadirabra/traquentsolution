import traquent
from traquent.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			traquent.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			traquent.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({traquent.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if traquent.db.db_type == "mariadb":
		...
		values = (traquent.db.cur_db_name,)

	col_type = traquent.db.sql(query, values)
