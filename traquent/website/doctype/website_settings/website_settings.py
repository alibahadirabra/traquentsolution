...
from traquent.utils import cint, encode, get_request_site_address
from traquent.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.website.doctype.top_bar_item.top_bar_item import TopBarItem
		from traquent.website.doctype.website_route_redirect.website_route_redirect import (
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not traquent.db.get_single_value("Google Settings", "enable"):
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not traquent.flags.in_migrate:
					traquent.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = traquent.bold(_("Allow API Indexing Access"))
...
def get_website_settings(context=None):
	...
	context = traquent._dict(context or {})
	settings: "WebsiteSettings" = traquent.get_cached_doc("Website Settings")
...
#UPDATES --sevval
	...
	context.read_only_mode = traquent.flags.read_only
