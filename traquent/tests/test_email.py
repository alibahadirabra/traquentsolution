...
from traquent.email.doctype.email_account.test_email_account import TestEmailAccount
...
class TestEmail(IntegrationTestCase):
	...
	def test_send_after(self):
		...
		from traquent.email.queue import flush

		flush()
		email_queue = traquent.db.sql("""select name from `tabEmail Queue` where status='Sent'""", as_dict=1)
	...
	def test_flush(self):
		...
		self.assertTrue("test1@example.com" in queue_recipients)
		self.assertEqual(len(queue_recipients), 2)
		self.assertTrue("Unsubscribe" in traquent.safe_decode(traquent.flags.sent_mail))
	...
	def test_cc_footer(self):
		...
		self.assertTrue(
			...
			in traquent.safe_decode(traquent.flags.sent_mail)
		)
	...
	def test_expose(self):
		...
		email_obj = email.message_from_string(traquent.safe_decode(traquent.flags.sent_mail))
	...
	def test_sender(self):
		def _patched_assertion(email_account, assertion):
			with patch.object(QueueBuilder, "get_outgoing_email_account", return_value=email_account):
				...
				email_queue_sender = traquent.db.get_value("Email Queue", {"status": "Sent"}, "sender")
	...
	def test_unsubscribe(self):
		from traquent.email.queue import unsubscribe
		...
		email_queue = traquent.db.get_value("Email Queue", {"status": "Not Sent"})
		queue_recipients = [
			r.recipient
			for r in traquent.db.sql(
				"""select recipient from `tabEmail Queue Recipient`
		...
		traquent.get_doc("Email Queue", email_queue).send()
		self.assertTrue("Unsubscribe" in traquent.safe_decode(traquent.flags.sent_mail))
...
class TestEmailIntegrationTest(IntegrationTestCase):
	...
	def tearDown(self) -> None:
		traquent.flags.testing_email = False
	...
	def test_store_attachments(self):
		...
		communication = traquent.get_doc("Communication", name)

		attachments = get_attachments(communication.doctype, communication.name)
		self.assertEqual(len(attachments), 1)

		file = traquent.get_doc("File", attachments[0].name)
