...
def get_secret():
	from traquent.utils.password import get_encryption_key

	return traquent.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = traquent.safe_decode(
		traquent.local.flags.signed_query_string or getattr(traquent.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = traquent.request.method == "GET"
		valid_request_data = not (traquent.request.form or traquent.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return traquent.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(traquent.utils.cstr(p) for p in params.values())
	if not secret:
		secret = traquent.local.conf.get("secret") or "secret"
