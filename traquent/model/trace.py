	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if traquent.flags.in_test:
				traquent.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				traquent.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if traquent.flags.in_test:
					traquent.throw(str(e), AssertionError)
				else:
...
from traquent.deprecation_dumpster import model_trace_traced_field_context as traced_field_context
