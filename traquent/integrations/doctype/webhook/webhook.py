...
from traquent.utils.background_jobs import get_queues_timeout
...
class Webhook(Document):
	...
	if TYPE_CHECKING:
		from traquent.integrations.doctype.webhook_data.webhook_data import WebhookData
		from traquent.integrations.doctype.webhook_header.webhook_header import WebhookHeader
	...
	def validate_docevent(self):
		if self.webhook_doctype:
			is_submittable = traquent.get_value("DocType", self.webhook_doctype, "is_submittable")
			if not is_submittable and self.webhook_docevent in [
				"on_submit",
				"on_cancel",
				"on_update_after_submit",
			]:
				traquent.throw(_("DocType must be Submittable for the selected Doc Event"))

	def validate_condition(self):
		temp_doc = traquent.new_doc(self.webhook_doctype)
		if self.condition:
			try:
				traquent.safe_eval(self.condition, eval_locals=get_context(temp_doc))
			except Exception as e:
				traquent.throw(_("Invalid Condition: {}").format(e))
	...
	def validate_request_url(self):
		try:
			...
			traquent.throw(_("Check Request URL"), exc=e)
	...
	def validate_repeating_fields(self):
		...
		if len(webhook_data) != len(set(webhook_data)):
			traquent.throw(_("Same Field is entered more than once"))
	...
	def preview_meets_condition(self, preview_document):
		...
		try:
			...
			met_condition = traquent.safe_eval(self.condition, eval_locals=get_context(doc))
		except Exception as e:
	...
	def preview_request_body(self, preview_document):
		try:
			doc = traquent.get_cached_doc(self.webhook_doctype, preview_document)
			return traquent.as_json(get_webhook_data(doc, self))
		except Exception as e:
			traquent.local.message_log = []
...
def enqueue_webhook(doc, webhook) -> None:
	...
	try:
		webhook: Webhook = traquent.get_doc("Webhook", webhook.get("name"))
		request_url = webhook.request_url
		if webhook.is_dynamic_url:
			request_url = traquent.render_template(webhook.request_url, get_context(doc))
		headers = get_webhook_headers(doc, webhook)
		data = get_webhook_data(doc, webhook)

	except Exception as e:
		traquent.logger().debug({"enqueue_webhook_error": e})
	...
	for i in range(3):
		try:
			...
			r.raise_for_status()
			traquent.logger().debug({"webhook_success": r.text})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data, r)
			break

		except requests.exceptions.ReadTimeout as e:
			traquent.logger().debug({"webhook_error": e, "try": i + 1})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data)
...
def log_request(
	...
	request_log = traquent.get_doc(
		{
			"doctype": "Webhook Request Log",
			"webhook": webhook,
			"reference_doctype": doctype,
			"reference_document": docname,
			"user": traquent.session.user if traquent.session.user else None,
			"url": url,
			"headers": traquent.as_json(headers) if headers else None,
			"data": traquent.as_json(data) if data else None,
...
def get_webhook_data(doc, webhook):
	...
	if webhook.webhook_data:
		...
		data = traquent.render_template(webhook.webhook_json, get_context(doc))
