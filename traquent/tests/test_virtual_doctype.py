...
class VirtualDoctypeTest(Document):
	...
	def get_list():
		...
		return [traquent._dict(doc) for name, doc in data.items()]
...
class TestVirtualDoctypes(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		cls.addClassCleanup(traquent.flags.pop, "allow_doctype_export", None)
	...
	def test_insert_update_and_load_from_desk(self):
		...
		docname = traquent.response.docs[0]["name"]

		doc = traquent.get_doc(TEST_DOCTYPE_NAME, docname)
	...
	def test_multiple_doc_insert_and_get_list(self):
		doc1 = traquent.new_doc(doctype=TEST_DOCTYPE_NAME)
		doc1.append("child_table", {"name": "first", "some_fieldname": "first-value"})
		doc1.insert()
		doc2 = traquent.new_doc(doctype=TEST_DOCTYPE_NAME)
	...
	def test_delete_doc(self):
		doc = traquent.get_doc(doctype=TEST_DOCTYPE_NAME).insert()

		traquent.delete_doc(doc.doctype, doc.name)
