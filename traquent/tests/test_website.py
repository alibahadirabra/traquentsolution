...
import traquent
from traquent import get_hooks
from traquent.tests import IntegrationTestCase
from traquent.utils import set_request
from traquent.website.page_renderers.static_page import StaticPage
from traquent.website.serve import get_response, get_response_content
from traquent.website.utils import build_response, clear_website_cache, get_home_page
...
class TestWebsite(IntegrationTestCase):
	...
	def test_home_page(self):
		...
		role = traquent.get_doc(doctype="Role", role_name="home-page-test", desk_access=0).insert(
		...
		traquent.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		traquent.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		traquent.set_user("Administrator")
		traquent.db.set_value("Role", "home-page-test", "home_page", "")

		# home page via portal settings
		traquent.db.set_single_value("Portal Settings", "default_portal_home", "test-portal-home")

		traquent.set_user("test-user-for-home-page@example.com")
		traquent.cache.hdel("home_page", traquent.session.user)
		self.assertEqual(get_home_page(), "test-portal-home")

		traquent.db.set_single_value("Portal Settings", "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		traquent.db.set_single_value("Website Settings", "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		traquent.db.set_single_value("Website Settings", "home_page", None)
		clear_website_cache()
		...
		clear_website_cache()
		with patch.object(traquent, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(traquent, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			traquent, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_page_load(self):
		...
		html = traquent.safe_decode(response.get_data())
	...
	def test_app(self):
		...
		traquent.local.session_obj = None
	...
	def test_redirect(self):
		import traquent.hooks

		traquent.set_user("Administrator")

		traquent.hooks.website_redirects = [
		...
		delattr(traquent.hooks, "website_redirects")
		traquent.cache.delete_key("app_hooks")
	...
	def test_printview_page(self):
		traquent.db.value_cache[("DocType", "Language", "name")] = (("Language",),)
		traquent.set_user("Administrator")
	...
	def test_json_sidebar_data(self):
		traquent.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		traquent.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_caching(self):
		# to enable caching
		traquent.flags.force_website_cache = True
		...
		traquent.flags.force_website_cache = False
	...
	def test_resolve_class(self):
		from traquent.utils.jinja_globals import resolve_class

		context = traquent._dict(primary=True)
		...
		self.assertEqual(
			traquent.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(traquent, "get_hooks", patched_get_hooks):
			traquent.set_user("Administrator")
			traquent.hooks.app_include_js.append("test_app_include.js")
			traquent.hooks.app_include_css.append("test_app_include.css")
			traquent.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			traquent.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})
