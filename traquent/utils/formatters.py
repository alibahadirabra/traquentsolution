...
import traquent
from traquent.model.meta import get_field_currency, get_field_precision
...
def format_value(value, df=None, doc=None, currency=None, translated=False, format=None):
	...
	if isinstance(df, str):
		df = traquent._dict(fieldtype=df)
	...
	if not df:
		...
		default_currency = traquent.db.get_default("currency")
		...
		if not BLOCK_TAGS_PATTERN.search(value):
			return traquent.safe_decode(value).replace("\n", "<br>")

	elif df.get("fieldtype") == "Markdown Editor":
		return traquent.utils.markdown(value)

	elif df.get("fieldtype") == "Table MultiSelect":
		values = []
		meta = traquent.get_meta(df.options)
		link_field = next(df for df in meta.fields if df.fieldtype == "Link")
		for v in value:
			v.update({"__link_titles": doc.get("__link_titles")})
			formatted_value = traquent.format_value(v.get(link_field.fieldname, ""), link_field, v)
		...
		if isinstance(value, str):
			return traquent._(value, context=df.parent or "")
