...
class PushNotification:
	...
	def send_notification_to_user(
		...
		if strip_html:
			body = traquent.utils.strip_html(body)
	...
	def is_enabled(self) -> bool:
		...
		"""
		return sbool(
			traquent.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
		)
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		"""
		notification_settings = traquent.get_doc("Push Notification Settings")
		...
		token = traquent.generate_hash(length=48)
	...
	def _site_name(self) -> str:
		return urlparse(traquent.utils.get_url()).hostname

	@property
	def _site_protocol(self) -> str:
		return urlparse(traquent.utils.get_url()).scheme

	@property
	def _site_port(self) -> str:
		site_uri = urlparse(traquent.utils.get_url())
