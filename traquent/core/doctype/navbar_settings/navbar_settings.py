...
class NavbarSettings(Document):
	...
	if TYPE_CHECKING:
		from traquent.core.doctype.navbar_item.navbar_item import NavbarItem
...
def get_app_logo():
	app_logo = traquent.db.get_single_value("Navbar Settings", "app_logo", cache=True)
	if not app_logo:
		logos = traquent.get_hooks("app_logo_url")
		app_logo = logos[0]
		if len(logos) == 2:
			app_logo = logos[1]
...
def get_navbar_settings():
	return traquent.get_single("Navbar Settings")
...
def sync_table(key, hook):
	...
	for item in traquent.get_hooks(hook):
