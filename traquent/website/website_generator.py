...
from traquent.search.website_search import remove_document_from_index, update_index_for_path
from traquent.website.utils import cleanup_page_name, clear_cache


class WebsiteGenerator(Document):
	website = traquent._dict()
	...
	def on_trash(self):
		...
		if self.allow_website_search_indexing():
			traquent.enqueue(remove_document_from_index, path=self.route, enqueue_after_commit=True)
	...
	def send_indexing_request(self, operation_type="URL_UPDATED"):
		...
		if (
			traquent.db.get_single_value("Website Settings", "enable_google_indexing")
	...
	def remove_old_route_from_index(self):
		...
		if self.allow_website_search_indexing() or traquent.flags.in_test:
			return
	...
	def update_website_search_index(self):
		...
		"""
		if not self.allow_website_search_indexing() or traquent.flags.in_test:
			return

		if self.is_website_published():
			traquent.enqueue(update_index_for_path, path=self.route, enqueue_after_commit=True)
		elif self.route:
