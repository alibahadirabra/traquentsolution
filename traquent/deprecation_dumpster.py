...
def is_column_missing(e):
	...
	return traquent.db.is_missing_column(e)
...
def get_js(items):
	...
	for src in items:
		...
		if ".." in src or src[0] != "assets":
			traquent.throw(_("Invalid file path: {0}").format("/".join(src)))

		contentpath = os.path.join(traquent.local.sites_path, *src)
		with open(contentpath) as srcfile:
			code = traquent.utils.cstr(srcfile.read())
...
def test_runner_print_mandatory_fields(*args, **kwargs):
	from traquent.tests.utils.generators import print_mandatory_fields
...
def test_runner_get_test_record_log(doctype):
	from traquent.tests.utils.generators import TestRecordManager
...
def test_runner_main(*args, **kwargs):
	from traquent.commands.testing import main
...
def tests_get_system_setting(key):
	...
	return traquent.db.get_single_value("System Settings", key)
...
def model_trace_traced_field_context(*args, **kwargs):
	from traquent.tests.classes.context_managers import trace_fields
...
def tests_utils_get_dependencies(doctype):
	...
	import traquent
	from traquent.tests.utils.generators import get_modules
...
def traquent_get_test_records(doctype):
	...
	traquent.flags.deprecation_dumpster_invoked = True
