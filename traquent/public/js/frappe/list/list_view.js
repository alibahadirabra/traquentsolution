...
traquent.views.ListView = class ListView extends traquent.views.BaseList {
	static load_last_view() {
		const route = traquent.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = traquent.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			traquent.set_route(
				"list",
				traquent.router.doctype_layout || doctype,
				traquent.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		this.debounced_refresh = traquent.utils.debounce(
	...
	has_permissions() {
		return traquent.perm.has_perm(this.doctype, 0, "read");
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			traquent.set_route("");
			traquent.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(traquent.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = traquent.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${traquent.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		traquent.msgprint(
			traquent.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? traquent.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						traquent.boot.link_title_doctypes.includes(df.options)
					) {
						traquent.model.with_doctype(df.options, () => {
							const meta = traquent.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = traquent.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = traquent.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				traquent.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !traquent.boot.read_only) {
			const doctype_name = __(traquent.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && traquent.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		traquent.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = traquent.meta.get_docfield.bind(null, this.doctype);
		...
		if (traquent.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (traquent.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
	...
	before_refresh() {
		if (traquent.route_options && this.filter_area) {
	...
	before_render() {
		...
		traquent.model.user_settings.save(this.doctype, "last_view", this.view_name);
		...
		const $columns = this.columns
			.map((col) => {
				let classes = [
					...
					traquent.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
	...
	get_column_html(col, doc) {
		...
		let translated_doctypes = traquent.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return traquent.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? traquent.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${traquent.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${traquent.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (traquent.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${traquent.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${traquent.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			traquent.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_tags_html(user_tags, limit, colored = false) {
		let get_tag_html = (tag) => {
			...
			if (tag) {
				if (colored) {
					color = traquent.get_palette(tag);
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			traquent.router.slug(traquent.router.doctype_layout || this.doctype)
	...
	get_subject_text(doc, title) {
		...
		if (traquent.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return traquent.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = traquent.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = traquent.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !traquent.utils.is_url(url)) {
			url = traquent.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = traquent.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = traquent.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				traquent.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			traquent.ui.toggle_like($this, doctype, name);
	...
	setup_realtime_updates() {
		...
		traquent.realtime.doctype_subscribe(this.doctype);
		traquent.realtime.off("list_update");
		traquent.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		traquent.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		traquent.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = traquent.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return traquent.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (traquent.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (traquent.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					traquent.set_route("list", "data-import", {
		...
		if (
			traquent.model.can_create("Custom Field") &&
			traquent.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						traquent.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						traquent.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (traquent.user.has_role("System Manager") && traquent.boot.developer_mode) {
			...
			items.push({
				...
				action: () => traquent.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		traquent.model.with_doctype(this.doctype, () => {
			new ListSettings({
				...
				meta: traquent.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (traquent.model.has_workflow(this.doctype)) {
			const actions = traquent.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						traquent
							.xcall("traquent.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!traquent.model.has_workflow(this.doctype)) return;
		...
		traquent
			.xcall("traquent.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				traquent.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					traquent.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk edit
		if (has_editable_fields(doctype) && !traquent.model.has_workflow(doctype)) {
		...
		// bulk printing
		if (traquent.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			traquent.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!traquent.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (traquent.model.can_cancel(doctype) && !traquent.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (traquent.model.can_delete(doctype) && !traquent.model.has_workflow(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && traquent.route_options) {
			params = new Map(Object.entries(traquent.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = traquent.meta.get_doctype_for_field(this.doctype, field);
			}
...
traquent.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return traquent.views.list_view[route];
};
