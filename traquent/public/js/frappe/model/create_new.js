...
$.extend(traquent.model, {
	...
	get_new_doc: function (doctype, parent_doc, parentfield, with_mandatory_children) {
		...
		var doc = {
			docstatus: 0,
			doctype: doctype,
			name: traquent.model.get_new_name(doctype),
			__islocal: 1,
			__unsaved: 1,
			owner: traquent.session.user,
		};
		traquent.model.set_default_values(doc, parent_doc);
		...
		if (parent_doc) {
			...
			traquent.provide("traquent.model.docinfo." + doctype + "." + doc.name);
		}

		traquent.model.add_to_locals(doc);

		if (with_mandatory_children) {
			traquent.model.create_mandatory_children(doc);
		}
		...
		if (traquent.route_options && traquent.route_options.name_field) {
			var meta = traquent.get_meta(doctype);
			// set title field / name as name
			if (meta.autoname && meta.autoname.indexOf("field:") !== -1) {
				doc[meta.autoname.substr(6)] = traquent.route_options.name_field;
			} else if (meta.autoname && meta.autoname === "prompt") {
				doc.__newname = traquent.route_options.name_field;
			} else if (meta.title_field) {
				doc[meta.title_field] = traquent.route_options.name_field;
			}

			delete traquent.route_options.name_field;
		}

		// set route options
		if (traquent.route_options && !doc.parent) {
			$.each(traquent.route_options, function (fieldname, value) {
				var df = traquent.meta.has_field(doctype, fieldname);
				if (
					df &&
	...
	make_new_doc_and_get_name: function (doctype, with_mandatory_children) {
		return traquent.model.get_new_doc(doctype, null, null, with_mandatory_children).name;
	},

	get_new_name: function (doctype) {
		// random hash is added to idenity mislinked files when doc is not saved and file is uploaded.
		return traquent.router.slug(`new-${doctype}-${traquent.utils.get_random(10)}`);
	},

	set_default_values: function (doc, parent_doc) {
		let doctype = doc.doctype;
		let docfields = traquent.meta.get_docfields(doctype);
		let updated = [];

		// Table types should be initialized
		let fieldtypes_without_default = traquent.model.no_value_type.filter(
			(fieldtype) => !traquent.model.table_fields.includes(fieldtype)
		);
		docfields.forEach((f) => {
			if (
			...
			let v = traquent.model.get_default_value(f, doc, parent_doc);
			if (v) {
	...
	create_mandatory_children: function (doc) {
		var meta = traquent.get_meta(doc.doctype);
		if (meta && meta.istable) return;

		// create empty rows for mandatory table fields
		traquent.meta.get_docfields(doc.doctype).forEach(function (df) {
			if (df.fieldtype === "Table" && df.reqd) {
				traquent.model.add_child(doc, df.fieldname);
			}
		});
	},
	...
	get_default_value: function (df, doc, parent_doc) {
		var user_default = "";
		var user_permissions = traquent.defaults.get_user_permissions();
		let allowed_records = [];
		let default_doc = null;
		let value = null;
		if (user_permissions) {
			({ allowed_records, default_doc } = traquent.perm.filter_allowed_docs_for_doctype(
		...
		if (df.fieldtype === "Link" && df.options !== "User") {
			...
			if (has_user_permissions && default_doc) {
				...
				if (!df.ignore_user_permissions) {
					var user_defaults = traquent.defaults.get_user_defaults(df.options);
				...
				if (!user_default) {
					user_default = traquent.defaults.get_user_default(df.fieldname);
				}
				if (
					!user_default &&
					...
					user_default = traquent.boot.user.last_selected_values[df.options];
				}
		...
		if (!value || df["default"]) {
			...
			if (default_val == "__user" || default_val.toLowerCase() == "user") {
				value = traquent.session.user;
			} else if (default_val == "user_fullname") {
				value = traquent.session.user_fullname;
			} else if (default_val == "Today") {
				value = traquent.datetime.get_today();
			} else if ((default_val || "").toLowerCase() === "now") {
				if (df.fieldtype == "Time") {
					value = traquent.datetime.now_time();
				} else {
					// datetime fields are stored in system tz
					value = traquent.datetime.system_datetime();
				}
			} else if (default_val[0] === ":") {
				var boot_doc = traquent.model.get_default_from_boot_docs(df, doc, parent_doc);
	...
	get_default_from_boot_docs: function (df, doc, parent_doc) {
		...
		if (traquent.get_list(df["default"]).length > 0) {
			var ref_fieldname = df["default"].slice(1).toLowerCase().replace(" ", "_");
			var ref_value = parent_doc
				? parent_doc[ref_fieldname]
				: traquent.defaults.get_user_default(ref_fieldname);
			var ref_doc = ref_value ? traquent.get_doc(df["default"], ref_value) : null;
	...
	add_child: function (parent_doc, doctype, parentfield, idx) {
		...
		if (arguments.length === 2) {
			parentfield = doctype;
			doctype = traquent.meta.get_field(parent_doc.doctype, parentfield).options;
		}

		// create row doc
		idx = idx ? idx - 0.1 : (parent_doc[parentfield] || []).length + 1;

		var child = traquent.model.get_new_doc(doctype, parent_doc, parentfield);
		child.idx = idx;
	...
	copy_doc: function (doc, from_amend, parent_doc, parentfield) {
		...
		var newdoc = traquent.model.get_new_doc(doc.doctype, parent_doc, parentfield);

		for (var key in doc) {
			// dont copy name and blank fields
			var df = traquent.meta.get_docfield(doc.doctype, key);
			...
			if (
				...
				if (traquent.model.table_fields.includes(df.fieldtype)) {
					for (var i = 0, j = value.length; i < j; i++) {
						var d = value[i];
						traquent.model.copy_doc(d, from_amend, newdoc, df.fieldname);
					}
				} else {
					newdoc[key] = doc[key];
				}
		...
		var user = traquent.session.user;
	...
	open_mapped_doc: function (opts) {
		...
		return traquent.call({
			type: "POST",
			method: "traquent.model.mapper.make_mapped_doc",
			args: {
				method: opts.method,
			...
			callback: function (r) {
				if (!r.exc) {
					...
					traquent.set_route("Form", r.message.doctype, r.message.name);
				}
			},
		});
	},
...
traquent.create_routes = {};
traquent.new_doc = function (doctype, opts, init_callback) {
	if (doctype === "File") {
	...
	return new Promise((resolve) => {
		if (opts && $.isPlainObject(opts)) {
			traquent.route_options = opts;
		}
		traquent.model.with_doctype(doctype, function () {
			if (traquent.create_routes[doctype]) {
				traquent.set_route(traquent.create_routes[doctype]).then(() => resolve());
			} else {
				traquent.ui.form
