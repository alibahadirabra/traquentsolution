...
def trigger_scheduler_event(context: CliCtxObj, event):
	...
	for site in context.sites:
		try:
			...
			traquent.connect()
			try:
				traquent.get_doc("Scheduled Job Type", {"method": event}).execute()
...
def enable_scheduler(context: CliCtxObj):
	...
	for site in context.sites:
		try:
			traquent.init(site)
			traquent.connect()
			traquent.utils.scheduler.enable_scheduler()
			traquent.db.commit()
...
def disable_scheduler(context: CliCtxObj):
	...
	for site in context.sites:
		try:
			traquent.init(site)
			traquent.connect()
			traquent.utils.scheduler.disable_scheduler()
			traquent.db.commit()
...
def scheduler(context: CliCtxObj, state: str, format: str, verbose: bool = False, site: str | None = None):
	...
	import traquent
	from traquent.utils.scheduler import is_scheduler_inactive, toggle_scheduler
	...
	with traquent.init_site(site=site):
		match state:
			case "status":
				traquent.connect()
...
def doctor(context: CliCtxObj, site=None):
	...
	from traquent.utils.doctor import doctor as _doctor
...
def show_pending_jobs(context: CliCtxObj, site=None):
	...
	from traquent.utils.doctor import pending_jobs as _pending_jobs
...
def purge_jobs(site=None, queue=None, event=None):
	...
	from traquent.utils.doctor import purge_pending_jobs

	traquent.init(site or "")
...
def start_worker(queue, quiet=False, rq_username=None, rq_password=None, burst=False, strategy=None):
	...
	from traquent.utils.background_jobs import start_worker
...
def start_worker_pool(queue, quiet=False, num_workers=2, burst=False):
	...
	from traquent.utils.background_jobs import start_worker_pool
...
def ready_for_migration(context: CliCtxObj, site=None):
	from traquent.utils.doctor import any_job_pending
