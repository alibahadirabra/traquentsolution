...
import traquent
from traquent.core.doctype.data_import.data_import import export_json, import_doc
...
def sync_fixtures(app=None):
	...
	traquent.flags.in_fixtures = True

	for app in apps:
		import_fixtures(app)
		import_custom_scripts(app)

	traquent.flags.in_fixtures = False


def import_fixtures(app):
	fixtures_path = traquent.get_app_path(app, "fixtures")
	...
	for fname in fixture_files:
		...
		file_path = traquent.get_app_path(app, "fixtures", fname)
		try:
			import_doc(file_path, sort=True)
		except (ImportError, traquent.DoesNotExistError) as e:
...
def import_custom_scripts(app):
	...
	scripts_folder = traquent.get_app_path(app, "fixtures", "custom_scripts")
...
def export_fixtures(app=None):
	...
	if app:
		...
		apps = traquent.get_installed_apps()
	for app in apps:
		fixture_auto_order = bool(next(iter(traquent.get_hooks("fixture_auto_order", app_name=app)), False))
		fixtures = traquent.get_hooks("fixtures", app_name=app)
		prefix = None
		for index, fixture in enumerate(fixtures, start=1):
			...
			if not os.path.exists(traquent.get_app_path(app, "fixtures")):
				os.mkdir(traquent.get_app_path(app, "fixtures"))

			filename = traquent.scrub(fixture)
			if prefix:
			...
			export_json(
				fixture,
				traquent.get_app_path(app, "fixtures", filename + ".json"),
				filters=filters,
