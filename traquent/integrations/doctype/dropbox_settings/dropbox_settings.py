...
from traquent.integrations.offsite_backup_utils import (
	get_chunk_site,
...
from traquent.utils import cint, encode, get_backups_path, get_files_path, get_request_site_address
from traquent.utils.background_jobs import enqueue
from traquent.utils.backups import new_backup
...
class DropboxSettings(Document):
	...
	def onload(self):
		if not self.app_access_key and traquent.conf.dropbox_access_key:
...
def take_backup():
	...
	enqueue(
		"traquent.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
	traquent.msgprint(_("Queued for backup. It may take a few minutes to an hour."))
...
def take_backups_if(freq):
	if traquent.db.get_single_value("Dropbox Settings", "backup_frequency") == freq:
...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	...
	try:
		if cint(traquent.db.get_single_value("Dropbox Settings", "enabled")):
			validate_file_size()
			...
			if cint(traquent.db.get_single_value("Dropbox Settings", "send_email_for_successful_backup")):
...
def backup_to_dropbox(upload_db_backup=True):
	...
	if upload_db_backup:
		if traquent.flags.create_new_backup:
...
def upload_from_folder(path, is_private, dropbox_folder, dropbox_client, did_not_upload, error_log):
	...
	if is_fresh_upload():
		...
		response = traquent._dict({"entries": []})

	path = str(path)

	for f in traquent.get_all(
		"File",
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(traquent.get_traceback())
...
def upload_file_to_dropbox(filename, folder, dropbox_client):
	...
	try:
		...
		if isinstance(e.error, dropbox.files.UploadError):
			...
			error += traquent.get_traceback()
			traquent.log_error(error)
		else:
			raise
...
def update_file_dropbox_status(file_name):
	traquent.db.set_value("File", file_name, "uploaded_to_dropbox", 1, update_modified=False)


def is_fresh_upload():
	file_name = traquent.db.get_value("File", {"uploaded_to_dropbox": 1}, "name")
...
def get_dropbox_settings(redirect_uri=False):
	...
	settings = traquent.get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or traquent.conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
		if settings.app_secret_key
		else traquent.conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				+ "/api/method/traquent.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish"
			}
		)
...
def dropbox_auth_finish():
	...
	if not callback.state or not callback.code:
		traquent.respond_as_web_page(
			...
			indicator_color="red",
			http_status_code=traquent.AuthenticationError.http_status_code,
		)
		return
	...
	traquent.local.response["location"] = "/app/dropbox-settings"
...
def set_dropbox_token(access_token, refresh_token=None):
	...
	dropbox_settings = traquent.get_single("Dropbox Settings")
