...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(traquent.conf.db_type)
...
def get_query(*args, **kwargs) -> QueryBuilder:
	from traquent.database.query import Engine
...
def DocType(*args, **kwargs):
	return traquent.qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
	return traquent.qb.Table(*args, **kwargs)
...
def patch_query_execute():
	...
	def execute_query(query, *args, **kwargs):
		...
		result = traquent.db.sql(query, params, *args, **kwargs)  # nosemgrep
	...
	def prepare_query(query):
		...
		if traquent.flags.in_safe_exec:
			from traquent.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX, check_safe_sql_query
			...
			if not check_safe_sql_query(query, throw=False):
				...
				if len(callstack) >= 3 and SERVER_SCRIPT_FILE_PREFIX in callstack[2].filename:
					raise traquent.PermissionError("Only SELECT SQL allowed in scripting")

		return query, param_collector.get_parameters()

	builder_class = traquent.qb._BuilderClasss
...
def patch_query_aggregation():
	...
	traquent.qb.min = _min
	traquent.qb.avg = _avg
	traquent.qb.sum = _sum
