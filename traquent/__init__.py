...
It is the framework which powers traquent. It is pretty generic and can
...
import traquent
from traquent.query_builder import (
	get_query,
	get_query_builder,
...
from traquent.utils.data import cint, cstr, sbool
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from traquent.translate import get_all_translations
	from traquent.utils import is_html, strip_html_tags
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from traquent.database.mariadb.database import MariaDBDatabase
	from traquent.database.postgres.database import PostgresDatabase
	from traquent.email.doctype.email_queue.email_queue import EmailQueue
	from traquent.model.document import Document
	from traquent.query_builder.builder import MariaDB, Postgres
	from traquent.utils.redis_wrapper import RedisWrapper
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force=False) -> None:
	...
	setup_module_map(include_all_apps=not (traquent.request or traquent.job or traquent.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	"""
	from traquent.database import get_db
...
def setup_redis_cache_connection():
	...
	if not cache:
		from traquent.utils.redis_wrapper import setup_cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from traquent.utils import get_traceback
...
def toast(message: str, indicator: Literal["blue", "green", "orange", "red", "yellow"] | None = None):
	traquent.msgprint(message, indicator=indicator, alert=True)
...
def create_folder(path, with_init=False):
	...
	from traquent.utils import touch_file
...
def get_roles(username=None) -> list[str]:
	...
	return traquent.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from traquent.utils import md_to_html
	...
	from traquent.email.doctype.email_queue.email_queue import QueueBuilder
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from traquent.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from traquent.utils import sanitize_html
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import traquent.utils.caching
	from traquent.website.router import clear_routing_cache

	if doctype:
		traquent.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		traquent.cache_manager.clear_user_cache(user)
	else:  # everything
		# Delete ALL keys associated with this site.
		keys_to_delete = set(traquent.cache.get_keys(""))
		for key in traquent.get_hooks("persistent_cache_keys"):
			keys_to_delete.difference_update(traquent.cache.get_keys(key))
		traquent.cache.delete_value(list(keys_to_delete), make_keys=False)
	...
	traquent.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from traquent.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	out = traquent.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		...
		traquent.flags.error_message = _("No permission for {0}").format(document_label)
...
def new_doc(
	...
	**kwargs,
	...
	from traquent.model.create_new import get_new_doc
...
def get_doc(*args, **kwargs):
	...
	doc = traquent.model.document.get_doc(*args, **kwargs)
...
def get_meta_module(doctype):
	import traquent.modules

	return traquent.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	import traquent.model.delete_doc

	return traquent.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return traquent.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = traquent.utils.strip(content)
...
def import_doc(path):
	...
	from traquent.core.doctype.data_import.data_import import import_doc
...
def copy_doc(doc: "Document", ignore_no_copy: bool = True) -> "Document":
	...
	from traquent.model.base_document import BaseDocument
...
def redirect(url):
	...
	from traquent.exceptions import Redirect
...
def build_match_conditions(doctype, as_condition=True):
	...
	import traquent.desk.reportview

	return traquent.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)
...
def get_list(doctype, *args, **kwargs):
	...
	return traquent.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)
...
def as_json(obj: dict | list, indent=1, separators=None, ensure_ascii=True) -> str:
	...
	from traquent.utils.response import json_handler
...
def format_value(*args, **kwargs):
	...
	import traquent.utils.formatters

	return traquent.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from traquent.utils.pdf import get_pdf
	from traquent.website.serve import get_response_without_exception_handling
...
def attach_print(
	...
	from traquent.translate import print_language
	from traquent.utils import scrub_urls
	from traquent.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import traquent.realtime

	return traquent.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return traquent.realtime.publish_realtime(*args, **kwargs)
...
def enqueue(*args, **kwargs):
	...
	"""
	import traquent.utils.background_jobs

	return traquent.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return traquent.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(module=None, with_more_info=False, allow_site=True, filter=None, max_size=100_000, file_count=20):
	...
	from traquent.utils.logger import get_logger
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from traquent.utils.safe_exec import safe_eval
...
def get_active_domains():
	from traquent.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	if meta.track_changes:
		...
		from traquent.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from traquent.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from traquent.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from traquent.desk.search import sanitize_searchfield
...
from traquent.utils.error import log_error
