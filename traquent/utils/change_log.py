...
def get_change_log(user=None):
	...
	last_known_versions = traquent._dict(
		json.loads(traquent.db.get_value("User", user, "last_known_versions") or "{}")
	)
...
def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(traquent.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = traquent.read_file(file_path)
...
def get_versions():
	...
	versions = {}
	for app in traquent.get_installed_apps(_ensure_on_bench=True):
		app_hooks = traquent.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = traquent.get_attr(app + ".__version__")
...
def check_for_update():
	...
	updates = traquent._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(traquent.cache.sismember("changelog-update-user-set", traquent.session.user))
...
def add_message_to_redis(update_json):
	...
	traquent.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in traquent.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in traquent.get_roles(user)]
...
def show_update_popup():
	...
	update_info = traquent.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if traquent.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				app = traquent._dict(app)
				security_msg = ""
				if app.security_issues:
					...
					security_msg = security_msg.format(traquent.bold(app.security_issues))
	...
	if update_message:
		...
		traquent.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = traquent.get_app_path(app, "..", "pyproject.toml")
