...
from traquent.utils import cint, get_datetime, getdate, has_common, now_datetime, nowdate
from traquent.utils.dashboard import cache_source
from traquent.utils.data import format_date
from traquent.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_reports = [traquent.db.escape(report) for report in get_allowed_report_names()]
	allowed_modules = [
...
def has_permission(doc, ptype, user):
	...
	if doc.roles:
		...
		allowed_doctypes = traquent.permissions.get_doctypes_with_read()
...
def get(
	...
	if chart_name:
		chart: DashboardChart = traquent.get_doc("Dashboard Chart", chart_name)
	else:
		chart = traquent._dict(traquent.parse_json(chart))
	...
	filters = traquent.parse_json(filters) or traquent.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	...
	if traquent.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard_link = traquent.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = traquent.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_group_by_chart_config(chart, filters) -> dict | None:
	...
	group_by_field_field = traquent.get_meta(doctype).get_field(
		group_by_field
	)  # get info about @group_by_field

	if data and group_by_field_field.fieldtype == "Link":  # if @group_by_field is link
		title_field = traquent.get_meta(group_by_field_field.options)  # get title field
		if title_field.title_field:  # if has title_field
			for item in data:  # replace chart labels from name to title value
				item.name = traquent.get_value(group_by_field_field.options, item.name, title_field.title_field)
...
class DashboardChart(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.desk.doctype.dashboard_chart_field.dashboard_chart_field import DashboardChartField
	...
	def on_update(self):
		traquent.cache.delete_key(f"chart-data:{self.name}")
	...
	def validate(self):
		if not traquent.conf.developer_mode and self.is_standard:
			traquent.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			traquent.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and traquent.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			traquent.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				traquent.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				traquent.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				traquent.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if traquent.get_meta(self.document_type).issingle:
			traquent.throw(_("You cannot create a dashboard chart from single DocTypes"))
