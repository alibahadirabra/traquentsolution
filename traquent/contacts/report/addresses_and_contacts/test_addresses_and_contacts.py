...
from traquent.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from traquent.tests import IntegrationTestCase


def get_custom_linked_doctype():
	if bool(traquent.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if traquent.flags.test_address_created:
		return

	address = traquent.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	traquent.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if traquent.flags.test_contact_created:
		return
	...
	traquent.flags.test_contact_created = True
