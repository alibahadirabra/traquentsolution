...
from traquent.utils import cint, get_fullname, get_link_to_form, getdate
...
class EnergyPointLog(Document):
	...
	def validate(self):
		...
		if self.type in ["Appreciation", "Criticism"] and self.user == self.owner:
			traquent.throw(_("You cannot give review points to yourself"))
	...
	def map_milestone_reference(self):
		...
		if self.reference_doctype == "Milestone":
			self.reference_doctype, self.reference_name = traquent.db.get_value(
	...
	def after_insert(self):
		...
		traquent.cache.hdel("energy_points", self.user)

		if self.type != "Review" and traquent.get_cached_value(
	...
	def on_trash(self):
		if self.type == "Revert":
			reference_log = traquent.get_doc("Energy Point Log", self.revert_of)
	...
	def revert(self, reason, ignore_permissions=False):
		...
		if self.type != "Auto":
			traquent.throw(_("This document cannot be reverted"))
...
def get_notification_message(doc):
	...
	if doc.type == "Auto":
		owner_name = traquent.bold("You")
		if points == 1:
			message = _("{0} gained {1} point for {2} {3}")
		else:
			message = _("{0} gained {1} points for {2} {3}")
		message = message.format(owner_name, traquent.bold(points), doc.rule, get_title_html(title))
		...
		message = message.format(traquent.bold(owner_name), get_title_html(title), traquent.bold(points))
		...
		message = message.format(traquent.bold(owner_name), get_title_html(title))

	return message


def get_alert_dict(doc):
	alert_dict = traquent._dict()
	owner_name = get_fullname(doc.owner)
	if doc.reference_doctype:
		doc_link = get_link_to_form(doc.reference_doctype, doc.reference_name)
	points = doc.points
	bold_points = traquent.bold(doc.points)
	if doc.type == "Auto":
		if points == 1:
...
def create_energy_points_log(ref_doctype, ref_name, doc, apply_only_once=False):
	...
	if doc.rule:
		...
		if log_exists:
			return traquent.get_doc("Energy Point Log", log_exists)

	new_log = traquent.new_doc("Energy Point Log")
...
def check_if_log_exists(ref_doctype, ref_name, rule, user=None):
	...
	return traquent.db.exists("Energy Point Log", filters)
...
def review(doc, points, to_user, reason, review_type="Appreciation"):
	...
	if current_review_points < points:
		traquent.msgprint(_("You do not have enough review points"))
		return
...
def send_summary(timespan):
	...
	if not is_email_notifications_enabled_for_type(traquent.session.user, "Energy Point"):
		return

	from_date = traquent.utils.add_to_date(None, weeks=-1)
	if timespan == "Monthly":
		from_date = traquent.utils.add_to_date(None, months=-1)
