...
function authenticate_with_traquent(socket, next) {
	...
	socket.traquent_request = (path, args = {}, opts = {}) => {
	...
	socket
		.traquent_request("/api/method/traquent.realtime.get_user_info")
		.then((res) => res.json())
		.then(({ message }) => {
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-traquent-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-traquent-site-name"]);
	} else if (
		conf.default_site &&
...
module.exports = authenticate_with_traquent;
