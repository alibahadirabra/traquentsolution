...
import traquent
from traquent.installer import parse_app_name
from traquent.model.document import Document
from traquent.tests import IntegrationTestCase, MockedRequestTestCase
...
from traquent.utils.change_log import (
	get_source_url,
	parse_github_url,
)
...
from traquent.utils.dateutils import get_dates_from_timegrain
from traquent.utils.diff import _get_value_from_version, get_version_diff, version_query
from traquent.utils.identicon import Identicon
from traquent.utils.image import optimize_image, strip_exif_data
from traquent.utils.make_random import can_make, get_random, how_many
from traquent.utils.response import json_handler
from traquent.utils.synchronization import LockTimeoutError, filelock
...
class TestHTMLUtils(IntegrationTestCase):
	def test_clean_email_html(self):
		from traquent.utils.html_utils import clean_email_html
...
class TestValidationUtils(IntegrationTestCase):
	...
	def test_valid_phone(self):
		...
		self.assertRaises(
			traquent.InvalidPhoneNumberError,
			validate_phone_number_with_country_code,
			"+420 1234567890",
			"field",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(traquent.InvalidNameError, validate_name, name, True)
...
class TestPythonExpressions(IntegrationTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(traquent.ValidationError, validate_python_code, expr)
...
class TestDiffUtils(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = traquent.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(traquent.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				traquent.utils.get_first_day_of_week("2020-12-25"),
				traquent.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				traquent.utils.get_first_day_of_week("2020-12-20"),
				traquent.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			traquent.utils.get_first_day_of_week("2020-12-25"),
			traquent.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			traquent.utils.get_first_day_of_week("2020-12-21"),
			traquent.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			traquent.utils.get_last_day_of_week("2020-12-24"),
			traquent.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			traquent.utils.get_last_day_of_week("2020-12-28"),
			traquent.utils.getdate("2021-01-02"),
		)

	def test_is_last_day_of_the_month(self):
		self.assertEqual(traquent.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(traquent.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestResponse(IntegrationTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				traquent.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestXlsxUtils(IntegrationTestCase):
	def test_unescape(self):
		from traquent.utils.xlsxutils import handle_html
...
class TestLinkTitle(IntegrationTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		"""
		custom_doctype = traquent.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		from traquent.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = traquent.local.response["_link_titles"]
...
class TestIntrospectionMagic(IntegrationTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(traquent.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(traquent.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(traquent.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(traquent.get_newargs(lambda: None, args), {})
...
class TestMiscUtils(IntegrationTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(traquent.local.site, get_sites())
...
class TestTBSanitization(IntegrationTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = traquent._dict({"password": "42", "pwd": "42", "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
...
class TestRounding(IntegrationTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(traquent.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestURLTrackers(IntegrationTestCase):
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": traquent.get_doc("UTM Source", "test_source"),
			"utm_medium": traquent.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": traquent.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}
