...
from traquent.desk.desktop import get_workspace_sidebar_items, save_new_widget
from traquent.desk.utils import validate_route_conflict
from traquent.model.document import Document
from traquent.model.rename_doc import rename_doc
from traquent.modules.export_file import delete_folder, export_to_files
...
class Workspace(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.desk.doctype.workspace_chart.workspace_chart import WorkspaceChart
		from traquent.desk.doctype.workspace_custom_block.workspace_custom_block import WorkspaceCustomBlock
		from traquent.desk.doctype.workspace_link.workspace_link import WorkspaceLink
		from traquent.desk.doctype.workspace_number_card.workspace_number_card import WorkspaceNumberCard
		from traquent.desk.doctype.workspace_quick_list.workspace_quick_list import WorkspaceQuickList
		from traquent.desk.doctype.workspace_shortcut.workspace_shortcut import WorkspaceShortcut
	...
	def validate(self):
		...
		if self.public and not is_workspace_manager() and not disable_saving_as_public():
			traquent.throw(_("You need to be Workspace Manager to edit this document"))
		...
		try:
			...
			traquent.throw(_("Content data shoud be a list"))

		for d in self.get("links"):
			if d.link_type == "Report" and d.is_query_report != 1:
				d.report_ref_doctype = traquent.get_value("Report", d.link_to, "ref_doctype")
	...
	def clear_cache(self):
		super().clear_cache()
		if self.for_user:
			traquent.cache.hdel("bootinfo", self.for_user)
		else:
	...
	def on_update(self):
		...
		if traquent.conf.developer_mode and self.public:
			if self.module:
		...
		for link in self.links:
			...
			if link.type == "Card Break":
				if card_links and (
					...
					or current_card.get("only_for") == traquent.get_system_settings("country")
				):
				...
				current_card = link
				card_links = []
			elif not link.get("only_for") or link.get("only_for") == traquent.get_system_settings("country"):
...
def disable_saving_as_public():
	return (
		...
		or traquent.flags.in_uninstall
...
def get_report_type(report):
	report_type = traquent.get_value("Report", report, "report_type")
...
def new_page(new_page):
	...
	if page.get("public") and not is_workspace_manager():
		...
		not page.get("public") and page.get("for_user") != traquent.session.user and not is_workspace_manager()
	):
		traquent.throw(_("Cannot create private workspace of other users"), traquent.PermissionError)

	elif not traquent.has_permission(doctype="Workspace", ptype="create"):
		traquent.flags.error_message = _("User {0} does not have the permission to create a Workspace.").format(
			traquent.bold(traquent.session.user)
		)
		raise traquent.PermissionError

	doc = traquent.new_doc("Workspace")
...
def update_page(name, title, icon, indicator_color, parent, public):
	...
	if not doc.get("public") and doc.get("for_user") != traquent.session.user and not is_workspace_manager():
		traquent.throw(
	...
	if doc:
		child_docs = traquent.get_all("Workspace", filters={"parent_page": doc.title, "public": doc.public})
		doc.title = title
		doc.icon = icon
		doc.indicator_color = indicator_color
		doc.parent_page = parent
		if doc.public != public:
			doc.sequence_id = traquent.db.count("Workspace", {"public": public}, cache=True)
			doc.public = public
		doc.for_user = "" if public else doc.for_user or traquent.session.user
		...
		if child_docs:
			for child in child_docs:
				child_doc = traquent.get_doc("Workspace", child.name)
				child_doc.parent_page = doc.title
				if child_doc.public != public:
					child_doc.public = public
				child_doc.for_user = "" if public else child_doc.for_user or traquent.session.user
...
def last_sequence_id(doc):
	doc_exists = traquent.db.exists({"doctype": "Workspace", "public": doc.public, "for_user": doc.for_user})
...
def get_page_list(fields, filters):
	return traquent.get_all("Workspace", fields=fields, filters=filters, order_by="sequence_id asc")


def is_workspace_manager():
	return "Workspace Manager" in traquent.get_roles()
