...
from traquent.automation.doctype.assignment_rule.assignment_rule import get_repeated
from traquent.contacts.doctype.contact.contact import (
	get_contacts_linked_from,
	get_contacts_linking_to,
)
from traquent.core.doctype.communication.email import make
...
from traquent.utils.background_jobs import get_jobs
...
class AutoRepeat(Document):
	...
	if TYPE_CHECKING:
		from traquent.automation.doctype.auto_repeat_day.auto_repeat_day import AutoRepeatDay
	...
	def on_update(self):
		traquent.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		traquent.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", "")
		traquent.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def validate_reference_doctype(self):
		if traquent.flags.in_test or traquent.flags.in_patch:
			return
		if not traquent.get_meta(self.reference_doctype).allow_auto_repeat:
			traquent.throw(
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not traquent.get_meta(self.reference_doctype).is_submittable:
			traquent.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(traquent.bold("Submit on Creation"))
			)
	...
	def validate_dates(self):
		...
		if self.end_date == self.start_date:
			traquent.throw(
				_("{0} should not be same as {1}").format(traquent.bold("End Date"), traquent.bold("Start Date"))
			)
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						traquent.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				traquent.throw(_("'Recipients' not specified"))
		...
		if len(set(auto_repeat_days)) != len(auto_repeat_days):
			...
			traquent.throw(
				_("Auto Repeat Day{0} {1} has been repeated.").format(
					plural, traquent.bold(", ".join(repeated_days))
				)
			)

	def update_auto_repeat_id(self):
		# check if document is already on auto repeat
		auto_repeat = traquent.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not traquent.flags.in_patch:
			traquent.throw(
				_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat)
			)
		else:
			traquent.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not traquent.flags.in_test:
				self.notify_error_to_user(error_log)

	def make_new_document(self):
		reference_doc = traquent.get_doc(self.reference_doctype, self.reference_document)
		new_doc = traquent.copy_doc(reference_doc, ignore_no_copy=False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = traquent.get_doc("Auto Repeat", self.name)
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = traquent.render_template(self.subject, {"doc": new_doc})
		...
		try:
			...
			).format(traquent.bold(_("Note")), traquent.bold(_("Allow Print for Draft")))
			attachments = None
		...
		if error_string:
			...
			message = traquent.render_template(self.message, {"doc": new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				traquent.msgprint(_("No contacts linked to document"), alert=True)
			else:
				self.recipients = ", ".join(email_ids)

	def disable_auto_repeat(self):
		traquent.db.set_value("Auto Repeat", self.name, "disabled", 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = traquent.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _("Auto Repeat failed for {0}").format(form_link)

		error_log_link = traquent.utils.get_link_to_form("Error Log", error_log.name)
...
def make_auto_repeat_entry():
	...
	if not jobs or enqueued_method not in jobs[traquent.local.site]:
		...
		traquent.enqueue(enqueued_method, data=data)


def create_repeated_entries(data):
	for d in data:
		doc = traquent.get_doc("Auto Repeat", d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				traquent.db.set_value("Auto Repeat", doc.name, "next_schedule_date", schedule_date)
...
def set_auto_repeat_as_completed():
	auto_repeat = traquent.get_all("Auto Repeat", filters={"status": ["!=", "Disabled"]})
	for entry in auto_repeat:
		doc = traquent.get_doc("Auto Repeat", entry.name)
		if doc.is_completed():
...
def make_auto_repeat(doctype, docname, frequency="Daily", start_date=None, end_date=None):
	...
	doc = traquent.new_doc("Auto Repeat")
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	...
	msg_preview = traquent.render_template(message, {"doc": doc})
	if subject:
		subject_preview = traquent.render_template(subject, {"doc": doc})
