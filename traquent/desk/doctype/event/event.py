...
from traquent.contacts.doctype.contact.contact import get_default_contact
...
from traquent.utils.user import get_enabled_system_users
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from traquent.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			traquent.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			traquent.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		if communications:
			for communication in communications:
				traquent.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				if comms := traquent.get_all("Communication", filters=filters, fields=["name"], distinct=True):
					for comm in comms:
						communication = traquent.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = traquent.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = traquent.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				traquent.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)
...
def delete_communication(event, reference_doctype, reference_docname):
	...
	comms = traquent.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = traquent.get_doc("Communication", comm.name).delete()
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={traquent.db.escape(user)})"""
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			traquent.set_user_lang(user.name, user.language)
			...
			traquent.sendmail(
				...
				subject=traquent._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[traquent._("Events in Today's Calendar"), "blue"],
			)
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = traquent.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				traquent.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = traquent.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					traquent.db.delete("Event", {"name": participation.parent})
					traquent.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = traquent.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			traquent.db.set_value("Event", event.name, "status", "Closed")
