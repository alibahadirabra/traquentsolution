...
import traquent
from traquent.core.doctype.rq_job.rq_job import RQJob, remove_failed_jobs, stop_job
from traquent.installer import update_site_config
from traquent.tests import IntegrationTestCase, UnitTestCase, timeout
from traquent.utils import cstr, execute_in_shell
from traquent.utils.background_jobs import get_job_status, is_job_enqueued
...
class TestRQJob(IntegrationTestCase):
	...
	def test_serialization(self):
		job = traquent.enqueue(method=self.BG_JOB, queue="short")
	...
	def test_configurable_ttl(self):
		traquent.conf.rq_job_failure_ttl = 600
		job = traquent.enqueue(method=self.BG_JOB, queue="short")
	...
	def test_get_list_filtering(self):
		...
		remove_failed_jobs()
		jobs = traquent.get_all("RQ Job", {"status": "failed"})
		...
		# Fail a job
		job = traquent.enqueue(method=self.BG_JOB, queue="short", fail=True)
		...
		non_failed_jobs = traquent.get_all("RQ Job", {"status": ("!=", "failed")})
		self.assertGreaterEqual(len(non_failed_jobs), 1)

		# Create a slow job and check if it's stuck in "Started"
		job = traquent.enqueue(method=self.BG_JOB, queue="short", sleep=10)
		time.sleep(3)
	...
	def test_delete_doc(self):
		job = traquent.enqueue(method=self.BG_JOB, queue="short")
		traquent.get_doc("RQ Job", job.id).delete()
	...
	def test_multi_queue_burst_consumption(self):
		for _ in range(3):
			for q in ["default", "short"]:
				traquent.enqueue(self.BG_JOB, sleep=1, queue=q)
	...
	def test_job_id_manual_dedup(self):
		job_id = "test_dedup"
		job = traquent.enqueue(self.BG_JOB, sleep=5, job_id=job_id)
	...
	def test_auto_job_dedup(self):
		job_id = "test_dedup"
		job1 = traquent.enqueue(self.BG_JOB, sleep=2, job_id=job_id, deduplicate=True)
		job2 = traquent.enqueue(self.BG_JOB, sleep=5, job_id=job_id, deduplicate=True)
		...
		job3 = traquent.enqueue(self.BG_JOB, fail=True, job_id=job_id, deduplicate=True)
		self.check_status(job3, "failed")
		job4 = traquent.enqueue(self.BG_JOB, sleep=1, job_id=job_id, deduplicate=True)
		self.check_status(job4, "finished")

	@timeout
	def test_enqueue_after_commit(self):
		job_id = traquent.generate_hash()

		traquent.enqueue(self.BG_JOB, enqueue_after_commit=True, job_id=job_id)
	...
	def test_clear_failed_jobs(self):
		...
		jobs = [traquent.enqueue(method=self.BG_JOB, queue="short", fail=True) for _ in range(limit * 2)]
