...
def extract_messages_from_code(code):
	...
	from traquent.model.utils import InvalidIncludePath, render_include

	try:
		code = traquent.as_unicode(render_include(code))

	# Exception will occur when it encounters John Resig's microtemplating code
	except (TemplateError, ImportError, InvalidIncludePath, OSError) as e:
		if isinstance(e, InvalidIncludePath) and hasattr(traquent.local, "message_log"):
