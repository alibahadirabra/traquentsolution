...
traquent.ui.form.on("Print Format", "onload", function (frm) {
	frm.add_fetch("doc_type", "module", "module");
});

traquent.ui.form.on("Print Format", {
	refresh: function (frm) {
		frm.set_intro("");
		frm.toggle_enable(["html", "doc_type", "module"], false);
		if (traquent.session.user === "Administrator" || frm.doc.standard === "No") {
			frm.toggle_enable(["html", "doc_type", "module"], true);
			frm.enable_save();
		}

		if (frm.doc.standard === "Yes" && traquent.session.user !== "Administrator") {
			frm.set_intro(__("Please duplicate this to make changes"));
		}
		frm.trigger("render_buttons");
		frm.toggle_display("standard", traquent.boot.developer_mode);
		...
		if (!frm.is_new()) {
			if (!frm.doc.custom_format) {
				frm.add_custom_button(__("Edit Format"), function () {
					if (!frm.doc.doc_type) {
						traquent.msgprint(__("Please select DocType first"));
						return;
					}
					if (frm.doc.print_format_builder_beta) {
						traquent.set_route("print-format-builder-beta", frm.doc.name);
					} else {
						traquent.set_route("print-format-builder", frm.doc.name);
					}
				});
			} else if (frm.doc.custom_format && !frm.doc.raw_printing) {
				frm.set_df_property("html", "reqd", 1);
			}
			if (traquent.model.can_write("Customize Form")) {
				traquent.model.with_doctype(frm.doc.doc_type, function () {
					let current_format = traquent.get_meta(frm.doc.doc_type).default_print_format;
					...
					frm.add_custom_button(__("Set as Default"), function () {
						traquent.call({
							method: "traquent.printing.doctype.print_format.print_format.make_default",
							args: {
								name: frm.doc.name,
							},
							callback: function () {
	...
	hide_absolute_value_field: function (frm) {
		...
		if (doctype) {
			traquent.model.with_doctype(doctype, () => {
				const meta = traquent.get_meta(doctype);
