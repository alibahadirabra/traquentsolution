...
from traquent.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
...
from traquent.utils import add_user_info, cint, format_duration
...
def get():
	...
	if is_virtual_doctype(args.doctype):
		...
		data = compress(traquent.call(controller.get_list, args=args, **args))
	else:
...
def get_count() -> int:
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		count = traquent.call(controller.get_count, args=args, **args)
	else:
		...
		if args.limit:
			...
			count = traquent.db.sql(f"""select count(*) from ( {partial_query} ) p""")[0][0]
		else:
...
def get_form_params():
	...
	data = traquent._dict(traquent.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = traquent.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			traquent.throw(_("Invalid aggregate function"))

		if traquent.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	traquent.throw(_("Field not permitted in query") + f": {fieldname}", traquent.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = traquent.get_meta(parenttype)
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		parenttype = parts[0]
		fieldname = parts[1]
		df = traquent.get_meta(data.doctype).get_field(parenttype)
...
def compress(data, args=None):
	...
	from traquent.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = traquent.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if traquent.db.exists("Report", name):
		report = traquent.get_doc("Report", name)
		if report.is_standard == "Yes":
			traquent.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			traquent.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != traquent.session.user and not report.has_permission("write"):
			traquent.throw(_("Insufficient Permissions for editing Report"), traquent.PermissionError)
	else:
	...
	traquent.msgprint(
		_("Report {0} saved").format(traquent.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
def delete_report(name):
	...
	report = traquent.get_doc("Report", name)
	if report.is_standard == "Yes":
		traquent.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		traquent.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != traquent.session.user and not report.has_permission("delete"):
		traquent.throw(_("Insufficient Permissions for deleting Report"), traquent.PermissionError)

	report.delete(ignore_permissions=True)
	traquent.msgprint(
		_("Report {0} deleted").format(traquent.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	traquent.permissions.can_export(doctype, raise_exception=True)
	...
	if traquent.local.lang == "en" or not translate_values:
	...
	if file_format_type == "CSV":
		...
		content = get_csv_bytes(
			[[handle_html(traquent.as_unicode(v)) if isinstance(v, str) else v for v in r] for r in data],
			csv_params,
		)
	elif file_format_type == "Excel":
		from traquent.utils.xlsxutils import make_xlsx
...
def get_field_info(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = traquent.get_meta(parenttype).get_field(fieldname)
			if df and df.fieldtype in ("Data", "Select", "Small Text", "Text"):
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = getattr(df, "translatable", False)
			elif df and df.fieldtype == "Link" and traquent.get_meta(df.options).translated_doctype:
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = True
...
def delete_items():
	...
	items = sorted(json.loads(traquent.form_dict.get("items")), reverse=True)
...
def delete_bulk(doctype, items):
	...
	for i, d in enumerate(items):
		try:
			traquent.flags.in_bulk_delete = True
			traquent.delete_doc(doctype, d)
			if len(items) >= 5:
...
def get_sidebar_stats(stats, doctype, filters=None):
	...
	if is_virtual_doctype(doctype):
		...
		data = traquent.call(controller.get_stats, args=args, **args)
	else:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = traquent.db.get_table_columns(doctype)
	except (traquent.db.InternalError, traquent.db.ProgrammingError):
	...
	for column in columns:
		...
		try:
			...
			if column == "_user_tags":
				...
				no_tag_count = traquent.get_list(
					doctype,
					fields=[column, "count(*)"],
