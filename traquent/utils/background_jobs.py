...
from traquent.utils import CallbackManager, cint, get_bench_id
from traquent.utils.commands import log
from traquent.utils.redis_queue import RedisQueue
...
def get_queues_timeout() -> dict[str, int]:
	...
	"""
	common_site_config = traquent.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	if deduplicate:
		if not job_id:
			traquent.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status() in (JobStatus.QUEUED, JobStatus.STARTED):
			traquent.logger().debug(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not traquent.flags.in_test:
		from traquent.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not traquent.flags.in_test)
	if call_directly:
		return traquent.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if traquent.local.flags.in_migrate:
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=traquent.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=traquent.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		traquent.db.after_commit.add(enqueue_call)
		return
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(traquent.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	traquent.local.job = traquent._dict(
		site=site,
		method=method_name,
		job_name=job_name,
		kwargs=kwargs,
	...
	for before_job_task in traquent.get_hooks("before_job"):
		traquent.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (traquent.db.InternalError, traquent.RetryBackgroundJobError) as e:
		traquent.db.rollback()

		if retry < 5 and (
			isinstance(e, traquent.RetryBackgroundJobError)
			or (traquent.db.is_deadlocked(e) or traquent.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			...
			traquent.log_error(title=method_name)
			raise

	except Exception as e:
		traquent.db.rollback()
		traquent.log_error(title=method_name)
		traquent.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		traquent.db.commit()
		...
		for after_job_task in traquent.get_hooks("after_job"):
			traquent.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		traquent.local.job.after_job.run()
...
def start_worker_pool(
	...
	import traquent.database.query  # sqlparse and indirect imports
	import traquent.query_builder  # pypika
	import traquent.utils  # common utils
...
def validate_queue(queue: str, default_queue_list: list | None = None) -> None:
	...
	if queue not in default_queue_list:
		traquent.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	...
	cred = traquent._dict()
	if traquent.conf.get("use_rq_auth"):
		if username:
			cred["username"] = username
			cred["password"] = password
		else:
			cred["username"] = traquent.get_site_config().rq_username or get_bench_id()
			cred["password"] = traquent.get_site_config().rq_password
	...
	try:
		...
		log(
			f"Please make sure that Redis Queue runs @ {traquent.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def create_job_id(job_id: str | None = None) -> str:
	...
	if not job_id:
		job_id = str(uuid4())
	return f"{traquent.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from traquent.utils import create_batch

	conf = traquent.conf if traquent.conf else traquent.get_conf(site=job.kwargs.get("site"))
...
def flush_telemetry():
	...
	to push events."""
	ph = getattr(traquent.local, "posthog", None)
