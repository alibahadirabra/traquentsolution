...
def execute():
	custom_doctypes = traquent.get_all("DocType", filters={"custom": 1})
	...
	for doctype in custom_doctypes:
		...
		custom_fields = traquent.get_all("Custom Field", filters={"dt": doctype.name}, fields=["*"])

		property_setter_map = {}

		for prop in property_setters:
			property_setter_map[prop.field_name] = prop
			traquent.db.delete("Property Setter", {"name": prop.name})

		meta = traquent.get_meta(doctype.name)
		...
		for cf in custom_fields:
			...
			if field:
				...
				df = traquent.new_doc("DocField", parent_doc=meta, parentfield="fields")
				df.update(cf)
				meta.fields.append(df)
			traquent.db.delete("Custom Field", {"name": cf.name})
