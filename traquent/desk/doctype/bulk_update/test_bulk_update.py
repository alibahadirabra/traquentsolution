...
from traquent.desk.doctype.bulk_update.bulk_update import submit_cancel_or_update_docs
...
class TestBulkUpdate(IntegrationTestCase):
	...
	def setUpClass(cls) -> None:
		...
		traquent.db.commit()
		for _ in range(50):
			traquent.new_doc(cls.doctype, some_fieldname=traquent.mock("name")).insert()
	...
	def test_bulk_submit_in_background(self):
		unsubmitted = traquent.get_all(self.doctype, {"docstatus": 0}, limit=5, pluck="name")
		...
		def check_docstatus(docs, status):
			...
			matching_docs = traquent.get_all(
				self.doctype, {"docstatus": status, "name": ("in", docs)}, pluck="name"
			)
			return set(matching_docs) == set(docs)

		unsubmitted = traquent.get_all(self.doctype, {"docstatus": 0}, limit=20, pluck="name")
		...
		submitted = traquent.get_all(self.doctype, {"docstatus": 1}, limit=20, pluck="name")
