...
def get_jenv():
	...
	if not getattr(traquent.local, "jenv", None):
		from jinja2 import DebugUndefined
		from jinja2.sandbox import SandboxedEnvironment

		from traquent.utils.safe_exec import UNSAFE_ATTRIBUTES, get_safe_globals
		...
		traquent.local.jenv = jenv

	return traquent.local.jenv
...
def validate_template(html):
	...
	try:
		...
		traquent.throw(f"Syntax error in template as line {e.lineno}: {e.message}")
...
def render_template(template, context=None, is_path=None, safe_render=True):
	...
	from traquent import _, get_traceback, throw
...
def get_jloader():
	...
	if not getattr(traquent.local, "jloader", None):
		from jinja2 import ChoiceLoader, PackageLoader, PrefixLoader

		apps = traquent.get_hooks("template_apps")
		if not apps:
			apps = list(
				reversed(
					traquent.local.flags.web_pages_apps or traquent.get_installed_apps(_ensure_on_bench=True)
				)
			)
...
def get_jinja_hooks():
	...
	if not getattr(traquent.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = traquent.get_module(obj_path)
			except ModuleNotFoundError:
				obj = traquent.get_attr(obj_path)
	...
	values = traquent.get_hooks("jinja")
