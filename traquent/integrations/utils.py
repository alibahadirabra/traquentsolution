...
import traquent
from traquent.utils import get_request_session
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = traquent.flags.integration_request = s.request(
		...
		if traquent.flags.integration_request_doc:
			traquent.flags.integration_request_doc.log_error()
		else:
			traquent.log_error()
		raise exc
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = traquent.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else traquent.as_json(obj, indent=1)
