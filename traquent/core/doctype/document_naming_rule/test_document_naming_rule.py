...
class TestDocumentNamingRule(IntegrationTestCase):
	...
	def test_naming_rule_by_condition(self):
		...
		# another rule
		naming_rule_1 = traquent.copy_doc(naming_rule)
		...
		naming_rule_2 = traquent.copy_doc(naming_rule)
		...
		todo = traquent.get_doc(
			doctype="ToDo", priority="High", description="Is this my name " + traquent.generate_hash()
		).insert()

		todo_1 = traquent.get_doc(
			doctype="ToDo", priority="Medium", description="Is this my name " + traquent.generate_hash()
		).insert()

		todo_2 = traquent.get_doc(
			doctype="ToDo", priority="Low", description="Is this my name " + traquent.generate_hash()
		).insert()
