...
def build_missing_files():
	...
	assets_json = traquent.read_file("assets/assets.json")
	if assets_json:
...
def get_assets_link(traquent_head) -> str:
	...
	if tag:
		...
		url = f"http://assets.frappeframework.com/{traquent_head}.tar.gz"
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{traquent.generate_hash()}"
...
def setup():
	...
	for app in traquent.get_all_apps(True):
		try:
			pymodules.append(traquent.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(traquent.local.sites_path, "assets")
...
def watch(apps=None):
	...
	live_reload = traquent.utils.cint(os.environ.get("LIVE_RELOAD", traquent.conf.live_reload))
...
def generate_assets_map():
	...
	for app_name in traquent.get_all_apps():
		app_doc_path = None

		pymodule = traquent.get_module(app_name)
