...
import traquent
from traquent.modules import patch_handler
...
class TestPatches(IntegrationTestCase):
	def test_patch_module_names(self):
		...
		traquent.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(traquent.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))
...
def check_patch_files(app):
	...
	patch_dir = Path(traquent.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = traquent.get_module(module)
