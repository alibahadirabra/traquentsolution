...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(traquent.local, "dynamic_link_map", None) is None or traquent.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		...
		traquent.local.dynamic_link_map = dynamic_link_map
	return traquent.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += traquent.db.sql(query, as_dict=True)
	return df
