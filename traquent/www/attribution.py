...
from traquent.permissions import is_system_user


def get_context(context):
	if not is_system_user():
		traquent.throw(_("You need to be a system user to access this page."), traquent.PermissionError)
...
def get_js_deps(app: str) -> list[dict]:
	package_json = Path(traquent.get_app_path(app, "..", "package.json"))
...
def get_pyproject_info(app: str) -> dict:
	pyproject_toml = Path(traquent.get_app_path(app, "..", "pyproject.toml"))
