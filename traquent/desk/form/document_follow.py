...
def follow_document(doctype, doc_name, user):
	...
	if not traquent.get_meta(doctype).track_changes:
		traquent.toast(_("Can't follow since changes are not tracked."))
		return False

	if user == "Administrator":
		traquent.toast(_("Administrator can't follow"))
		return False

	if not traquent.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		traquent.toast(_("Document follow is not enabled for this user."))
		return False

	if not is_document_followed(doctype, doc_name, user):
		doc = traquent.new_doc("Document Follow")
		doc.update({"ref_doctype": doctype, "ref_docname": doc_name, "user": user})
		doc.save()
		traquent.toast(_("Following document {0}").format(doc_name))
		return doc
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		traquent.delete_doc("Document Follow", doc[0].name)
		traquent.toast(_("Un-following document {0}").format(doc_name))
		return False
	return False
...
def get_user_list(frequency):
	...
	User = DocType("User")
	return (
		traquent.qb.from_(DocumentFollow)
		.join(User)
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = traquent.parse_json(v.data)
			time = traquent.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = traquent.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", traquent.utils.add_days(traquent.utils.nowdate(), -7)],
			["modified", "<", traquent.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", traquent.utils.add_days(traquent.utils.nowdate(), -1)],
			["modified", "<", traquent.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", traquent.utils.add_to_date(traquent.utils.now_datetime(), hours=-1)],
			["modified", "<", traquent.utils.now_datetime()],
		]
