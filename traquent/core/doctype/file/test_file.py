...
from traquent.core.doctype.file.exceptions import FileTypeNotAllowed
from traquent.core.doctype.file.utils import get_corrupted_image_msg, get_extension
from traquent.desk.form.utils import add_comment
from traquent.exceptions import ValidationError
from traquent.tests import IntegrationTestCase, UnitTestCase
from traquent.utils import get_files_path, set_request
...
def make_test_doc(ignore_permissions=False):
	...
	d.description = "Test"
	d.assigned_by = traquent.session.user
...
def make_test_image_file(private=False):
	...
	# remove those flags
	_test_file: "File" = traquent.get_doc("File", test_file.name)
...
class TestSimpleFile(IntegrationTestCase):
	...
	def test_save(self):
		_file = traquent.get_doc("File", {"file_url": self.saved_file_url})
...
class TestFSRollbacks(IntegrationTestCase):
	def test_rollback_from_file_system(self):
		file_name = content = traquent.generate_hash()
		file = traquent.new_doc("File", file_name=file_name, content=content).insert()
...
class TestExtensionValidations(IntegrationTestCase):
	...
	def test_allowed_extension(self):
		...
		file_name = content = traquent.generate_hash()
		bad_file = traquent.new_doc("File", file_name=f"{file_name}.png", content=content)
		self.assertRaises(FileTypeNotAllowed, bad_file.insert)

		bad_file = traquent.new_doc("File", file_name=f"{file_name}.csv", content=content).insert()
		traquent.db.rollback()
...
class TestBase64File(IntegrationTestCase):
	def setUp(self):
		...
		_file: traquent.Document = traquent.get_doc(
			{
				"doctype": "File",
	...
	def test_saved_content(self):
		_file: traquent.Document = traquent.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
...
class TestSameFileName(IntegrationTestCase):
	def test_saved_content(self):
		...
		_file = traquent.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = traquent.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = traquent.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = traquent.get_doc("File", {"file_url": _file2.file_url})
...
class TestSameContent(IntegrationTestCase):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(traquent.exceptions.AttachmentLimitReached, file2.insert)
	...
	def test_utf8_bom_content_decoding(self):
		...
		_file.save()
		saved_file = traquent.get_doc("File", _file.name)
...
class TestFile(IntegrationTestCase):
	...
	def tearDown(self):
		try:
			traquent.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except traquent.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = traquent.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = traquent.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = traquent.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = traquent.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = traquent.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			traquent.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = traquent.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(traquent.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = traquent.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = traquent.get_doc("File", file2.name)
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: "File" = traquent.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": traquent.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = traquent.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": traquent.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = traquent.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			traquent.message_log[0].get("message"),
			f"File '{traquent.utils.get_url('unknown.jpg')}' not found",
		)
...
class TestAttachment(IntegrationTestCase):
	...
	def tearDownClass(cls):
		traquent.db.rollback()
		traquent.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = traquent.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestAttachmentsAccess(IntegrationTestCase):
	def setUp(self) -> None:
		traquent.db.delete("File", {"is_folder": 0})
...
class TestFileUtils(IntegrationTestCase):
	def test_extract_images_from_doc(self):
		is_private = not traquent.get_meta("ToDo").make_attachments_public
		...
		self.assertTrue(traquent.db.exists("File", {"attached_to_name": todo.name, "is_private": is_private}))
		...
		filename = traquent.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{traquent.get_doc("File", filename).file_url}', todo.description)
	...
	def test_extract_images_from_comment(self):
		...
		"""
		is_private = not traquent.get_meta("ToDo").make_attachments_public
		test_doc = traquent.get_doc(doctype="ToDo", description="comment test").insert()
		comment = add_comment(
			"ToDo",
			test_doc.name,
		...
		self.assertTrue(
			traquent.db.exists("File", {"attached_to_name": test_doc.name, "is_private": is_private})
		)
	...
	def test_extract_images_from_communication(self):
		...
		"""
		is_private = not traquent.get_meta("Communication").make_attachments_public
		communication = traquent.get_doc(
		...
		self.assertTrue(
			traquent.db.exists("File", {"attached_to_name": communication.name, "is_private": is_private})
		)
...
class TestFileOptimization(IntegrationTestCase):
	...
	def test_optimize_folder(self):
		test_folder = traquent.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(IntegrationTestCase):
	...
	def tearDown(self) -> None:
		traquent.set_user("Administrator")
		traquent.db.rollback()
		traquent.delete_doc("DocType", "Test For Attachment")
	...
	def test_attach_unattached_guest_file(self):
		...
		d = traquent.new_doc("Test For Attachment")
	...
	def test_private_remains_private_even_if_same_hash(self):
		file_name = "test" + traquent.generate_hash()
		content = file_name.encode()

		doc_pub: "File" = traquent.new_doc("File")  # type: ignore
		doc_pub.file_url = f"/files/{file_name}.txt"
		doc_pub.content = content
		doc_pub.save()

		doc_pri: "File" = traquent.new_doc("File")  # type: ignore
