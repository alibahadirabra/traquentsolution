...
from traquent.core.doctype.recorder.db_optimizer import DBOptimizer, DBTable
from traquent.custom.doctype.property_setter.property_setter import make_property_setter
from traquent.model.document import Document
from traquent.recorder import RECORDER_REQUEST_HASH
from traquent.recorder import get as get_recorder_data
from traquent.utils import cint, cstr, evaluate_filters, get_table_name
...
class Recorder(Document):
	...
	if TYPE_CHECKING:
		from traquent.core.doctype.recorder_query.recorder_query import RecorderQuery
		from traquent.core.doctype.recorder_suggested_index.recorder_suggested_index import (
...
def serialize_request(request):
	request = traquent._dict(request)
	if request.get("calls"):
		for i in request.calls:
			i["stack"] = traquent.as_json(i["stack"])
			i["explain_result"] = traquent.as_json(i["explain_result"])
	request.update(
		name=request.get("uuid"),
		number_of_queries=request.get("queries"),
		time_in_queries=request.get("time_queries"),
		request_headers=traquent.as_json(request.get("headers"), indent=4),
		form_dict=traquent.as_json(request.get("form_dict"), indent=4),
...
def add_indexes(indexes):
	...
	for index in indexes:
		traquent.enqueue(_add_index, table=index["table"], column=index["column"])
	traquent.msgprint(_("Enqueued creation of indexes"), alert=True)


def _add_index(table, column):
	doctype = get_doctype_name(table)
	traquent.db.add_index(doctype, [column])
...
def _optimize(recorder_id):
	record: Recorder = traquent.get_doc("Recorder", recorder_id)
	...
	data = traquent.cache.hget(RECORDER_REQUEST_HASH, record.name)
	data["suggested_indexes"] = [{"table": idx[0][0], "column": idx[0][1]} for idx in suggested_indexes]
	traquent.cache.hset(RECORDER_REQUEST_HASH, record.name, data)
	traquent.publish_realtime("recorder-analysis-complete", user=traquent.session.user)
	traquent.msgprint(_("Query analysis complete. Check suggested indexes."), realtime=True, alert=True)
...
def _optimize_query(query):
	...
	for table in tables:
		...
		if not stats:
			return
		db_table = DBTable.from_traquent_ouput(stats)
...
def _fetch_table_stats(doctype: str, columns: list[str]) -> dict | None:
	...
	if not traquent.db.table_exists(doctype):
		return

	table = get_table_name(doctype, wrap_in_backticks=True)

	schema = []
	for field in traquent.db.sql(f"describe {table}", as_dict=True):
		schema.append(
			{
				"column": field["Field"],
				"type": field["Type"],
	...
	indexes = []
	for idx in traquent.db.sql(f"show index from {table}", as_dict=True):
		indexes.append(
			{
...
def _get_column_cardinality(table, column):
	return traquent.db.sql(f"select count(distinct {column}) from {table}")[0][0]
