...
from traquent.utils.safe_exec import ServerScriptNotEnabled, get_safe_globals, safe_exec
...
class TestSafeExec(IntegrationTestCase):
	...
	def test_safe_eval(self):
		...
		user.enabled = 1
		self.assertTrue(traquent.safe_eval("user_type == 'System User'", eval_locals=user.as_dict()))
		self.assertEqual(
			"System User Test", traquent.safe_eval("user_type + ' Test'", eval_locals=user.as_dict())
		)
		self.assertEqual(1, traquent.safe_eval("int(enabled)", eval_locals=user.as_dict()))

	def test_safe_eval_wal(self):
		self.assertRaises(SyntaxError, traquent.safe_eval, "(x := (40+2))")
	...
	def test_attrdict(self):
		# jinja
		traquent.render_template("{% set my_dict = _dict() %} {{- my_dict.works -}}")
	...
	def test_print(self):
		test_str = traquent.generate_hash()
		safe_exec(f"print('{test_str}')")
		self.assertEqual(traquent.local.debug_log[-1], test_str)
