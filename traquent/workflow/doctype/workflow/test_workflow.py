...
class TestWorkflow(IntegrationTestCase):
	...
	def tearDown(self):
		...
		self.patcher.stop()
		traquent.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = traquent.get_all("Workflow Action", fields=["*"])
...
def create_todo_workflow():
	from traquent.tests.ui_test_helpers import UI_TEST_USER

	if traquent.db.exists("Workflow", "Test ToDo"):
		traquent.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not traquent.db.exists("Role", TEST_ROLE):
		traquent.get_doc(doctype="Role", role_name=TEST_ROLE).insert(ignore_if_duplicate=True)
		if traquent.db.exists("User", UI_TEST_USER):
			traquent.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = traquent.new_doc("Workflow")
...
def create_new_todo():
	return traquent.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
