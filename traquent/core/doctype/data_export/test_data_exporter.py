...
import traquent
from traquent.core.doctype.data_export.exporter import DataExporter
...
class TestDataExporter(IntegrationTestCase):
	...
	def create_test_data(self, force=False):
		...
		"""
		if force:
			traquent.delete_doc(self.doctype_name, self.doc_name)

		if not traquent.db.exists(self.doctype_name, self.doc_name):
			self.doc = traquent.get_doc(
			...
			self.doc = traquent.get_doc(self.doctype_name, self.doc_name)

	def test_export_content(self):
		exp = DataExporter(doctype=self.doctype_name, file_type="CSV")
		exp.build_response()

		self.assertEqual(traquent.response["type"], "csv")
		self.assertEqual(traquent.response["doctype"], self.doctype_name)
		self.assertTrue(traquent.response["result"])
		self.assertRegex(traquent.response["result"], r"Child Title 1.*?,50")
		self.assertRegex(traquent.response["result"], r"Child Title 2.*?,51")
		...
		for type in ["csv", "Excel"]:
			with self.subTest(type=type):
				...
				if type == "csv":
					...
					self.assertEqual(traquent.response["type"], "binary")
					self.assertEqual(
						traquent.response["filename"], self.doctype_name + ".xlsx"
					)  # 'Test DocType for Export Tool.xlsx')
					self.assertTrue(traquent.response["filecontent"])
