...
import traquent
from traquent.integrations.oauth2 import encode_params
from traquent.tests import IntegrationTestCase
from traquent.tests.test_api import get_test_client, make_request, suppress_stdout
...
class traquentRequestTestCase(IntegrationTestCase):
	...
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			traquent.local.login_manager.login_as("test@example.com")
			self._sid = traquent.session.sid
...
class TestOAuth20(traquentRequestTestCase):
	...
	def setUp(self):
		...
		self.oauth_client = traquent.new_doc("OAuth Client")
	...
	def test_revoke_token(self):
		client = traquent.get_doc("OAuth Client", self.client_id)
	...
	def test_login_using_implicit_token(self):
		oauth_client = traquent.get_doc("OAuth Client", self.client_id)
...
def get_full_url(endpoint):
	...
	return urljoin(traquent.utils.get_url(), endpoint)
