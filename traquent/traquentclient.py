...
class traquentClient:
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": traquent.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return traquent._dict(self.post_process(res))
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": traquent.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return traquent._dict(self.post_process(res))
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not traquent.db.exists("User", doc.get("owner")):
				traquent.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = traquent.db.get_single_value(doctype, "modified")
