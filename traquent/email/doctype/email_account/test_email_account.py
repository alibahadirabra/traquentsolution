...
from traquent.email.doctype.email_account.email_account import notify_unreplied
from traquent.email.email_body import get_message_id
from traquent.email.receive import Email, InboundMail, SentEmailInInboxError
...
class TestEmailAccount(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		email_account = traquent.get_doc("Email Account", "_Test Email Account 1")
	...
	def setUp(self):
		...
		traquent.flags.sent_mail = None
		traquent.db.delete("Email Queue")
		traquent.db.delete("Unhandled Email")
	...
	def test_incoming(self):
		...
		comm = traquent.get_doc("Communication", {"sender": "test_sender@example.com"})
		self.assertTrue("test_receiver@example.com" in comm.recipients)
		# check if todo is created
		self.assertTrue(traquent.db.get_value(comm.reference_doctype, comm.reference_name, "name"))

	def test_unread_notification(self):
		todo = traquent.get_last_doc("ToDo")

		comm = traquent.new_doc(
			"Communication",
	...
	def test_incoming_with_attach(self):
		...
		existing_file = traquent.get_doc({"doctype": "File", "file_name": "traquent-conf-14.png"})
		traquent.delete_doc("File", existing_file.name)
		...
		self.assertTrue("traquent-conf-14.png" in [f.file_name for f in attachments])
	...
	def test_outgoing(self):
		...
		mail = email.message_from_string(traquent.get_last_doc("Email Queue").message)
	...
	def test_sendmail(self):
		...
		sent_mail = email.message_from_string(traquent.safe_decode(traquent.flags.sent_mail))
	...
	def test_print_format(self):
		...
		sent_mail = email.message_from_string(traquent.get_last_doc("Email Queue").message)
	...
	def test_threading(self):
		...
		sent_name = make(
			...
			doctype="ToDo",
			name=traquent.get_last_doc("ToDo").name,
			send_email=True,
		)["name"]
		...
		sent = traquent.get_doc("Communication", sent_name)
	...
	def test_threading_by_message_id(self):
		...
		event = traquent.get_doc(doctype="Event", subject="test-message").insert()
		...
		last_mail = traquent.get_doc("Email Queue", dict(reference_name=event.name))
	...
	def test_handle_bad_emails(self):
		...
		self.assertTrue(traquent.db.get_value("Unhandled Email", {"message_id": message_id}))
	...
	def test_handle_bad_encoding(self):
		...
		self.assertTrue(traquent.db.get_value("Unhandled Email", {"uid": uid}))
	...
	def test_append_to(self):
		...
		self.assertTrue(traquent.db.exists(communication.reference_doctype, communication.reference_name))
	...
	def test_append_to_with_imap_folders(self):
		...
		for mail in mails:
			...
			if mail.append_to == "ToDo":
				...
				self.assertTrue(communication.reference_name)
				self.assertTrue(
					traquent.db.exists(communication.reference_doctype, communication.reference_name)
				)
			else:
				test_folder_mails += 1
	...
	def mocked_get_inbound_mails(
		...
		from traquent.email.receive import EmailServer
...
class TestInboundMail(IntegrationTestCase):
	...
	def test_find_parent_communication_from_header(self):
		...
		communication = self.new_communication()
		mail_content = self.get_test_mail(fname="reply-4.raw").replace(
			"{{ message_id }}", f"<{communication.name}@{traquent.local.site}>"
		)
...
def cleanup(sender=None):
	...
	names = traquent.get_list("Communication", filters=filters, fields=["name"])
	for name in names:
		traquent.delete_doc_if_exists("Communication", name.name)
		traquent.delete_doc_if_exists("Communication Link", {"parent": name.name})
