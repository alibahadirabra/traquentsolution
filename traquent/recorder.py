...
from traquent.database.database import is_query_type
...
class RecorderConfig:
	...
	def __post_init__(self):
		if not (self.record_jobs or self.record_requests):
			traquent.throw("You must record one of jobs or requests")

	def store(self):
		traquent.cache.set_value(RECORDER_CONFIG_FLAG, self, expires_in_sec=RECORDER_AUTO_DISABLE)

	@classmethod
	def retrieve(cls):
		return traquent.cache.get_value(RECORDER_CONFIG_FLAG) or cls()

	@staticmethod
	def delete():
		traquent.cache.delete_value(RECORDER_CONFIG_FLAG)


def record_sql(*args, **kwargs):
	start_time = time.monotonic()
	result = traquent.db._sql(*args, **kwargs)
	end_time = time.monotonic()

	query = getattr(traquent.db, "last_query", None)
	if not query or isinstance(result, str):
		# run=0, doesn't actually run the query so last_query won't be present
		return result

	stack = []
	if traquent.local._recorder.config.capture_stack:
	...
	traquent.local._recorder.register(data)
	return result


def get_current_stack_frames():
	from traquent.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
...
def post_process():
	...
	traquent.db.begin(read_only=True)  # Explicitly start read only transaction

	config = RecorderConfig.retrieve()
	result = list(traquent.cache.hgetall(RECORDER_REQUEST_HASH).values())
	...
	for request in result:
		for call in request["calls"]:
			...
			if config.explain and is_query_type(formatted_query, ("select", "update", "delete")):
				...
				try:
					call["explain_result"] = traquent.db.sql(f"EXPLAIN {formatted_query}", as_dict=True)
				except Exception:
					pass
		mark_duplicates(request)
		traquent.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
...
def record(force=False):
	if traquent.cache.get_value(RECORDER_INTERCEPT_FLAG) or force:
		traquent.local._recorder = Recorder(force=force)


def dump():
	if hasattr(traquent.local, "_recorder"):
		traquent.local._recorder.dump()
...
class Recorder:
	def __init__(self, force=False):
		...
		if (
			...
			and traquent.request
			and self.config.request_filter in traquent.request.path
		):
			self.path = traquent.request.path
			self.cmd = traquent.local.form_dict.cmd or ""
			self.method = traquent.request.method
			self.headers = dict(traquent.local.request.headers)
			self.form_dict = traquent.local.form_dict
			self.event_type = "HTTP Request"
		elif self.config.record_jobs and traquent.job and self.config.jobs_filter in traquent.job.method:
			self.event_type = "Background Job"
			self.path = traquent.job.method
			self.cmd = None
			self.method = None
			self.headers = None
			self.form_dict = None
		...
		self.uuid = traquent.generate_hash(length=10)
	...
	def dump(self):
		...
		traquent.cache.hset(RECORDER_REQUEST_SPARSE_HASH, self.uuid, request_data)

		request_data["calls"] = self.calls
		request_data["headers"] = self.headers
		request_data["form_dict"] = self.form_dict
		request_data["profile"] = profiler_output
		traquent.cache.hset(RECORDER_REQUEST_HASH, self.uuid, request_data)
	...
	@staticmethod
	def _patch_sql():
		traquent.db._sql = traquent.db.sql
		traquent.db.sql = record_sql

	@staticmethod
	def _unpatch_sql():
		traquent.db.sql = traquent.db._sql
...
def do_not_record(function):
	...
	def wrapper(*args, **kwargs):
		if hasattr(traquent.local, "_recorder"):
			traquent.local._recorder.cleanup()
			del traquent.local._recorder
...
def administrator_only(function):
	...
	def wrapper(*args, **kwargs):
		if traquent.session.user != "Administrator":
			traquent.throw(_("Only Administrator is allowed to use Recorder"))
		return function(*args, **kwargs)
...
def stop(*args, **kwargs):
	traquent.cache.delete_value(RECORDER_INTERCEPT_FLAG)
...
def get(uuid=None, *args, **kwargs):
	if uuid:
		result = traquent.cache.hget(RECORDER_REQUEST_HASH, uuid)
	else:
		result = list(traquent.cache.hgetall(RECORDER_REQUEST_SPARSE_HASH).values())
	return result
...
def delete(*args, **kwargs):
	traquent.cache.delete_value(RECORDER_REQUEST_SPARSE_HASH)
	traquent.cache.delete_value(RECORDER_REQUEST_HASH)
...
def record_queries(func: Callable):
	...
	def wrapped(*args, **kwargs):
		record(force=True)
		traquent.local._recorder.path = f"Function call: {func.__module__}.{func.__qualname__}"
...
def import_data(file: str) -> None:
	file_doc = traquent.get_doc("File", {"file_url": file})
	file_content = json.loads(file_doc.get_content())
	for request in file_content:
		traquent.cache.hset(RECORDER_REQUEST_SPARSE_HASH, request["uuid"], request)
		traquent.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
