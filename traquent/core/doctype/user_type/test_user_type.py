...
class TestUserType(IntegrationTestCase):
	...
	def test_add_select_perm_doctypes(self):
		...
		doc = traquent.get_meta("Contact")
		link_fields = doc.get_link_fields()
		select_doctypes = traquent.get_all(
	...
	def test_print_share_email_default(self):
		...
		perm = traquent.get_all("Custom DocPerm", filters={"role": "_Test User Type"}, fields=["*"])[0]
...
def create_user_type(user_type):
	if traquent.db.exists("User Type", user_type):
		traquent.delete_doc("User Type", user_type)

	user_type_limit = {traquent.scrub(user_type): 1}
...
def create_role():
	if not traquent.db.exists("Role", "_Test User Type"):
		traquent.get_doc(
