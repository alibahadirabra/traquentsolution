...
import traquent
from traquent import _, conf
from traquent.query_builder.utils import DocType
from traquent.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = traquent.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = traquent.form_dict.doctype
	dn = traquent.form_dict.docname
	file_url = traquent.form_dict.file_url
	filename = traquent.form_dict.filename
	traquent.form_dict.is_private = cint(traquent.form_dict.is_private)

	if not filename and not file_url:
		traquent.msgprint(_("Please select a file or url"), raise_exception=True)
	...
	if dt and dn:
		...
		comment = traquent.get_doc(dt, dn).add_comment(
			"Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	file_size = traquent.form_dict.file_size
	...
	try:
		...
		return traquent.get_doc("File", f.duplicate_entry)
	return f
...
def get_uploaded_content():
	...
	if "filedata" in traquent.form_dict:
		...
		traquent.uploaded_content = safe_b64decode(traquent.form_dict.filedata)
		traquent.uploaded_filename = traquent.form_dict.filename
		return traquent.uploaded_filename, traquent.uploaded_content
	else:
		traquent.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in traquent.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = traquent.get_doc("File", name)
		return {k: b.get(k) for k in traquent.get_hooks()["write_file_keys"]}
	return False
...
def write_file(content, fname, is_private=0):
	...
	traquent.create_folder(file_path)
	# write the file
	if isinstance(content, str):
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in traquent.get_all("File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"):
			if from_delete:
				# If deleting a doc, directly delete files
				traquent.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = traquent.db.get_value("File", fid, ["attached_to_doctype", "attached_to_name", "file_name"])
		if attached:
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = traquent.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = traquent.flags.in_web_form or doc.has_permission("write")
		if not file_name:
			file_name = traquent.db.get_value("File", fid, "file_name")
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = traquent.utils.get_site_path("public", "files", parts[-1])

		else:
			path = traquent.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = traquent.get_all("File", {"file_name": fname}, pluck="name")
...
def is_safe_path(path: str) -> bool:
	...
	basedir = traquent.get_site_path()
