...
def set_default(doc, key):
	...
	if not doc.is_default:
		traquent.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		traquent.get_doc("DocType", d.parent)
		for d in traquent.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(traquent.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(traquent.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def get_fetch_values(doctype, fieldname, value):
	...
	db_values = traquent.db.get_value(
...
def is_virtual_doctype(doctype: str):
	if traquent.flags.in_install or traquent.flags.in_migrate:
		if traquent.db.has_column("DocType", "is_virtual"):
			return traquent.db.get_value("DocType", doctype, "is_virtual")
	else:
		return getattr(traquent.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if traquent.flags.in_install or traquent.flags.in_migrate:
		...
		return getattr(traquent.get_meta(doctype), "issingle", False)
