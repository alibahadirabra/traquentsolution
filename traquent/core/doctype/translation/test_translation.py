...
class TestTranslation(IntegrationTestCase):
	def setUp(self):
		traquent.db.delete("Translation")

	def tearDown(self):
		traquent.local.lang = "en"
		from traquent.translate import clear_cache
	...
	def test_doctype(self):
		...
		for key, val in translation_data.items():
			traquent.local.lang = key

			translation = create_translation(key, val)
			self.assertEqual(_(val[0]), val[1])

			traquent.delete_doc("Translation", translation.name)
...
def create_translation(key, val):
	translation = traquent.new_doc("Translation")
