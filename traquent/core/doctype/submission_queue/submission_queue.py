...
from traquent.utils import now, time_diff_in_seconds
...
class SubmissionQueue(Document):
	...
	@property
	def queued_doc(self):
		return getattr(self, "to_be_queued_doc", traquent.get_doc(self.ref_doctype, self.ref_docname))
	...
	def clear_old_logs(days=30):
		...
		table = traquent.qb.DocType("Submission Queue")
	...
	def background_submission(self, to_be_queued_doc: Document, action_for_queuing: str):
		...
		try:
			...
			values = {"status": "Failed", "exception": traquent.get_traceback(with_context=True)}
			traquent.db.rollback()

		values["ended_at"] = now()
		traquent.db.set_value(self.doctype, self.name, values, update_modified=False)
	...
	def notify(self, submission_status: str, action: str):
		...
		message_replacements = (
			traquent.bold(action),
			traquent.bold(str(self.ref_doctype)),
			traquent.bold(str(self.ref_docname)),
		)
	...
	def unlock_doc(self):
		...
		self.queued_doc.unlock()
		traquent.msgprint(_("Document Unlocked"))


def queue_submission(doc: Document, action: str, alert: bool = True):
	queue = traquent.new_doc("Submission Queue")
...
def get_latest_submissions(doctype, docname):
	...
	latest_submission = traquent.db.get_value(
		"Submission Queue",
