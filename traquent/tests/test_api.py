...
from traquent.utils import cint, get_test_client, get_url

try:
	_site = traquent.local.site
except Exception:
	_site = None
...
class TestResourceAPI(traquentAPITestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = traquent.get_doc({"doctype": "ToDo", "description": traquent.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			traquent.delete_doc_if_exists(cls.DOCTYPE, name)
		traquent.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = traquent._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		data = {"description": traquent.mock("paragraph"), "sid": self.sid}
...
class TestReadOnlyMode(traquentAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": traquent.mock("paragraph"), "sid": self.sid}
			)
...
class TestResponse(traquentAPITestCase):
	...
	def test_download_private_file_with_unique_url(self):
		test_content = traquent.generate_hash()
