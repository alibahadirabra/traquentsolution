...
class RedisWrapper(redis.Redis):
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{traquent.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		if key in traquent.local.cache:
			val = traquent.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(traquent.local.cache) if regex.match(cstr(k))]
	...
	def delete_value(self, keys, user=None, make_keys=True, shared=False):
		...
		for key in keys:
			traquent.local.cache.pop(key, None)
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		traquent.local.cache.setdefault(_name, {})[key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		_name = self.make_key(name, shared=shared)
		if _name not in traquent.local.cache:
			traquent.local.cache[_name] = {}

		if not key:
			return None

		if key in traquent.local.cache[_name]:
			return traquent.local.cache[_name][key]
		...
		if value is not None:
			...
			traquent.local.cache[_name][key] = value
		elif generator:
			value = generator()
	...
	def hdel(
		...
		name_in_local_cache = _name in traquent.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in traquent.local.cache[_name]:
				del traquent.local.cache[_name][keys]
			if pipeline:
		...
		for key in keys:
			if name_in_local_cache:
				if key in traquent.local.cache[_name]:
					del traquent.local.cache[_name][key]
...
def setup_cache():
	if traquent.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in traquent.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=traquent.conf.get("redis_cache_sentinel_username"),
			sentinel_password=traquent.conf.get("redis_cache_sentinel_password"),
			master_username=traquent.conf.get("redis_cache_master_username"),
			master_password=traquent.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			traquent.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(traquent.conf.get("redis_cache"))
