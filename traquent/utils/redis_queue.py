...
import traquent
from traquent.utils import get_bench_id, random_string
...
class RedisQueue:
	...
	def add_user(self, username, password=None):
		...
		return traquent._dict(user_settings) if is_created else {}

	@classmethod
	def get_connection(cls, username=None, password=None):
		if traquent.conf.redis_queue_sentinel_enabled:
			from traquent.utils.redis_wrapper import get_sentinel_connection

			sentinels = [tuple(node.split(":")) for node in traquent.conf.get("redis_queue_sentinels", [])]
			sentinel = get_sentinel_connection(
				sentinels=sentinels,
				sentinel_username=traquent.conf.get("redis_queue_sentinel_username"),
				sentinel_password=traquent.conf.get("redis_queue_sentinel_password"),
				master_username=traquent.conf.get("redis_queue_master_username", username),
				master_password=traquent.conf.get("redis_queue_master_password", password),
			)
			conn = sentinel.master_for(traquent.conf.get("redis_queue_master_service"))
			conn.ping()
			return conn
		conn = redis.from_url(traquent.conf.redis_queue, username=username, password=password)
		conn.ping()
		return conn
