...
from traquent.locale import get_date_format, get_first_day_of_the_week, get_number_format, get_time_format
from traquent.utils.deprecations import deprecated
from traquent.utils.number_format import NUMBER_FORMAT_MAP, NumberFormat
...
def getdate(
	...
	try:
		...
		traquent.throw(
			traquent._("{} is not a valid date string.").format(traquent.bold(string_date)),
			title=traquent._("Invalid Date"),
		)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			traquent.throw(traquent._("Please select a valid date filter"), title=traquent._("Invalid Date"))
...
def get_system_timezone() -> str:
	...
	return traquent.get_system_settings("time_zone") or "Asia/Kolkata"  # Default to India ?!
...
def now() -> str:
	...
	if traquent.flags.current_date:
		return (
			getdate(traquent.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format() -> str:
	...
	if getattr(traquent.local, "user_date_format", None) is None:
		traquent.local.user_date_format = get_date_format()

	return traquent.local.user_date_format
...
def get_user_time_format() -> str:
	...
	if getattr(traquent.local, "user_time_format", None) is None:
		traquent.local.user_time_format = get_time_format()

	return traquent.local.user_time_format
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(traquent.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(traquent.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(traquent.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
...
def validate_duration_format(duration):
	if not DURATION_PATTERN.match(duration):
		traquent.throw(
			traquent._("Value {0} must be in the valid duration format: d h m s").format(traquent.bold(duration))
		)
...
def global_date_format(date: DateTimeLikeObject, format="long") -> str:
	...
	date = getdate(date)
	return babel.dates.format_date(date, locale=(traquent.local.lang or "en").replace("-", "_"), format=format)
...
def flt(s: NumericType | str, precision: int | None = None, rounding_method: str | None = None) -> float:
	...
	try:
		...
		if isinstance(e, traquent.InvalidRoundingMethod):
			raise
		num = 0.0
...
def cstr(s, encoding="utf-8") -> str:
	...
	return traquent.as_unicode(s, encoding)
...
def rounded(num, precision=0, rounding_method=None):
	...
	rounding_method = (
		rounding_method or traquent.get_system_settings("rounding_method") or "Banker's Rounding (legacy)"
	)
	...
	if rounding_method == "Banker's Rounding (legacy)":
		...
		traquent.throw(
			traquent._("Unknown Rounding Method: {}").format(rounding_method),
			exc=traquent.InvalidRoundingMethod,
		)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		traquent.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(
	...
	if precision is None:
		precision = cint(traquent.db.get_default("currency_precision")) or None
	...
	if number_format.decimal_separator:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = traquent.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and traquent.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = traquent.db.get_value("Currency", currency, "symbol", cache=True) or currency
		symbol_on_right = traquent.db.get_value("Currency", currency, "symbol_on_right", cache=True)

		if symbol_on_right:
			amount = f"{amount} {traquent._(symbol)}"
		else:
			amount = f"{traquent._(symbol)} {amount}"
...
def money_in_words(
	...
	from traquent.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = traquent.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = get_number_format()

	fraction_units = traquent.db.get_value("Currency", main_currency, "fraction_units", cache=True)
...
def in_words(integer: int, in_million=True) -> str:
	...
	locale = "en_IN" if not in_million else traquent.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src: str) -> dict[str, str] | None:
	...
	from traquent import cache, safe_decode
	from traquent.core.doctype.file.utils import get_local_image

	if not src:
		traquent.throw(f"Invalid source for image: {src}")
	...
	def _get_base64():
		file_path = traquent.get_site_path("public", src.lstrip("/"))
...
def pdf_to_base64(filename: str) -> bytes | None:
	...
	from traquent.utils.file_manager import get_file_path
...
def pretty_date(iso_datetime: datetime.datetime | str) -> str:
	...
	locale = traquent.local.lang.replace("-", "_") if traquent.local.lang else None
...
def comma_or(some_list: list | tuple, add_quotes=True) -> str:
	...
	e.g. ['a', 'b', 'c'] -> "'a', 'b' or 'c'"
	"""
	return comma_sep(some_list, traquent._("{0} or {1}"), add_quotes)
...
def comma_and(some_list: list | tuple, add_quotes=True) -> str:
	...
	e.g. ['a', 'b', 'c'] -> "'a', 'b' and 'c'"
	"""
	return comma_sep(some_list, traquent._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list: list | tuple, pattern: str, add_quotes=True) -> str:
	...
	if isinstance(some_list, list | tuple):
		...
		if not some_list:
			...
			return pattern.format(", ".join(traquent._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(uri: str | None = None, full_address: bool = False) -> str:
	...
	host_name = traquent.local.conf.host_name or traquent.local.conf.hostname
	...
	if not host_name:
		...
		elif traquent.local.site:
			protocol = "http://"

			if traquent.local.conf.ssl_certificate:
				protocol = "https://"

			elif traquent.local.conf.wildcard:
				domain = traquent.local.conf.wildcard.get("domain")
				if (
					domain
					and traquent.local.site.endswith(domain)
					and traquent.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + traquent.local.site

		else:
			host_name = traquent.db.get_single_value("Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = traquent.get_request_header("REQUEST_URI", "")

	port = traquent.conf.http_port
	if not port and traquent.conf.developer_mode:
		port = traquent.conf.webserver_port

	if (
		# XXX: This config is used as proxy for "is production mode enabled?"
		not traquent.conf.restart_supervisor_on_update
		and not traquent.conf.restart_systemd_on_update
		and host_name
...
def get_host_name_from_request() -> str:
	...
	if hasattr(traquent.local, "request") and traquent.local.request and traquent.local.request.host:
		protocol = "https://" if "https" == traquent.get_request_header("X-Forwarded-Proto", "") else "http://"
		return protocol + traquent.local.request.host
...
def filter_operator_is(value: str, pattern: str) -> bool:
	...
	if pattern == "set":
		...
		traquent.throw(traquent._(f"Invalid argument for operator 'IS': {pattern}"))
...
def get_filter(doctype: str, f: dict | list | tuple, filters_config=None) -> "traquent._dict":
	"""Return a `_dict` like:
	...
	"""
	from traquent.database.utils import NestedSetHierarchy
	from traquent.model import child_table_fields, default_fields, optional_fields
	...
	if not isinstance(f, list | tuple):
		traquent.throw(traquent._("Filter must be a tuple or list (in a list)"))
	...
	if len(f) == 3:
		...
		traquent.throw(
			traquent._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f))
		)

	f = traquent._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
	...
	if f.operator.lower() not in valid_operators:
		traquent.throw(traquent._("Operator must be one of {0}").format(", ".join(valid_operators)))
	...
	if f.doctype and (f.fieldname not in default_fields + optional_fields + child_table_fields):
		...
		meta = traquent.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):
			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if traquent.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = traquent.get_meta(f.doctype).get_field(f.fieldname) if f.doctype else None
	except traquent.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name: str) -> None:
	...
	if traquent.db and traquent.db.db_type == "mariadb":
	...
	def _raise_exception():
		traquent.throw(_("Invalid field name {0}").format(column_name), traquent.DataError)
...
def generate_hash(*args, **kwargs) -> str:
	...
	"""
	return traquent.generate_hash(*args, **kwargs)
...
def get_user_info_for_avatar(user_id: str) -> _UserInfo:
	...
	"""
	try:
		user = traquent.get_cached_doc("User", user_id)
...
def validate_python_code(string: str, fieldname: str | None = None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = traquent._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		traquent.throw(msg, title=traquent._("Syntax Error"))
	except Exception as e:
		traquent.msgprint(
			traquent._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def is_site_link(link: str) -> bool:
	...
	if link.startswith("/"):
		return True
	return urlparse(link).netloc == urlparse(traquent.utils.get_url()).netloc
...
def add_trackers_to_url(
	...
	def get_utm_values():
		_source = traquent.db.get_value("UTM Source", source, "slug") or source
		_campaign = traquent.db.get_value("UTM Campaign", campaign, "slug") or campaign
		_medium = traquent.db.get_value("UTM Medium", medium, "slug") or medium
	...
	source, medium, campaign = traquent.cache.get_value(
...
def map_trackers(url_trackers: dict, create: bool = False):
	...
	if url_source := url_trackers.get("utm_source", url_trackers.get("source")):
		...
		if create and source == url_source and not traquent.db.exists("UTM Source", source):
			source = traquent.new_doc("UTM Source")
	...
	if url_medium := url_trackers.get("utm_medium", url_trackers.get("medium")):
		...
		if create and medium == url_medium and not traquent.db.exists("UTM Medium", medium):
			medium = traquent.new_doc("UTM Medium")
	...
	if url_campaign := url_trackers.get("utm_campaign", url_trackers.get("campaign")):
		...
		if create and campaign == url_campaign and not traquent.db.exists("UTM Campaign", campaign):
			campaign = traquent.new_doc("UTM Campaign")
