...
from traquent.translate import (
...
from traquent.utils import get_bench_path, set_request
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	traquent.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(IntegrationTestCase):
	...
	def tearDown(self):
		traquent.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(traquent.cache.hget(USER_TRANSLATION_KEY, traquent.local.lang))
		self.assertIsNotNone(traquent.cache.hget(MERGED_TRANSLATION_KEY, traquent.local.lang))

		clear_cache()

		self.assertIsNone(traquent.cache.hget(USER_TRANSLATION_KEY, traquent.local.lang))
		self.assertIsNone(traquent.cache.hget(MERGED_TRANSLATION_KEY, traquent.local.lang))
	...
	def test_read_language_variant(self):
		...
		try:
			traquent.local.lang = "pt-BR"
			self.assertEqual(_("Mobile No"), "Telefone Celular")
			traquent.local.lang = "pt"
...
def verify_translation_files(app):
	...
	translations_dir = Path(traquent.get_app_path(app)) / "translations"
