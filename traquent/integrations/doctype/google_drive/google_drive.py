...
from traquent.utils import get_backups_path, get_bench_path
...
class GoogleDrive(Document):
	...
	def get_access_token(self):
		if not self.refresh_token:
			button_label = traquent.bold(_("Allow Google Drive Access"))
...
def authorize_access(reauthorize=False, code=None):
	...
	oauth_code = traquent.db.get_single_value("Google Drive", "authorization_code") if not code else code
	oauth_obj = GoogleOAuth("drive")

	if not oauth_code or reauthorize:
		if reauthorize:
			traquent.db.set_single_value("Google Drive", "backup_folder_id", "")
...
def get_google_drive_object():
	...
	account = traquent.get_doc("Google Drive")
	oauth_obj = GoogleOAuth("drive")
...
def check_for_folder_in_google_drive():
	...
	def _create_folder_in_google_drive(google_drive, account):
		...
		try:
			...
			traquent.db.set_single_value("Google Drive", "backup_folder_id", folder.get("id"))
			traquent.db.commit()
	...
	try:
		...
		traquent.throw(_("Google Drive - Could not find folder in Google Drive - Error Code {0}").format(e))

	for f in google_drive_folders.get("files"):
		if f.get("name") == account.backup_folder_name:
			traquent.db.set_single_value("Google Drive", "backup_folder_id", f.get("id"))
			traquent.db.commit()
...
def upload_system_backup_to_google_drive():
	...
	for fileurl in file_urls:
		...
		try:
			...
			traquent.throw(_("Google Drive - Could not locate - {0}").format(e))
	...
	traquent.db.set_single_value("Google Drive", "last_backup_on", traquent.utils.now_datetime())
	send_email(True, "Google Drive", "Google Drive", "email")
	return _("Google Drive Backup Successful.")


def daily_backup():
	drive_settings = traquent.db.get_singles_dict("Google Drive", cast=True)
