...
from traquent.model.naming import set_new_name
from traquent.model.utils.link_count import notify_link_count
...
from traquent.utils.defaults import get_not_null_defaults
...
def get_controller(doctype):
	...
	if traquent.local.dev_server or traquent.flags.in_migrate:
		return import_controller(doctype)

	site_controllers = traquent.controllers.setdefault(traquent.local.site, {})
...
def import_controller(doctype):
	from traquent.model.document import Document
	from traquent.utils.nestedset import NestedSet

	module_name = "Core"
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = traquent.db.get_value("DocType", doctype, fieldname="*")
		if doctype_info:
	...
	module_path = None
	class_overrides = traquent.get_hooks("override_doctype_class")
	...
	if class_overrides and class_overrides.get(doctype):
		...
		custom_module = traquent.get_module(module_path)
		custom_class_ = getattr(custom_module, custom_classname, None)
		if not issubclass(custom_class_, class_):
			original_class_path = traquent.bold(f"{class_.__module__}.{class_.__name__}")
			traquent.throw(
				f"{doctype}: {traquent.bold(import_path)} must be a subclass of {original_class_path}",
...
class BaseDocument:
	...
	@cached_property
	def meta(self):
		return traquent.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return traquent.db.get_value(self.doctype, self.name, key)
		...
		for fieldname in self.meta.get_valid_fields():
			...
			if df:
				...
				if isinstance(value, list) and df.fieldtype not in table_fields:
					traquent.throw(_("Value for {0} cannot be a list").format(_(df.label, context=df.parent)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in traquent.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from traquent.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			traquent.local.valid_columns[self.doctype] = valid

		return traquent.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return traquent.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and traquent.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.owner = self.modified_by = traquent.session.user
		...
		try:
			...
			if traquent.db.is_primary_key_violation(e):
				...
				if not ignore_if_duplicate:
					traquent.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), traquent.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise traquent.DuplicateEntryError(self.doctype, self.name, e)

			elif traquent.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if traquent.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if traquent.db.db_type != "postgres":
			...
			traquent.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise traquent.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		if getattr(self.meta, "issingle", False):
			traquent.db.set_single_value(self.doctype, "modified", self.modified, update_modified=False)
		else:
			traquent.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						traquent.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						traquent.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not meta.get("is_virtual"):
					if not fields_to_fetch:
						...
						values = _dict(name=traquent.db.get_value(doctype, docname, "name", cache=True))
					else:
						...
						values = (
							traquent.db.get_value(doctype, docname, values_to_fetch, as_dict=True)
							or empty_values
						)
				...
				if meta.get("is_virtual"):
					values = traquent.get_doc(doctype, docname).as_dict()
				...
				if values:
					...
					if not values.name:
						...
						and traquent.get_meta(doctype).is_submittable
						and cint(traquent.db.get_value(doctype, docname, "docstatus")) == DocStatus.cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from traquent.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or traquent.get_meta(doctype).get_field(
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (traquent.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		...
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in traquent.STANDARD_USERS) and (data in traquent.STANDARD_USERS):
					continue
	...
	def _validate_constants(self):
		if traquent.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = traquent.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				traquent.throw(
					...
					traquent.CannotChangeConstantError,
				)
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				traquent.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				traquent.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		traquent.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, traquent.bold(_(df.label, context=df.parent)), max_length, value
			),
			traquent.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = traquent.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					traquent.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							traquent.bold(db_value),
							traquent.bold(self_value),
						),
						traquent.UpdateAfterSubmitError,
	...
	def _sanitize_content(self):
		...
		for fieldname, value in self.get_valid_dict(ignore_virtual=True).items():
			...
			value = traquent.as_unicode(value)
	...
	def _save_passwords(self):
		...
		from traquent.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def precision(self, fieldname, parentfield=None) -> int | None:
		...
		from traquent.model.meta import get_field_precision
		...
		if fieldname not in self._precision[cache_key]:
			...
			df = traquent.get_meta(doctype).get_field(fieldname)
	...
	def get_formatted(
		...
		from traquent.utils.formatters import format_value
		...
		if (
			...
			currency = traquent.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = traquent.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_text_editor(self):
		from traquent.core.doctype.file.utils import extract_images_from_doc
