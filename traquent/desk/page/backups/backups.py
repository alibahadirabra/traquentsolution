...
from traquent.utils import cint, get_site_path, get_url
from traquent.utils.data import convert_utc_to_system_timezone
...
def get_scheduled_backup_limit():
	backup_limit = traquent.db.get_singles_value("System Settings", "backup_limit")
...
def schedule_files_backup(user_email):
	...
	if method not in queued_jobs[traquent.local.site]:
		...
		traquent.msgprint(_("Queued for backup. You will receive an email with the download link"))
	else:
		traquent.msgprint(_("Backup job is already queued. You will receive an email with the download link"))


def backup_files_and_notify_user(user_email=None):
	from traquent.utils.backups import backup
