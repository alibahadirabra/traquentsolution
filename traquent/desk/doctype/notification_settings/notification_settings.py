...
class NotificationSettings(Document):
	...
	if TYPE_CHECKING:
		from traquent.desk.doctype.notification_subscribed_document.notification_subscribed_document import (
	...
	def on_update(self):
		from traquent.desk.notifications import clear_notification_config

		clear_notification_config(traquent.session.user)


def is_notifications_enabled(user):
	enabled = traquent.db.get_value("Notification Settings", user, "enabled")
	if enabled is None:
		return True
	return enabled
...
def is_email_notifications_enabled(user):
	enabled = traquent.db.get_value("Notification Settings", user, "enable_email_notifications")
	if enabled is None:
		return True
	return enabled
...
def is_email_notifications_enabled_for_type(user, notification_type):
	...
	fieldname = "enable_email_" + traquent.scrub(notification_type)
	enabled = traquent.db.get_value("Notification Settings", user, fieldname, ignore=True)
	if enabled is None:
		return True
...
def create_notification_settings(user):
	if not traquent.db.exists("Notification Settings", user):
		_doc = traquent.new_doc("Notification Settings")
		_doc.name = user
...
def toggle_notifications(user: str, enable: bool = False, ignore_permissions=False):
	try:
		settings = traquent.get_doc("Notification Settings", user)
...
def get_subscribed_documents():
	...
	try:
		if traquent.db.exists("Notification Settings", traquent.session.user):
			doc = traquent.get_doc("Notification Settings", traquent.session.user)
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabNotification Settings`.name = {traquent.db.escape(user)})"""
...
def has_permission(doc, ptype="read", user=None):
	...
	if "System Manager" in traquent.get_roles(user):
...
def set_seen_value(value, user):
	...
	traquent.db.set_value("Notification Settings", user, "seen", value, update_modified=False)
