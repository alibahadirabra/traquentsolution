...
def get_context(context):
	doc = traquent.get_doc("Contact Us Settings", "Contact Us Settings")
...
def send_message(sender, message, subject="Website Query"):
	...
	with suppress(traquent.OutgoingEmailError):
		if forward_to_email := traquent.db.get_single_value("Contact Us Settings", "forward_to_email"):
			traquent.sendmail(recipients=forward_to_email, reply_to=sender, content=message, subject=subject)
...
{0}"""
	...
	system_language = traquent.db.get_single_value("System Settings", "language")
	# add to to-do ?
	traquent.get_doc(
