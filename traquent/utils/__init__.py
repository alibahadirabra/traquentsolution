...
from traquent.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc

# utility functions like cint, int, flt, etc.
from traquent.utils.data import *
from traquent.utils.html_utils import sanitize_html
...
def get_fullname(user=None):
	...
	if not hasattr(traquent.local, "fullnames"):
		traquent.local.fullnames = {}

	if not traquent.local.fullnames.get(user):
		p = traquent.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			traquent.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			traquent.local.fullnames[user] = user

	return traquent.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return traquent.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			traquent.throw(
				error_message.format(traquent.bold(phone_number), traquent.bold(fieldname)),
				title=error_title,
				exc=traquent.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		traquent.throw(
			traquent._("{0} is not a valid Phone Number").format(phone_number), traquent.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	if not match and throw:
		traquent.throw(traquent._("{0} is not a valid Name").format(name), traquent.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = traquent.utils.escape_html(e)
				traquent.throw(
					traquent._("{0} is not a valid Email Address").format(invalid_email),
					traquent.InvalidEmailAddressError,
				)
			return None
		else:
			return email_id
...
def validate_url(
	...
	if not is_valid and throw:
		traquent.throw(traquent._("'{0}' is not a valid URL").format(traquent.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if traquent.flags.in_import or traquent.flags.in_install or traquent.flags.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(traquent.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
...
def get_gravatar(email: str) -> str:
	...
	from traquent.utils.identicon import Identicon
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", traquent._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	traquent.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return traquent.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return traquent.db.set_default(key, val)
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise traquent.CommandFailedError(
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = traquent.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return traquent.local.site_path
...
def get_bench_id():
	return traquent.get_conf().get("bench_id", get_bench_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or traquent.local.site}@{get_bench_id()}"
...
def get_hook_method(hook_name, fallback=None):
	method = traquent.get_hooks().get(hook_name)
	if method:
		method = traquent.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in traquent.get_hooks(hook):
		out = out or traquent.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(traquent.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in traquent.get_installed_apps():
				include_path = traquent.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(traquent.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else traquent.utils.md_to_html(text)
...
def get_site_info():
	from traquent.email.queue import get_emails_sent_this_month
	from traquent.utils.user import get_system_managers

	# only get system users
	users = traquent.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", traquent.STANDARD_USERS)},
	...
	system_settings = traquent.db.get_singles_dict("System Settings")
	space_usage = traquent._dict((traquent.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": traquent.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in traquent.get_hooks("get_site_info"):
		site_info.update(traquent.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(traquent.as_json(site_info))
...
def get_db_count(*args):
	...
	db_count = {}
	for doctype in args:
		db_count[doctype] = traquent.db.count(doctype)

	return json.loads(traquent.as_json(db_count))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = traquent.as_unicode(filters)
...
def set_request(**kwargs):
	...
	traquent.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return traquent.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(traquent.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return traquent.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = traquent.parse_json(traquent.read_file("assets/assets.json"))

		if assets_rtl := traquent.read_file("assets/assets-rtl.json"):
			assets.update(traquent.parse_json(assets_rtl))

		return assets

	if not hasattr(traquent.local, "assets_json"):
		if not traquent.conf.developer_mode:
			traquent.local.assets_json = traquent.cache.get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			traquent.local.assets_json = _get_assets()

	return traquent.local.assets_json
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	for info in missing_info:
		user_info.setdefault(info.name, traquent._dict()).update(
