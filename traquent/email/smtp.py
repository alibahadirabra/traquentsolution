...
from traquent.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(traquent.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			traquent.msgprint(
				...
				raise_exception=traquent.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				traquent.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=traquent.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					traquent.msgprint(res[1], raise_exception=traquent.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if traquent.request and hasattr(traquent.request, "after_response"):
			traquent.request.after_response.add(self.quit)
		elif traquent.job:
			traquent.job.after_job.add(self.quit)
		elif not traquent.flags.in_test:
			# Console?
			import atexit
