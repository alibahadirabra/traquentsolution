...
import traquent
from traquent.defaults import _clear_cache
from traquent.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		traquent.init(site)
		traquent.connect()
		enable_scheduler = cint(traquent.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		traquent.db.close()
...
def _new_site(
	...
	if not db_name:
		db_name = f"_{traquent.generate_hash(length=16)}"
	...
	if rollback_callback:
		rollback_callback.add(lambda: shutil.rmtree(traquent.get_site_path()))
	...
	scheduler_status = "disabled" if traquent.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from traquent.database import bootstrap_database, drop_user_and_database, setup_database

	if not db_type:
		db_type = traquent.conf.db_type
	...
	traquent.flags.in_install_db = True

	traquent.flags.root_login = root_login
	traquent.flags.root_password = root_password
	...
	traquent.conf.admin_password = traquent.conf.admin_password or admin_password

	remove_missing_apps()

	traquent.db.create_auth_table()
	traquent.db.create_global_search_table()
	traquent.db.create_user_settings_table()

	traquent.flags.in_install_db = False
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from traquent.model.sync import sync_for
	from traquent.modules.utils import sync_customizations
	from traquent.utils.fixtures import sync_fixtures

	traquent.flags.in_install = name
	traquent.flags.ignore_in_install = False

	traquent.clear_cache()
	app_hooks = traquent.get_hooks(app_name=name)
	...
	if name not in traquent.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = traquent.get_attr(before_install)()
		if out is False:
			return
	...
	traquent.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		traquent.get_attr(after_install)()

	for fn in traquent.get_hooks("after_app_install"):
		traquent.get_attr(fn)(name)
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		traquent.db.set_global("installed_apps", json.dumps(installed_apps))
		traquent.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = traquent.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in traquent.get_installed_apps():
	...
	for app in traquent.get_installed_apps():
		if app != app_name:
			hooks = traquent.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from traquent.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	traquent.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		traquent.get_attr(before_uninstall)()

	for fn in traquent.get_hooks("before_app_uninstall"):
		traquent.get_attr(fn)(app_name)

	modules = traquent.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		traquent.get_attr(after_uninstall)()

	for fn in traquent.get_hooks("after_app_uninstall"):
		traquent.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	traquent.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in traquent.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					traquent.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			traquent.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in traquent.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				traquent.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = traquent.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if traquent.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			traquent.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			traquent.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from traquent.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		traquent.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	traquent.db.commit()


def init_singles():
	singles = traquent.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if traquent.db.get_singles_dict(single):
			continue

		try:
			doc = traquent.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, traquent.DoesNotExistError):
...
def make_conf(
	...
	sites_path = traquent.local.sites_path
	traquent.destroy()
...
def make_site_config(
	...
	traquent.create_folder(os.path.join(traquent.local.site_path))
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(traquent.local, "conf"):
		traquent.local.conf[key] = value


def get_site_config_path():
	return os.path.join(traquent.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = traquent.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = traquent.get_module_list(app)
	for module in modules:
		d = traquent.new_doc("Module Def")
		d.app_name = app
...
def extract_files(site_name, file_path):
	...
	from traquent.utils import get_bench_relative_path
...
def is_downgrade(sql_file_path, verbose=False):
	...
	current_version = Version(traquent.__version__)
...
def partial_restore(sql_file_path, verbose=False):
	if traquent.conf.db_type == "mariadb":
		from traquent.database.mariadb.setup_db import import_db_from_sql
	elif traquent.conf.db_type == "postgres":
		import warnings

		from traquent.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		if (executable := which(executable_name)) is None:
			traquent.throw(
				...
				exc=traquent.ExecutableNotFound,
			)
		try:
			traquent.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise traquent.InvalidDatabaseFile
