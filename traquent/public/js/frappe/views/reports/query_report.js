...
traquent.provide("traquent.query_reports");

traquent.standard_pages["query-report"] = function () {
	var wrapper = traquent.container.add_page("query-report");
	...
	traquent.query_report = new traquent.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		traquent.query_report.show();
	});
};

traquent.views.QueryReport = class QueryReport extends traquent.views.BaseList {
	show() {
	...
	setup_defaults() {
		...
		this.refresh = traquent.utils.throttle(this.refresh, 300);
	...
	update_url_with_filters() {
		if (traquent.get_route_str() == this.page_name) {
	...
	setup_events() {
		traquent.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					traquent.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (traquent.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, traquent.route_options);

		if (this.report_name !== traquent.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (traquent.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!traquent.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!traquent.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = traquent.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = traquent.dashboard_utils.get_dashboard_link_field();
		const set_standard = traquent.boot.developer_mode;
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			traquent.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"traquent.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || traquent.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"traquent.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		traquent.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			traquent.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return traquent.model
			.with_doc("Report", this.report_name)
			.then((doc) => {
				this.report_doc = doc;
			})
			.then(() => traquent.model.with_doctype(this.report_doc?.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (traquent.query_reports[this.report_name]) {
				this.report_settings = traquent.query_reports[this.report_name];
				resolve();
			} else {
				traquent
					...
					.then((settings) => {
						traquent.dom.eval(settings.script || "");
						traquent.after_ajax(() => {
							...
							traquent.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return traquent.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			traquent.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = traquent.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					traquent.throw(
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = traquent.route_options;
		...
		if (route_options) {
			...
			return traquent.run_serially(promises);
		}
	}
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = traquent.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	get_query_params() {
		const query_string = traquent.utils.get_query_string(traquent.get_route_str());
		return traquent.utils.get_query_params(query_string);
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
				...
				function () {
					window.open(
						traquent.urllib.get_full_url(
							"/api/method/traquent.core.doctype.prepared_report.prepared_report.download_attachment?" +
								"dn=" +
			...
			let pretty_diff = traquent.datetime.comment_when(doc.report_end_time);
			const days_old = traquent.datetime.get_day_diff(
				traquent.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					traquent.set_route(traquent.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		traquent
			.xcall(
				"traquent.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = traquent.warn(
					...
					traquent.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		traquent
			.xcall("traquent.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				traquent.call({
					method: "traquent.core.doctype.prepared_report.prepared_report.make_prepared_report",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				traquent.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (data.length > 1000000) {
			...
			this.toggle_message(true, `${traquent.utils.icon("solid-warning")} ${msg}`);
			return;
		}
		...
		if (
			...
			let datatable_options = {
				...
				direction: traquent.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: traquent.utils.report_column_total,
				},
			};
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new traquent.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = traquent.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new traquent.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map((column) => {
			column = traquent.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = traquent.datetime.user_to_obj(keyword);
					const cellValue = traquent.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = traquent.get_meta(this.report_doc.ref_doctype);
		traquent.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		traquent.call("traquent.core.doctype.access_log.access_log.make_access_log", {
	...
	pdf_report(print_settings) {
		const base_url = traquent.urllib.get_base_url();
		const print_css = traquent.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = traquent.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = traquent.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: traquent.boot.lang,
			layout_direction: traquent.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		traquent.render_pdf(html, print_settings);
	}

	get_filters_html_for_print() {
		const applied_filters = this.get_filter_values();
		return Object.keys(applied_filters)
			.filter((fieldname) => !traquent.utils.is_empty(applied_filters[fieldname]))
			.map((fieldname) => {
				const docfield = traquent.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];
				return `<div class="filter-row">
					<b>${__(docfield.label, null, docfield.parent)}:</b> ${traquent.format(value, docfield)}
				</div>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = traquent.report_utils.get_export_dialog(
			__(this.report_name),
			extra_fields,
			({
				file_format,
				...
				let applied_filters = Object.fromEntries(
					Object.entries(filters).map(([key, value]) => [
						traquent.query_report.get_filter(key).df.label,
						traquent.query_report.get_filter(key).df.fieldtype == "Check"
				...
				const args = {
					cmd: "traquent.desk.query_report.export_query",
					report_name: this.report_name,
				...
				open_url_post(traquent.request.url, args);
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = traquent.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = traquent.ui.get_print_settings(
						false,
				...
				condition: () => traquent.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => traquent.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				...
				action: () => {
					let d = new traquent.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									traquent.model.with_doctype(doctype, () => {
										let options = traquent.meta
											.get_docfields(doctype)
											.filter(traquent.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = traquent.meta.get_docfield(values.doctype, values.field);
							...
							custom_columns.push({
								fieldname: this.columns
									...
									? df.fieldname + "-" + traquent.scrub(values.doctype)
									: df.fieldname,
							...
							traquent.call({
								method: "traquent.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
		...
		if (traquent.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new traquent.ui.Dialog({
						...
						primary_action: (values) => {
							traquent.call({
								method: "traquent.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									traquent.set_route("query-report", r.message);
								},
							});
						},
					});
	...
	add_custom_column(
		...
		this.data.forEach((row) => {
			if (column[0].fieldname.includes("-")) {
				row[column_field + "-" + traquent.scrub(new_column_data.doctype)] =
