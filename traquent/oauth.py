...
from traquent.utils.data import cstr, get_system_timezone, now_datetime
...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = traquent.db.get_value("OAuth Client", {"name": client_id})
		if cli_id:
			client = traquent.get_doc("OAuth Client", client_id)
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = (
			cstr(traquent.db.get_value("OAuth Client", client_id, "redirect_uris"))
			.strip()
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		return traquent.db.get_value("OAuth Client", client_id, "default_redirect_uri")
	...
	def save_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		oac = traquent.new_doc("OAuth Authorization Code")
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			oc = traquent.get_doc("OAuth Client", request.client_id)
		else:
			# Extract token, instantiate OAuth Bearer Token and use clientid from there.
			if "refresh_token" in traquent.form_dict:
				oc = traquent.get_doc(
					"OAuth Client",
					traquent.db.get_value(
						"OAuth Bearer Token",
						{"refresh_token": traquent.form_dict["refresh_token"]},
						"client",
					),
				)
			elif "token" in traquent.form_dict:
				oc = traquent.get_doc(
					"OAuth Client",
					traquent.db.get_value("OAuth Bearer Token", traquent.form_dict["token"], "client"),
				)
			else:
				oc = traquent.get_doc(
					"OAuth Client",
					traquent.db.get_value(
						"OAuth Bearer Token",
						traquent.get_request_header("Authorization").split(" ")[1],
						"client",
					),
				)
		try:
		...
		return traquent.session.user == user_id

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = traquent.db.get_value("OAuth Client", client_id, "name")
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = traquent.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		validcodes = traquent.get_all(
			"OAuth Authorization Code",
			filters={"client": client_id, "validity": "Valid"},
		)

		if code in [vcode["name"] for vcode in validcodes]:
			request.scopes = traquent.db.get_value("OAuth Authorization Code", code, "scopes").split(
				get_url_delimiter()
			)
			request.user = traquent.db.get_value("OAuth Authorization Code", code, "user")
			code_challenge_method = traquent.db.get_value(
				"OAuth Authorization Code", code, "code_challenge_method"
			)
			code_challenge = traquent.db.get_value("OAuth Authorization Code", code, "code_challenge")

			if code_challenge and not request.code_verifier:
				if traquent.db.exists("OAuth Authorization Code", code):
					traquent.delete_doc("OAuth Authorization Code", code, ignore_permissions=True)
					traquent.db.commit()
				return False
	...
	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = traquent.db.get_value("OAuth Client", client_id, "default_redirect_uri")

		redirect_uris = traquent.db.get_value("OAuth Client", client_id, "redirect_uris")
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = traquent.new_doc("OAuth Bearer Token")
		otoken.client = request.client["name"]
		try:
			otoken.user = (
				request.user
				if request.user
				else traquent.db.get_value(
					"OAuth Bearer Token",
			...
			otoken.user = traquent.session.user
		...
		return traquent.db.get_value("OAuth Client", request.client["name"], "default_redirect_uri")

	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		# Authorization codes are use once, invalidate it when a Bearer token
		# has been acquired.

		traquent.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		traquent.db.commit()
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		otoken = traquent.get_doc("OAuth Bearer Token", token)
		is_token_valid = (now_datetime() < otoken.expiration_time) and otoken.status != "Revoked"
		client_scopes = traquent.db.get_value("OAuth Client", otoken.client, "scopes").split(
			get_url_delimiter()
		)
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = traquent.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			traquent.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		elif token_type_hint == "refresh_token":
			traquent.db.set_value("OAuth Bearer Token", {"refresh_token": token}, "status", "Revoked")
		else:
			traquent.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		traquent.db.commit()
	...
	def validate_refresh_token(self, refresh_token, client, request, *args, **kwargs):
		...
		otoken = traquent.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token, "status": "Active"})
	...
	def finalize_id_token(self, id_token, token, token_handler, request):
		...
		return traquent.safe_decode(id_token_encoded)

	def get_authorization_code_nonce(self, client_id, code, redirect_uri, request):
		if traquent.get_value("OAuth Authorization Code", code, "validity") == "Valid":
			return traquent.get_value("OAuth Authorization Code", code, "nonce")

		return None

	def get_authorization_code_scopes(self, client_id, code, redirect_uri, request):
		scope = traquent.get_value("OAuth Client", client_id, "scopes")
		if not scope:
			scope = []
		else:
	...
	def validate_id_token(self, token, scopes, request):
		try:
			id_token = traquent.get_doc("OAuth Bearer Token", token)
	...
	def validate_jwt_bearer_token(self, token, scopes, request):
		try:
			jwt = traquent.get_doc("OAuth Bearer Token", token)
	...
	def validate_silent_login(self, request):
		...
		- OpenIDConnectHybrid
		"""
		if traquent.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
	...
	def validate_user_match(self, id_token_hint, scopes, claims, request):
		...
		if id_token_hint:
			try:
				...
				client_id, client_secret = traquent.get_value(
					"OAuth Client",
					payload.get("aud"),
				...
				if payload.get("sub") and client_id and client_secret:
					...
					if verified_payload:
						return user.name == traquent.session.user

			except Exception:
				return False

		elif traquent.session.user != "Guest":
			return True
...
def delete_oauth2_data():
	traquent.db.delete("OAuth Authorization Code", {"validity": "Invalid"})
	traquent.db.delete("OAuth Bearer Token", {"status": "Revoked"})


def get_client_scopes(client_id):
	scopes_string = traquent.db.get_value("OAuth Client", client_id, "scopes")
...
def generate_json_error_response(e):
	...
	traquent.local.response["http_status_code"] = getattr(e, "status_code", 500)
	return
