...
from traquent.permissions import add_permission, add_user_permission
from traquent.utils import get_link_to_form
from traquent.utils.modules import get_modules_from_app
...
class UserType(Document):
	...
	if TYPE_CHECKING:
		from traquent.core.doctype.user_document_type.user_document_type import UserDocumentType
		from traquent.core.doctype.user_select_document_type.user_select_document_type import (
			UserSelectDocumentType,
		)
		from traquent.core.doctype.user_type_module.user_type_module import UserTypeModule
	...
	def clear_cache(self):
		...
		if not self.is_standard:
			traquent.cache.delete_value("non_standard_user_types")
	...
	def on_trash(self):
		if self.is_standard:
			traquent.throw(_("Standard user type {0} can not be deleted.").format(traquent.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = traquent.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = traquent.conf.get("user_type_doctype_limit", {}).get(traquent.scrub(self.name))

		if not limit and traquent.session.user != "Administrator":
			traquent.throw(
				_("User does not have permission to create the new {0}").format(traquent.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			traquent.throw(_("The field {0} is mandatory").format(traquent.bold(_("Role"))))

		if not traquent.db.get_value("Role", self.role, "is_custom"):
			traquent.throw(
				_("The role {0} should be a custom role.").format(
					traquent.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in traquent.get_all("User", filters={"user_type": self.name}):
			user = traquent.get_cached_doc("User", row.name)
	...
	def update_modules_in_user(self, user):
		block_modules = traquent.get_all(
			"Module Def",
	...
	def add_select_perm_doctypes(self):
		if traquent.flags.ignore_select_perm:
			return
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in traquent.get_all(
			"Custom DocPerm", filters={"role": self.role, "parent": ["not in", doctypes]}
		):
			traquent.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = traquent.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	custom_doctypes = traquent.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		traquent.throw(_("User Id Field is mandatory in the user type {0}").format(traquent.bold(doc.name)))

	if traquent.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = traquent.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label

		traquent.msgprint(
			_(
				"To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record."
			).format(
				traquent.bold(doc.role),
				traquent.bold(user),
				traquent.bold(label),
				traquent.bold(user),
				traquent.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not traquent.db.table_exists("User Type") or traquent.flags.in_migrate:
		return

	user_types = traquent.cache.get_value(
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				user_perm = traquent.get_doc("User Permission", perm_data[0])
