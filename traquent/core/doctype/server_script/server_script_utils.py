...
def run_server_script_for_doc_event(doc, event):
	...
	if scripts:
		...
		for script_name in scripts:
			traquent.get_cached_doc("Server Script", script_name).execute_doc(doc)
...
def get_server_script_map():
	...
	# 	}
	# }
	if traquent.flags.in_patch and not traquent.db.table_exists("Server Script"):
		return {}

	script_map = traquent.cache.get_value("server_script_map")
	if script_map is None:
		script_map = {"permission_query": {}}
		enabled_server_scripts = traquent.get_all(
			"Server Script",
		...
		traquent.cache.set_value("server_script_map", script_map)
