...
import traquent
from traquent import _, safe_encode, task
...
from traquent.utils.verified_command import get_signed_params
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from traquent.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if traquent.session.user != "Administrator":
			traquent.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = traquent.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False) -> "EmailQueue":
		...
		doc = traquent.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = traquent.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return traquent.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			traquent.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(traquent.db.get_default("suspend_email_queue")) == 1
		):
			return False
	...
	def send(self, smtp_server_instance: SMTPServer = None, traquent_mail_client: traquentMail = None):
		...
		with SendMailContext(self, smtp_server_instance, traquent_mail_client) as ctx:
			...
			if traquent.flags.in_test and not traquent.flags.testing_email:
				traquent.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = traquent.qb.DocType("Email Queue")
		email_recipient = traquent.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			traquent.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			traquent.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from traquent.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": traquent.get_traceback()}
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = traquent.get_doc("File", file_filters)
				...
				print_format_file = traquent.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not traquent.get_system_settings("store_attached_pdf_document"):
			return

		file_data = traquent._dict(file_name=file_name, is_private=1)
		...
		if traquent.db.exists("File", file_data):
			return

		file = traquent.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	...
	email_queue = traquent.qb.DocType("Email Queue")
	traquent.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, traquent.session.user
...
def on_doctype_update():
	...
	traquent.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	traquent.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(traquent.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				traquent.qb.from_(EmailUnsubscribe)
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in traquent.utils.create_batch(final_recipients, 1000):
				traquent.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=traquent.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=traquent.flags.in_test or send_now,
					queue="long",
				)
	...
	def as_dict(self, include_recipients=True):
		...
		except traquent.InvalidEmailAddressError:
