...
from traquent.desk.form.save import savedocs
...
class TestFixtureImport(IntegrationTestCase):
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: list[str]) -> str:
		for name in dummy_name_list:
			doc = traquent.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(traquent.db.exists("DocType", "temp_doctype"))
		...
		traquent.db.truncate("temp_doctype")
		...
		self.assertEqual(traquent.db.count("temp_doctype"), len(dummy_name_list))

		data = traquent.get_all("temp_doctype", "member_name")
		traquent.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(traquent.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = traquent.qb.DocType("Singles")
		truncate_query = (
			traquent.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()

		import_doc(path_to_exported_fixtures)

		data = traquent.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
