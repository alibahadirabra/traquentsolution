...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			...
			traquent.exceptions.SiteNotSpecifiedError,
			traquent.exceptions.IncorrectSitePath,
			traquent.exceptions.CommandFailedError,
		) as e:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise traquent.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://traquentframework.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ testing_commands
