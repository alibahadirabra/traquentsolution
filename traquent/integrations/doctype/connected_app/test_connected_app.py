...
import traquent
from traquent.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_oauth_client():
	oauth_client = traquent.new_doc("OAuth Client")
...
class TestConnectedApp(IntegrationTestCase):
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = traquent.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = traquent.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = traquent.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = traquent.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
