...
from traquent.model.rename_doc import bulk_rename, update_document_title
from traquent.modules.utils import get_doc_path
...
def patch_db(endpoints: list[str] | None = None):
	...
	traquent.db.savepoint(save_point=savepoint)
	try:
...
class TestRenameDoc(IntegrationTestCase):
	...
	def setUpClass(self):
		...
		super().setUpClass()
		self._original_developer_flag = traquent.conf.developer_mode
		traquent.conf.developer_mode = 1
	...
	def tearDownClass(self):
		...
		traquent.delete_doc_if_exists("DocType", "Renamed Doc")
		traquent.db.sql_ddl("drop table if exists `tabRenamed Doc`")

		# delete the documents created
		for docname in self.available_documents:
			traquent.delete_doc(self.test_doctype, docname)

		for dt in self.doctype.values():
			if traquent.db.exists("DocType", dt):
				traquent.delete_doc("DocType", dt)
				traquent.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{dt}`")

		# reset original value of developer_mode conf
		traquent.conf.developer_mode = self._original_developer_flag

	def setUp(self):
		traquent.flags.link_fields = {}
		if self._testMethodName == "test_doc_rename_method":
			self.property_setter = traquent.get_doc(
				{
	...
	def test_merging_docs(self):
		...
		merged_todo = traquent.rename_doc(self.test_doctype, first_todo, second_todo, merge=True, force=True)
		merged_todo_doc = traquent.get_doc(self.test_doctype, merged_todo)
		self.available_documents.remove(first_todo)

		with self.assertRaises(DoesNotExistError):
			traquent.get_doc(self.test_doctype, first_todo)
	...
	def test_rename_controllers(self):
		...
		self.assertNotEqual(get_controller(self.doctype.old), traquent.model.document.Document)

		old_doctype_path = get_doc_path("Custom", "DocType", self.doctype.old)

		# rename doc via wrapper API accessible via /desk
		traquent.rename_doc("DocType", self.doctype.old, self.doctype.new)

		# check if database and controllers are updated
		self.assertTrue(traquent.db.exists("DocType", self.doctype.new))
		self.assertFalse(traquent.db.exists("DocType", self.doctype.old))
	...
	def test_rename_doctype(self):
		...
		to_rename_record = traquent.get_doc(
			{"doctype": "Rename This", "linked_to_doctype": "Rename This"}
		).insert()

		# Rename doctype
		self.assertEqual(
			"Renamed Doc", traquent.rename_doc("DocType", "Rename This", "Renamed Doc", force=True)
		)

		# Test if Doctype value has changed in Link field
		linked_to_doctype = traquent.db.get_value("Renamed Doc", to_rename_record.name, "linked_to_doctype")
		...
		new_name = "ToDo"
		self.assertEqual(new_name, traquent.rename_doc("Renamed Doc", old_name, new_name, force=True))

	def test_update_document_title_api(self):
		test_doctype = "Module Def"
		test_doc = traquent.get_doc(
			{
				"doctype": test_doctype,
				"module_name": f"Test-test_update_document_title_api-{traquent.generate_hash()}",
				"custom": True,
			}
		)
		...
		doc_before = traquent.get_doc(test_doctype, dn)
		return_value = update_document_title(doctype=dt, docname=dn, new_name=new_name)
		doc_after = traquent.get_doc(test_doctype, return_value)
	...
	def test_doc_rename_method(self):
		...
		new_name = f"{name}-{traquent.generate_hash(length=4)}"
		doc = traquent.get_doc(self.test_doctype, name)
		doc.rename(new_name, merge=traquent.db.exists(self.test_doctype, new_name))
	...
	def test_parenttype(self):
		...
		parent_a_instance = traquent.get_doc(
			doctype=parent_a.name, test_table=[{"some_fieldname": "x"}], name="XYZ"
		).insert()

		parent_b_instance = traquent.get_doc(
