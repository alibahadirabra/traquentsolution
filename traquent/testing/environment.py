...
def _initialize_test_environment(site, config):
	...
	traquent.flags.print_messages = logger.getEffectiveLevel() < logging.INFO
	traquent.flags.tests_verbose = logger.getEffectiveLevel() < logging.INFO
...
def _disable_scheduler_if_needed():
	...
	global scheduler_disabled_by_user
	scheduler_disabled_by_user = traquent.utils.scheduler.is_scheduler_disabled(verbose=False)
	if not scheduler_disabled_by_user:
