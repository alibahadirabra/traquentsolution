...
from traquent.desk.form.utils import get_pdf_link
from traquent.desk.notifications import clear_doctype_notifications
from traquent.model.document import Document
from traquent.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from traquent.utils import get_datetime, get_url
from traquent.utils.background_jobs import enqueue
from traquent.utils.data import get_link_to_form
from traquent.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	traquent.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		traquent.qb.from_(WorkflowAction)
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(traquent.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = traquent.session.user
	...
	if logged_in_user == "Guest":
		traquent.set_user(logged_in_user)
...
def return_success_page(doc):
	traquent.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), traquent.bold(doc.get("name")), traquent.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	traquent.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			traquent.bold(doc.get("name")),
			traquent.bold(doc_workflow_state),
			traquent.bold(traquent.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else traquent.session.user

	allowed_roles = traquent.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(traquent.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		traquent.qb.update(WorkflowAction)
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = traquent._dict(
					{
						"possible_actions": [],
						"email": traquent.db.get_value("User", user, "email"),
					}
				)
...
def create_workflow_actions_for_roles(roles, doc):
	if not roles:
		return
	workflow_action = traquent.get_doc(
		{
...
def get_common_email_args(doc):
	...
	if email_template:
		subject = traquent.render_template(email_template.subject, vars(doc))
		response = traquent.render_template(email_template.response, vars(doc))
	else:
	...
	lang = doc.get("language") or (
		traquent.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template(doc):
	...
	template_name = traquent.db.get_value(
	...
	if not template_name:
		return
	return traquent.get_doc("Email Template", template_name)


def get_state_optional_field_value(workflow_name, state):
	return traquent.get_cached_value(
