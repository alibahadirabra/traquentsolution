...
import traquent
from traquent.query_builder import DocType, Field
from traquent.utils import cstr, get_bench_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list | None = None) -> str:
	...
	"""
	is_logged_in = traquent.session.user != "Guest"

	# fetch language from form_dict
	if traquent.form_dict._lang:
		language = get_lang_code(traquent.form_dict._lang or get_parent_language(traquent.form_dict._lang))
		if language:
			return language
	...
	accept_language = list(traquent.request.accept_languages.values())
	...
	return traquent.db.get_default("lang") or "en"
...
def get_lang_code(lang: str) -> str | None:
	return traquent.db.get_value("Language", {"name": lang}) or traquent.db.get_value(
...
def set_default_language(lang):
	...
	if traquent.db.get_default("lang") != lang:
		traquent.db.set_default("lang", lang)
	traquent.local.lang = lang


def get_lang_dict():
	"""Return all languages in dict format, full name is the key e.g. `{"english":"en"}`."""
	return dict(
		traquent.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(traquent.local.lang)
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	try:
		return traquent.cache.hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
		...
		traquent.logger().error("Unable to load translations", exc_info=True)
		return {}
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from traquent.gettext.translate import get_translations_from_mo

	for app in apps or traquent.get_installed_apps(_ensure_on_bench=True):
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(traquent.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				traquent.log_error(message=msg, title="Error in translation file")
				if throw:
					traquent.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	...
	return traquent.cache.hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [traquent.unscrub(m) for m in traquent.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			traquent.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			traquent.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = traquent.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	if not meta.custom:
		doctype_file_path = traquent.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = traquent.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = traquent.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Workflow":
				workflows = traquent.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(traquent.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = traquent.db.get_values(
			document_state,
		...
		actions = traquent.db.get_values(
			"Workflow Transition",
...
def get_messages_from_custom_fields(app_name):
	...
	for fixture in fixtures:
		if isinstance(fixture, str) and fixture == "Custom Field":
			custom_fields = traquent.get_all(
...
def get_messages_from_report(name):
	...
	messages = _get_messages_from_page_or_report(
		"Report", name, traquent.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = traquent.db.get_value(doctype, name, "module")

	doc_path = traquent.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, traquent.scrub(name) + ".py"))
...
def get_server_messages(app):
	...
	app_walk = os.walk(traquent.get_app_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	from traquent.utils.jinja_globals import bundled_asset

	messages = []
	app_include_js = traquent.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = traquent.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(traquent.local.sites_path, file_path.lstrip("/"))
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from traquent.gettext.extractors.utils import extract_messages_from_code

	traquent.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in traquent.flags.scanned_files:
		return []

	traquent.flags.scanned_files.add(path)
...
def extract_messages_from_javascript_code(code: str) -> list[tuple[int, str, str | None]]:
	...
	messages = []
	from traquent.gettext.extractors.javascript import extract_javascript
...
def get_untranslated(lang, untranslated_file, get_all=False, app="_ALL_APPS"):
	...
	clear_cache()
	apps = traquent.get_all_apps(True)
	if app != "_ALL_APPS":
		if app not in apps:
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		traquent.get_file_items(untranslated_file, ignore_empty_lines=False),
		traquent.get_file_items(translated_file, ignore_empty_lines=False),
		strict=False,
	):
...
def import_translations(lang, path):
	...
	for app in traquent.get_all_apps(True):
...
def migrate_translations(source_app, target_app):
	...
	strings_in_source_app = [m[1] for m in traquent.translate.get_messages_for_app(source_app)]
	strings_in_target_app = [m[1] for m in traquent.translate.get_messages_for_app(target_app)]

	strings_in_target_app_but_not_in_source_app = list(
		set(strings_in_target_app) - set(strings_in_source_app)
	)

	languages = traquent.translate.get_all_languages()

	source_app_translations_dir = traquent.get_app_path(source_app, "translations")
	target_app_translations_dir = traquent.get_app_path(target_app, "translations")
...
def rebuild_all_translation_files():
	...
	for lang in get_all_languages():
		for app in traquent.get_all_apps():
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = traquent.get_app_path(app, "translations")
	traquent.create_folder(tpath)
...
def send_translations(translation_dict):
	...
	traquent.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = traquent.db.get_values(
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	def get_language_codes():
		return traquent.get_all("Language", filters={"enabled": 1}, pluck="name")

	def get_all_language_with_name():
		return traquent.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if with_language_name:
		return traquent.cache.get_value("languages_with_name", get_all_language_with_name)
	else:
		return traquent.cache.get_value("languages", get_language_codes)


def get_preferred_language_cookie():
	return traquent.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = traquent.get_all("DocType", {"translated_doctype": 1}, pluck="name")
...
def print_language(language: str):
	...
	_lang = traquent.local.lang
	_jenv = traquent.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	traquent.local.lang = language
	traquent.local.jenv = None

	yield

	# restore original values
	traquent.local.lang = _lang
	traquent.local.jenv = _jenv
