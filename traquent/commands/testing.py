...
if TYPE_CHECKING:
	...
	from traquent.testing import TestRunner
...
def main(
	...
	try:
		...
		for app, category, result in results:
			_print_result(traquent.unscrub(app or "Unspecified App"), traquent.unscrub(category), result)
...
def _load_doctype_list(doctype_list_path):
	...
	with open(traquent.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from traquent.testing import discover_doctype_tests
...
def _get_doctypes_for_module_def(app, module_def):
	...
	doctypes = []
	doctypes_ = traquent.get_list(
		"DocType",
		filters={"module": module_def, "istable": 0},
		fields=["name", "module"],
		as_list=True,
	)
	from traquent.modules import get_module_name
...
@click.option(
	"--doctype-list-path",
	help="Path to .txt file for list of doctypes. Example traquent/tests/server/agriculture.txt",
)
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		traquent.init(site)
		allow_tests = traquent.get_conf().allow_tests
...
def run_ui_tests(
	...
	site = get_site(context)
	traquent.init(site)
	app_base_path = traquent.get_app_source_path(app)
	site_url = traquent.utils.get_site_url(site)
	admin_password = traquent.get_conf().admin_password
	...
	traquent.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
