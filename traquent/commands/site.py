...
from traquent.utils import CallbackManager
...
def new_site(
	...
	"Create a new site"
	from traquent.installer import _new_site

	traquent.init(site, new_site=True)
...
def _restore(
	*,
	...
	from traquent.installer import extract_files
	from traquent.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	err, out = traquent.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from traquent.installer import _new_site, is_downgrade, is_partial, validate_database_sql
...
def partial_restore(context: CliCtxObj, sql_file_path, verbose, encryption_key=None):
	from traquent.installer import is_partial, partial_restore
	from traquent.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
...
def _reinstall(
	...
	try:
		...
		installed = traquent.get_installed_apps()
		traquent.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context: CliCtxObj, apps, force=False):
	...
	from traquent.installer import install_app as _install_app
	from traquent.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except traquent.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{traquent.get_traceback(with_context=True)}"
...
def list_apps(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		apps = traquent.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(traquent.get_installed_apps())
	...
	if format == "json":
		click.echo(traquent.as_json(summary_dict))
...
def add_db_index(context: CliCtxObj, doctype, column):
	...
	for site in context.sites:
		...
		traquent.connect()
		try:
			traquent.db.add_index(doctype, columns)
			if len(columns) == 1:
...
def describe_database_table(context, doctype, column):
	...
	from traquent.core.doctype.recorder.recorder import _fetch_table_stats

	for site in context.sites:
		traquent.init(site=site)
		traquent.connect()
		try:
...
def add_system_manager(context: CliCtxObj, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		traquent.connect()
		try:
			traquent.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			traquent.db.commit()
		finally:
			traquent.destroy()
	if not context.sites:
...
def disable_user(context: CliCtxObj, email):
	...
	site = get_site(context)
	with traquent.init_site(site):
		traquent.connect()
		user = traquent.get_doc("User", email)
		user.enabled = 0
...
def migrate(context: CliCtxObj, skip_failing=False, skip_search_index=False):
	...
	from traquent.migrate import SiteMigration
...
def run_patch(context: CliCtxObj, module, force):
	...
	for site in context.sites:
		...
		try:
			traquent.connect()
			traquent.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			traquent.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context: CliCtxObj, module, doctype, docname):
	...
	for site in context.sites:
		try:
			traquent.init(site)
			traquent.connect()
			traquent.reload_doc(module, doctype, docname, force=context.force)
			traquent.db.commit()
		finally:
			traquent.destroy()
	if not context.sites:
...
def reload_doctype(context: CliCtxObj, doctype):
	...
	for site in context.sites:
		try:
			traquent.init(site)
			traquent.connect()
			traquent.reload_doctype(doctype, force=context.force)
			traquent.db.commit()
		finally:
			traquent.destroy()
	if not context.sites:
...
def add_to_hosts(context: CliCtxObj):
	"Add site to hosts"
	for site in context.sites:
		traquent.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(traquent.get_traceback(with_context=True))
			exit_code = 1
			continue
		if traquent.get_system_settings("encrypt_backup") and traquent.get_site_config().encryption_key:
			click.secho(
...
def _drop_site(
	...
	from traquent.database import drop_user_and_database
	from traquent.utils.backups import scheduled_backup
	...
	traquent.flags.root_login = db_root_username
	traquent.flags.root_password = db_root_password

	drop_user_and_database(traquent.conf.db_name, traquent.conf.db_user)

	archived_sites_path = archived_sites_path or os.path.join(
		traquent.utils.get_bench_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	traquent.init(site)
	old_path = traquent.utils.get_site_path()
...
def set_last_active_for_user(context: CliCtxObj, user=None):
	...
	with traquent.init_site(site):
		...
		traquent.db.set_value("User", user, "last_active", now_datetime())
		traquent.db.commit()
...
def publish_realtime(context: CliCtxObj, event, message, room, user, doctype, docname, after_commit):
	...
	from traquent import publish_realtime
...
def browse(context: CliCtxObj, site, user=None):
	...
	if site not in traquent.utils.get_sites():
	...
	if user:
		...
		if traquent.conf.developer_mode or user == "Administrator":
			...
			sid = f"/app?sid={traquent.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{traquent.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context: CliCtxObj, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = traquent.conf.ngrok_authtoken
...
def clear_log_table(context: CliCtxObj, doctype, days, no_backup):
	...
	"""
	from traquent.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from traquent.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from traquent.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise traquent.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context: CliCtxObj, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		database_tables: list[str] = (
			traquent.qb.from_(information_schema.tables)
			.select(table_name)
			.where(information_schema.tables.table_schema == traquent.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = traquent.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				traquent.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[traquent.local.site] = TABLES_TO_DROP
		traquent.destroy()
...
def trim_tables(context: CliCtxObj, dry_run, format, no_backup):
	...
	from traquent.model.meta import trim_tables
	from traquent.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {traquent.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {traquent.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({traquent.local.site: data}, indent=1, sort_keys=True))
	else:
		from traquent.utils.commands import render_table
...
def bypass_patch(context: CliCtxObj, patch_name: str, yes: bool):
	...
	from traquent.modules.patch_handler import update_patch_log
