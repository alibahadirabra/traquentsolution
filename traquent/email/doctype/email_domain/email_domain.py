...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if traquent.local.flags.in_patch or traquent.local.flags.in_test or traquent.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in traquent.get_all("Email Account", filters={"domain": self.name}):
			try:
