...
from traquent.custom.doctype.customize_form.test_customize_form import TestCustomizeForm
from traquent.desk.notifications import get_open_count
...
class TestDashboardConnections(IntegrationTestCase):
	...
	def test_external_doctype_link_with_dashboard_override(self):
		...
		# create a test doc
		todo_doc = traquent.get_doc(doctype="ToDo", description="test").insert()
		traquent.get_doc(doctype="Test Doctype D", title="d-001", doclink=todo_doc.name).insert()
		traquent.get_doc(doctype="Test Doctype E", title="e-001", todo=todo_doc.name).insert()
...
def add_links_in_child_tables():
	test_child_table_with_link_to_doctype_a = traquent.get_doc(
	...
	test_child_table_with_link_to_doctype_b = traquent.get_doc(
