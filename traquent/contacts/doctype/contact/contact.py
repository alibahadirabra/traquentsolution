...
from traquent.contacts.address_and_contact import set_link_title
from traquent.core.doctype.access_log.access_log import make_access_log
from traquent.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from traquent.model.document import Document
from traquent.model.naming import append_number_if_name_exists
from traquent.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	if TYPE_CHECKING:
		from traquent.contacts.doctype.contact_email.contact_email import ContactEmail
		from traquent.contacts.doctype.contact_phone.contact_phone import ContactPhone
		from traquent.core.doctype.dynamic_link.dynamic_link import DynamicLink
	...
	def autoname(self):
		...
		if traquent.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			traquent.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = traquent.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not traquent.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not traquent.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			traquent.throw(_("Only one {0} can be set as primary.").format(traquent.bold("Email ID")))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			traquent.throw(
				_("Only one {0} can be set as primary.").format(traquent.bold(traquent.unscrub(fieldname)))
			)
...
def download_vcard(contact: str):
	...
	traquent.response["filename"] = f"{contact.name}.vcf"
	traquent.response["filecontent"] = vcard.serialize().encode("utf-8")
...
def download_vcards(contacts: str):
	...
	from traquent.utils.data import now

	contact_ids = traquent.parse_json(contacts)

	vcards = []
	for contact_id in contact_ids:
		contact = traquent.get_doc("Contact", contact_id)
		contact.check_permission()
		vcard = contact.get_vcard()
	...
	traquent.response["filename"] = f"{timestamp} Contacts.vcf"
	traquent.response["filecontent"] = "\n".join(vcards).encode("utf-8")
	traquent.response["type"] = "binary"
...
def invite_user(contact: str):
	...
	if not contact.email_id:
		traquent.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = traquent.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	...
	doctype = "Contact"
	if not traquent.get_meta(doctype).get_field(searchfield) and searchfield not in traquent.db.DEFAULT_COLUMNS:
		return []
...
def address_query(links):
	...
	for link in links:
		if not traquent.has_permission(
...
def get_contact_name(email_id):
	contact = traquent.get_all("Contact Email", filters={"email_id": email_id}, fields=["parent"], limit=1)
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = traquent.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = traquent.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return traquent.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	from traquent.contacts.doctype.address.address import get_condensed_address

	if not traquent.has_permission("Contact", "read"):
		return []

	contact_list = traquent.get_list(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
	...
	for contact in contact_list:
		contact["email_ids"] = traquent.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = traquent.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address and traquent.has_permission("Address", "read"):
			address = traquent.get_doc("Address", contact.address)
