...
from traquent.core.doctype.user_permission.user_permission import (
	add_user_permissions,
	remove_applicable,
)
from traquent.permissions import add_permission, has_user_permission
from traquent.tests import IntegrationTestCase, UnitTestCase
from traquent.website.doctype.blog_post.test_blog_post import make_test_blog
...
class TestUserPermission(IntegrationTestCase):
	def setUp(self):
		...
		traquent.db.delete("User Permission", {"user": ("in", test_users)})
		traquent.delete_doc_if_exists("DocType", "Person")
		traquent.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		traquent.delete_doc_if_exists("DocType", "Doc A")
		traquent.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(traquent.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		add_user_permissions(param)
		traquent.db.delete("User Permission", filters={"for_value": test_blog.name})
		traquent.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not traquent.db.exists("Blog Category", category):
				traquent.get_doc({"doctype": "Blog Category", "title": category}).insert()
		...
		traquent.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = traquent.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = traquent.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = traquent.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = traquent.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = traquent.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = traquent.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not traquent.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = traquent.get_doc({"doctype": "Person", "person_name": "Parent", "is_group": 1}).insert()

		child_record = traquent.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(traquent.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(traquent.get_doc("Person", child_record.name), user.name))

		#  give access of Parent DocType to Blogger role
		add_permission("Person", "Blogger")
		traquent.set_user(user.name)
		visible_names = traquent.get_list(
			doctype="Person",
			pluck="person_name",
		)

		user_permission = traquent.get_doc(
		...
		# hides child records
		self.assertTrue(has_user_permission(traquent.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(traquent.get_doc("Person", child_record.name), user.name))

		visible_names_after_hide_descendants = traquent.get_list(
			"Person",
			pluck="person_name",
		)
	...
	def test_user_perm_on_new_doc_with_field_default(self):
		...
		if not traquent.db.exists("DocType", "Doc A"):
			doc = new_doctype(
				"Doc A",
				fields=[
					{
		...
		traquent.set_user("new_doc_test@example.com")

		new_doc = traquent.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		...
		if not traquent.db.exists("Session Default", {"ref_doctype": "DocType"}):
		...
		traquent.set_user("user_default_test@example.com")
...
def create_user(email, *roles):
	...
	if traquent.db.exists("User", email):
		return traquent.get_doc("User", email)
