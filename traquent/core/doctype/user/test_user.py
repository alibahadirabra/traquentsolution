...
from traquent.core.doctype.user.user import (
	User,
...
class TestUser(IntegrationTestCase):
	def tearDown(self):
		...
		traquent.db.set_single_value("System Settings", "enable_password_policy", 0)
		traquent.db.set_single_value("System Settings", "minimum_password_score", "")
	...
	def test_user_type(self):
		user_id = traquent.generate_hash() + "@example.com"
		...
		traquent.delete_doc("User", new_user.name)

	def test_delete(self):
		traquent.get_doc("User", "test@example.com").add_roles("_Test Role 2")
		self.assertRaises(traquent.LinkExistsError, delete_doc, "Role", "_Test Role 2")
		traquent.db.delete("Has Role", {"role": "_Test Role 2"})
		delete_doc("Role", "_Test Role 2")

		if traquent.db.exists("User", "_test@example.com"):
			delete_contact("_test@example.com")
			delete_doc("User", "_test@example.com")

		user = traquent.copy_doc(self.globalTestRecords["User"][1])
		user.email = "_test@example.com"
		user.insert()

		traquent.get_doc({"doctype": "ToDo", "description": "_Test"}).insert()

		delete_contact("_test@example.com")
		delete_doc("User", "_test@example.com")

		self.assertTrue(
			not traquent.db.sql("""select * from `tabToDo` where allocated_to=%s""", ("_test@example.com",))
		)

		traquent.copy_doc(self.globalTestRecords["Role"][1]).insert()

	def test_get_value(self):
		self.assertEqual(traquent.db.get_value("User", "test@example.com"), "test@example.com")
		self.assertEqual(traquent.db.get_value("User", {"email": "test@example.com"}), "test@example.com")
		self.assertEqual(
			traquent.db.get_value("User", {"email": "test@example.com"}, "email"), "test@example.com"
		)
		self.assertEqual(
			traquent.db.get_value("User", {"email": "test@example.com"}, ["first_name", "email"]),
		...
		test_user = traquent.db.sql("select * from tabUser where name='test@example.com'", as_dict=True)[0]
		self.assertEqual(
			traquent.db.get_value("User", {"email": "test@example.com"}, "*", as_dict=True), test_user
		)

		self.assertEqual(traquent.db.get_value("User", "xxxtest@example.com"), None)

		traquent.db.set_single_value("Website Settings", "_test", "_test_val")
		self.assertEqual(traquent.db.get_value("Website Settings", None, "_test"), "_test_val")
		self.assertEqual(traquent.db.get_value("Website Settings", "Website Settings", "_test"), "_test_val")

	def test_high_permlevel_validations(self):
		user = traquent.get_meta("User")
		self.assertTrue("roles" in [d.fieldname for d in user.get_high_permlevel_fields()])

		me = traquent.get_doc("User", "testperm@example.com")
	...
	def test_delete_user(self):
		...
		delete_contact(new_user.name)
		traquent.delete_doc("User", new_user.name)
		self.assertFalse(traquent.db.exists("User", new_user.name))
	...
	def test_password_strength(self):
		...
		traquent.db.set_single_value("System Settings", "enable_password_policy", 1)
		traquent.db.set_single_value("System Settings", "minimum_password_score", 2)
		...
		self.assertRaises(traquent.exceptions.ValidationError, handle_password_test_fail, result["feedback"])
		self.assertRaises(
			traquent.exceptions.ValidationError, handle_password_test_fail, result
		...
		self.assertRaises(traquent.exceptions.ValidationError, user.save)
		user.reload()
	...
	def test_comment_mentions(self):
		...
		traquent.delete_doc("User Group", "Team")
	...
	def test_user_rename(self):
		...
		traquent.rename_doc("User", user.name, new_name)
		self.assertTrue(traquent.db.exists("Notification Settings", new_name))

		traquent.delete_doc("User", new_name)

	def test_signup(self):
		import traquent.website.utils

		random_user = traquent.mock("email")
		random_user_name = traquent.mock("name")
		# disabled signup
		...
		self.assertEqual(traquent.cache.hget("redirect_after_login", random_user), "/welcome")

		# re-register
		self.assertTupleEqual(sign_up(random_user, random_user_name, "/welcome"), (0, "Already Registered"))

		# disabled user
		user = traquent.get_doc("User", random_user)
		user.enabled = 0
		user.save()
	...
	def test_reset_password(self):
		...
		traquent.local.request_ip = "127.0.0.69"

		traquent.set_user("testpassword@example.com")
		test_user = traquent.get_doc("User", "testpassword@example.com")
		...
		self.assertRaises(traquent.exceptions.AuthenticationError, verify_password, old_password)
...
def test_user(
	*, first_name: str | None = None, email: str | None = None, roles: list[str], commit=False, **kwargs
	...
	try:
		first_name = first_name or traquent.generate_hash()
		email = email or (first_name + "@example.com")
		user: User = traquent.new_doc(
			"User",
		...
		commit and traquent.db.commit()


def delete_contact(user):
	traquent.db.delete("Contact", {"email_id": user})
	traquent.db.delete("Contact Email", {"email_id": user})
