...
from traquent.model.base_document import (
...
from traquent.utils import cast, cint, cstr
...
def get_meta(doctype: str | Document, cached=True) -> "_Meta":
	...
	if cached and not isinstance(doctype, Document):
		if meta := traquent.cache.hget("doctype_meta", doctype_name):
			return meta

	meta = Meta(doctype)
	traquent.cache.hset("doctype_meta", meta.name, meta)
	return meta
...
def get_table_columns(doctype):
	return traquent.db.get_table_columns(doctype)
...
class Meta(Document):
	...
	standard_set_once_fields = (
		traquent._dict(fieldname="creation", fieldtype="Datetime"),
		traquent._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(traquent._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self) -> list[str]:
		if not hasattr(self, "_valid_columns"):
			table_exists = traquent.db.table_exists(self.name)
	...
	def get_valid_fields(self) -> list[str]:
		if not hasattr(self, "_valid_fields"):
			if (traquent.flags.in_install or traquent.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not traquent.db.table_exists("Custom Field"):
			return

		custom_fields = traquent.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not traquent.db.table_exists("Property Setter"):
			return

		property_setters = traquent.db.get_values(
			"Property Setter",
	...
	def set_custom_permissions(self):
		...
		if traquent.flags.in_patch or traquent.flags.in_install:
			return

		if not self.istable and self.name not in ("DocType", "DocField", "DocPerm", "Custom DocPerm"):
			custom_perms = traquent.get_all(
				"Custom DocPerm",
				fields="*",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(traquent._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if traquent.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if traquent.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = traquent.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = traquent._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in traquent.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = traquent._dict(traquent.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = traquent.scrub(self.module)
		doctype = traquent.scrub(self.name)
		template_path = traquent.get_module_path(
...
def is_single(doctype):
	try:
		return traquent.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	if not traquent.is_table(dt):
		return ""

	return (
		traquent.db.get_value(
			"DocField",
			{"fieldtype": ("in", traquent.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	traquent.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(traquent.local, "field_currency", None):
		traquent.local.field_currency = traquent._dict()

	if not (
		traquent.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and traquent.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = traquent.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if traquent.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = traquent.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			traquent.local.field_currency.setdefault((doc.doctype, ref_docname), traquent._dict()).setdefault(
				df.fieldname, currency
			)

	return traquent.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and traquent.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from traquent.locale import get_number_format

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(traquent.db.get_default("currency_precision"))
		if not precision:
			number_format = get_number_format()
			precision = number_format.precision
	else:
		precision = cint(traquent.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return traquent._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return traquent._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return traquent._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return traquent._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in traquent.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	traquent.cache.hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = traquent.db.get_table_columns(doctype)
	fields = traquent.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		traquent.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
