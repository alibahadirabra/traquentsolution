...
from traquent.modules import export_customizations, export_module_json, get_module_path
from traquent.modules.utils import export_doc, sync_customizations
...
class TestUtils(IntegrationTestCase):
	def setUp(self):
		self._dev_mode = traquent.local.conf.developer_mode
		traquent.local.conf.developer_mode = True

	def tearDown(self):
		traquent.db.rollback()
		traquent.local.conf.developer_mode = self._dev_mode
		traquent.local.flags.pop("in_import", None)

	def test_export_module_json_no_export(self):
		traquent.local.flags.in_import = True
		doc = traquent.get_last_doc("DocType")
	...
	def test_export_module_json(self):
		...
		with open(export_doc_path) as f:
			export_doc_before = traquent.parse_json(f.read())

		last_modified_before = os.path.getmtime(export_doc_path)
		self.addCleanup(write_file, path=export_doc_path, content=traquent.as_json(export_doc_before))

		traquent.flags.in_import = False
		traquent.conf.developer_mode = True
		...
		with open(f"{export_path}.json") as f:
			traquent.parse_json(f.read())  # export_doc_after
	...
	def test_reload_doc(self):
		traquent.db.set_value("DocType", "Note", "migration_hash", "", update_modified=False)
		self.assertFalse(traquent.db.get_value("DocType", "Note", "migration_hash"))
		...
		self.assertEqual(
			traquent.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Text",
		)
		traquent.reload_doctype("Note")
		self.assertEqual(
			traquent.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Data",
		)
	...
	def test_export_doc(self):
		...
		note.title = traquent.generate_hash(length=10)
		note.save()
...
def temp_doctype():
	try:
		...
		traquent.db.sql_ddl(f"DROP TABLE `tab{doctype.name}`")
...
def note_customizations():
	try:
		...
		delete_path(traquent.get_module_path("Desk", "Note"))
