...
class TestRedisCache(traquentAPITestCase):
	...
	def test_user_cache(self):
		...
		def calculate_area(radius: float) -> float:
			nonlocal function_call_count
			PI_APPROX = ENGINEERING_PI if traquent.session.user == "Engineer" else PI
			function_call_count += 1
...
class TestDocumentCache(traquentAPITestCase):
	...
	def setUp(self) -> None:
		self.test_value = traquent.generate_hash()

	def test_caching(self):
		traquent.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		with self.assertQueryCount(0):
			doc = traquent.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
		...
		doc_after_rollback = traquent.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
...
class TestRedisWrapper(traquentAPITestCase):
	def test_delete_keys(self):
		...
		for i in range(5):
			traquent.cache.set_value(f"{prefix}{i}", 1)

		self.assertEqual(len(traquent.cache.get_keys(prefix)), 5)
		traquent.cache.delete_keys(prefix)
		self.assertEqual(len(traquent.cache.get_keys(prefix)), 0)

	def test_hash(self):
		key = "test_hash"

		# Confirm that there's no data initially
		exists = traquent.cache.exists(key)
		self.assertFalse(exists)

		# Insert 5 key-value pairs
		for i in range(5):
			traquent.cache.hset(key, f"key_{i}", f"value_{i}")

		# Check that we have 5 values
		values = traquent.cache.hgetall(key)
		self.assertEqual(len(values), 5)

		# Check that each value matches
		for i in range(5):
			value = traquent.cache.hget(key, f"key_{i}")
			self.assertEqual(value, f"value_{i}")

		# Check the keys themselves
		keys = traquent.cache.hkeys(key)
		for i in range(5):
			self.assertIn(f"key_{i}".encode(), keys)

		# Delete a single key and check that we still have the remaining 4
		traquent.cache.hdel(key, "key_1")
		values = traquent.cache.hgetall(key)
		self.assertEqual(len(values), 4)

		# Delete 2 keys and check that we still have the remaining 2
		traquent.cache.hdel(key, ["key_2", "key_3"])
		values = traquent.cache.hgetall(key)
		self.assertEqual(len(values), 2)

		# Delete the hash itself and confirm that there's no data
		traquent.cache.delete_value(key)
		exists = traquent.cache.exists(key)
		self.assertFalse(exists)

	def test_user_cache_clear(self):
		from traquent.cache_manager import user_cache_keys

		# Set some keys that a user's cache would usually have
		user1 = traquent.utils.random_string(10)
		user2 = traquent.utils.random_string(10)
		for key in user_cache_keys:
			traquent.cache.hset(key, user1, key)
			traquent.cache.hset(key, user2, key)

		traquent.clear_cache(user=user1)

		# Check that the keys for user1 are gone
		for key in set(user_cache_keys) - {"home_page"}:
			self.assertFalse(traquent.cache.hexists(key, user1))
			self.assertTrue(traquent.cache.hexists(key, user2))

	def test_doctype_cache_clear(self):
		from traquent.cache_manager import doctype_cache_keys

		# Set some keys that a user's cache would usually have
		doctype1 = new_doctype(traquent.utils.random_string(10))
		doctype2 = new_doctype(traquent.utils.random_string(10))
		for key in doctype_cache_keys:
			traquent.cache.hset(key, doctype1.name, key)
			traquent.cache.hset(key, doctype2.name, key)

		traquent.clear_cache(doctype=doctype1.name)

		# Check that the keys for doctype1 are gone
		for key in doctype_cache_keys:
			self.assertFalse(traquent.cache.hexists(key, doctype1.name))
			self.assertTrue(traquent.cache.hexists(key, doctype2.name))

	def test_backward_compat_cache(self):
		self.assertEqual(traquent.cache, traquent.cache())
