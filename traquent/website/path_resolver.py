...
import traquent
from traquent.website.page_renderers.document_page import DocumentPage
from traquent.website.page_renderers.list_page import ListPage
from traquent.website.page_renderers.not_found_page import NotFoundPage
from traquent.website.page_renderers.print_page import PrintPage
...
from traquent.website.page_renderers.web_form import WebFormPage
from traquent.website.router import evaluate_dynamic_routes
from traquent.website.utils import can_cache, get_home_page
...
class PathResolver:
	...
	def resolve(self):
		...
		request = traquent._dict()
		if hasattr(traquent.local, "request"):
			request = traquent.local.request or request

		# check if the request url is in 404 list
		if request.url and can_cache() and traquent.cache.hget("website_404", request.url):
			return self.path, NotFoundPage(self.path)

		try:
			resolve_redirect(self.path, request.query_string)
		except traquent.Redirect as e:
			return traquent.flags.redirect_location, RedirectPage(self.path, e.http_status_code)

		if traquent.get_hooks("website_path_resolver"):
			for handler in traquent.get_hooks("website_path_resolver"):
				endpoint = traquent.get_attr(handler)(self.path)
		else:
			try:
				endpoint = resolve_path(self.path)
			except werkzeug.routing.exceptions.RequestRedirect as e:
				traquent.flags.redirect_location = e.new_url
				return traquent.flags.redirect_location, RedirectPage(e.new_url, e.code)
	...
	def get_custom_page_renderers():
		custom_renderers = []
		for renderer_path in traquent.get_hooks("page_renderer") or []:
			try:
				renderer = traquent.get_attr(renderer_path)
...
def resolve_redirect(path, query_string=None):
	...
	"""
	redirects = traquent.get_hooks("website_redirects")
	redirects += traquent.get_all(
	...
	redirect_to = traquent.cache.hget("website_redirects", path)

	if redirect_to:
		if isinstance(redirect_to, dict):
			traquent.flags.redirect_location = redirect_to["path"]
			raise traquent.Redirect(redirect_to["status_code"])
		traquent.flags.redirect_location = redirect_to
	...
	for rule in redirects:
		...
		if query_string and rule.get("match_with_query_string"):
			path_to_match = path + "?" + traquent.safe_decode(query_string)

		try:
			match = re.match(pattern, path_to_match)
		except re.error:
			traquent.log_error("Broken Redirect: " + pattern)
		...
		if match:
			...
			raise traquent.Redirect(status_code)
...
def get_website_rules():
	...
	def _get():
		rules = traquent.get_hooks("website_route_rules")
		for d in traquent.get_all("DocType", "name, route", dict(has_web_view=1)):
			if d.route:
	...
	return traquent.cache.get_value("website_route_rules", _get)


def validate_path(path: str):
	if not PathResolver(path).is_valid_path():
		traquent.throw(traquent._("Path {0} it not a valid path").format(traquent.bold(path)))
