...
import traquent
from traquent.core.doctype.scheduled_job_type.scheduled_job_type import ScheduledJobType
from traquent.model.document import Document
from traquent.utils.background_jobs import get_queue, get_queue_list, get_redis_conn
from traquent.utils.caching import redis_cache
from traquent.utils.data import add_to_date
from traquent.utils.scheduler import get_scheduler_status, get_scheduler_tick
	...
	def suppress_exception(func: Callable):
		...
		def wrapper(*args, **kwargs):
			try:
				...
				traquent.log(traquent.get_traceback())
				# nosemgrep
				traquent.msgprint(
					f"System Health check step {traquent.bold(step)} failed: {e}", alert=True, indicator="red"
				)
...
class SystemHealthReport(Document):
	...
	if TYPE_CHECKING:
		from traquent.desk.doctype.system_health_report_errors.system_health_report_errors import (
			SystemHealthReportErrors,
		)
		from traquent.desk.doctype.system_health_report_failing_jobs.system_health_report_failing_jobs import (
			SystemHealthReportFailingJobs,
		)
		from traquent.desk.doctype.system_health_report_queue.system_health_report_queue import (
			SystemHealthReportQueue,
		)
		from traquent.desk.doctype.system_health_report_tables.system_health_report_tables import (
			SystemHealthReportTables,
		)
		from traquent.desk.doctype.system_health_report_workers.system_health_report_workers import (
	...
	def fetch_scheduler(self):
		...
		failing_jobs = traquent.db.multisql(
			{
		...
		for job_type in traquent.get_all(
			"Scheduled Job Type",
			filters={"stopped": 0, "last_execution": ("<", threshold)},
			fields="*",
			order_by="last_execution asc",
		):
			job_type: ScheduledJobType = traquent.get_doc(doctype="Scheduled Job Type", **job_type)
	...
	def fetch_email_stats(self):
		...
		self.total_outgoing_emails = traquent.db.count("Email Queue", filters)
		self.pending_emails = traquent.db.count("Email Queue", {"status": "Not Sent", **filters})
		self.failed_emails = traquent.db.count("Email Queue", {"status": "Error", **filters})
		self.unhandled_emails = traquent.db.count("Unhandled Email", filters)
		self.handled_emails = traquent.db.count(
			"Communication",
	...
	def fetch_errors(self):
		...
		filters = {"creation": (">", threshold), "modified": (">", threshold)}
		self.total_errors = traquent.db.count("Error Log", filters)

		top_errors = traquent.db.sql(
	...
	def fetch_database_details(self):
		...
		self.database = traquent.db.db_type
		self.db_storage_usage = sum(table.size for table in data)
		for row in data[:5]:
			self.append("top_db_tables", row)
		self.database_version = traquent.db.sql("select version()")[0][0]

		if traquent.db.db_type == "mariadb":
			self.bufferpool_size = traquent.db.sql("show variables like 'innodb_buffer_pool_size'")[0][1]
			self.binary_logging = traquent.db.sql("show variables like 'log_bin'")[0][1]

	@health_check("Cache")
	def fetch_cache_details(self):
		self.cache_keys = len(traquent.cache.get_keys(""))
		self.cache_memory_usage = traquent.cache.execute_command("INFO", "MEMORY").get("used_memory_human")

	@health_check("Storage")
	def fetch_storage_details(self):
		from traquent.desk.page.backups.backups import get_context
	...
	def fetch_user_stats(self):
		...
		self.total_users = traquent.db.count("User", {"enabled": 1})
		self.new_users = traquent.db.count("User", {"enabled": 1, "creation": (">", threshold)})
		self.failed_logins = traquent.db.count(
			"Activity Log",
			{
				"operation": "login",
				"status": "Failed",
		...
		self.active_sessions = traquent.db.count("Sessions")
...
def get_job_status(job_id: str | None = None):
	traquent.only_for("System Manager")
	try:
		return traquent.get_doc("RQ Job", job_id).status
	except Exception:
...
def _get_directory_size(*path):
	folder = os.path.abspath(traquent.get_site_path(*path))
