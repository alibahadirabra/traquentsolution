...
from traquent.desk.notifications import notify_mentions
from traquent.exceptions import ImplicitCommitError
...
class Comment(Document):
	...
	def validate(self):
		if not self.comment_email:
			self.comment_email = traquent.session.user
		self.content = traquent.utils.sanitize_html(self.content, always_sanitize=True)
...
def on_doctype_update():
	traquent.db.add_index("Comment", ["reference_doctype", "reference_name"])
...
def get_comments_from_parent(doc):
	...
	try:
		if is_virtual_doctype(doc.reference_doctype):
			...
			_comments = traquent.db.get_value(doc.reference_doctype, doc.reference_name, "_comments") or "[]"

	except Exception as e:
		if traquent.db.is_missing_table_or_column(e):
			_comments = "[]"
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if (
		...
		or traquent.db.get_value("DocType", reference_doctype, "issingle")
	...
	try:
		...
		if traquent.db.is_missing_column(e) and getattr(traquent.local, "request", None):
			pass
		elif traquent.db.is_data_too_long(e):
			raise traquent.DataTooLongException
		else:
			raise
	else:
		...
		# Clear route cache
		if route := traquent.get_cached_value(reference_doctype, reference_name, "route"):
			clear_cache(route)
