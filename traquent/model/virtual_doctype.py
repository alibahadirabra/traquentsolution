...
class VirtualDoctype(Protocol):
	...
	@staticmethod
	def get_list(**kwargs) -> list[traquent._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		traquent.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			traquent.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					traquent.bold(doctype), traquent.bold(m), traquent.bold(_as_str(method))
				),
