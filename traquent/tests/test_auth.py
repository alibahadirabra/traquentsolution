...
class TestAuth(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.HOST_NAME = traquent.get_site_config().host_name or get_site_url(traquent.local.site)
	...
	def tearDownClass(cls):
		traquent.delete_doc("User", cls.test_user_email, force=True)
		traquent.local.request_ip = None
		traquent.form_dict.email = None
		traquent.local.response["http_status_code"] = None

	def set_system_settings(self, k, v):
		traquent.db.set_single_value("System Settings", k, v)
