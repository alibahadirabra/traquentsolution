...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = traquent._dict({"scheduled": False, "wait": 0})
		...
		if traquent.form_dict.cmd:
			path = f"/api/method/{traquent.form_dict.cmd}"
		else:
	...
	try:
		...
		with hub.configure_scope() as scope:
			if (
				...
				set_scope(scope)
			if traquent.request:
				evt_processor = _make_wsgi_event_processor(traquent.request.environ, False)
				scope.add_event_processor(evt_processor)
				if traquent.request.is_json:
					scope.set_context("JSON Body", traquent.request.json)
				elif traquent.request.form:
					scope.set_context("Form Data", traquent.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], traquent.ValidationError | traquent.PermissionError):
					return
		...
		traquent.logger().error("Failed to capture exception", exc_info=True)
