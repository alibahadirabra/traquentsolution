...
class PrintSettings(Document):
	...
	def validate(self):
		if self.pdf_page_size == "Custom" and not (self.pdf_page_height and self.pdf_page_width):
			traquent.throw(_("Page height and width cannot be zero"))
...
def is_print_server_enabled():
	if not hasattr(traquent.local, "enable_print_server"):
		traquent.local.enable_print_server = cint(
			traquent.db.get_single_value("Print Settings", "enable_print_server")
		)

	return traquent.local.enable_print_server
