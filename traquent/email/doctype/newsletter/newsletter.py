...
from traquent.email.doctype.email_group.email_group import add_subscribers
from traquent.rate_limiter import rate_limit
from traquent.utils.safe_exec import is_job_queued
from traquent.utils.verified_command import get_signed_params, verify_request
...
class Newsletter(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		from traquent.email.doctype.newsletter_attachment.newsletter_attachment import NewsletterAttachment
		from traquent.email.doctype.newsletter_email_group.newsletter_email_group import NewsletterEmailGroup
	...
	def get_sending_status(self):
		...
		emails_queued = is_job_queued(
			job_name=traquent.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			traquent.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			traquent.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			traquent.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			traquent.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			traquent.throw(_("Newsletter must be published to send webview link in email"))

	def validate_scheduling_date(self):
		if (
			self.schedule_sending
			and traquent.utils.get_datetime(self.schedule_send) < traquent.utils.now_datetime()
		):
			traquent.throw(_("Past dates are not allowed for Scheduling."))
	...
	def queue_all(self):
		...
		self.email_sent_at = traquent.utils.now()
	...
	def send_newsletter(self, emails: list[str], test_email: bool = False):
		...
		sender = self.send_from or traquent.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message(medium="email")

		is_auto_commit_set = bool(traquent.db.auto_commit_on_many_writes)
		traquent.db.auto_commit_on_many_writes = not traquent.flags.in_test
	...
	def get_message(self, medium=None) -> str:
		...
		if self.content_type == "Markdown":
			message = traquent.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		html = traquent.render_template(message, {"doc": self.as_dict()})
	...
	def add_source(self, html: str, medium="None") -> str:
		...
		for link in links:
			...
			if href and not href.startswith("#"):
				if not traquent.utils.is_site_link(href):
					continue
				new_href = traquent.utils.add_trackers_to_url(
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or traquent.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	doc = traquent.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):
	...
	email_confirmation_template = traquent.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = traquent.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = traquent.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=None):
	...
	try:
		group = traquent.get_doc("Email Group", email_group)
	except traquent.DoesNotExistError:
		group = traquent.get_doc({"doctype": "Email Group", "title": email_group}).insert(
	...
	if welcome_url:
		...
		traquent.local.response["location"] = welcome_url
	else:
...
def send_scheduled_email():
	...
	scheduled_newsletter = traquent.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", traquent.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = traquent.get_doc("Newsletter", newsletter_name)
			...
			traquent.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
...
def newsletter_email_read(recipient_email=None, reference_doctype=None, reference_name=None):
	...
	verify_request()
	try:
		doc = traquent.get_cached_doc("Newsletter", reference_name)
		if doc.add_viewed(recipient_email, force=True, unique_views=True):
			newsletter = traquent.qb.DocType("Newsletter")
			(
				traquent.qb.update(newsletter)
...
def get_default_email_group():
	return _("Website", lang=traquent.db.get_default("language"))
