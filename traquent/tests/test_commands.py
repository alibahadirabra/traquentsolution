...
import traquent
import traquent.commands.scheduler
import traquent.commands.site
import traquent.commands.utils
import traquent.recorder
from traquent.installer import add_to_installed_apps, remove_app
...
from traquent.utils import add_to_date, get_bench_path, get_bench_relative_path, now
from traquent.utils.backups import BackupGenerator, fetch_latest_backups
from traquent.utils.jinja_globals import bundled_asset
from traquent.utils.scheduler import enable_scheduler, is_scheduler_inactive

_result: Result | None = None
TEST_SITE = "commands-site-O4PN2QK.test"  # added random string tag to avoid collisions
CLI_CONTEXT = traquent._dict(sites=[TEST_SITE])
...
def missing_in_backup(doctypes: list, file: os.PathLike) -> list:
	...
	"""
	predicate = 'COPY public."tab{}"' if traquent.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
@contextmanager
def maintain_locals():
	pre_site = traquent.local.site
	pre_flags = traquent.local.flags.copy()
	pre_db = traquent.local.db

	try:
		yield
	finally:
		post_site = getattr(traquent.local, "site", None)
		if not post_site or post_site != pre_site:
			traquent.init(pre_site)
			traquent.local.db = pre_db
			traquent.local.flags.update(pre_flags)
...
class BaseTestCommands(IntegrationTestCase):
	...
	def setup_test_site(cls):
		cmd_config = {
			"test_site": TEST_SITE,
			"admin_password": traquent.conf.admin_password,
			"db_type": traquent.conf.db_type,
			"db_root_username": traquent.conf.root_login,
			"db_root_password": traquent.conf.root_password,
		}
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout, traquent.bold(text="DocType"))
	...
	def test_restore(self):
		...
		global_config = {
			"admin_password": traquent.conf.admin_password,
			"root_login": traquent.conf.root_login,
			"root_password": traquent.conf.root_password,
			"db_type": traquent.conf.db_type,
		}
		...
		with self.switch_site(TEST_SITE):
			public_file = traquent.new_doc(
				"File", file_name=f"test_{traquent.generate_hash()}", content=traquent.generate_hash()
			).insert()
			private_file = traquent.new_doc(
				"File", file_name=f"test_{traquent.generate_hash()}", content=traquent.generate_hash()
			).insert()
		...
		with self.switch_site(TEST_SITE):
			...
			traquent.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		...
		with self.switch_site(TEST_SITE):
			self.assertTrue(traquent.db.table_exists("ToDo", cached=False))
	...
	def test_partial_restore(self):
			...
			traquent.get_doc(
				{
					...
					"description": traquent.mock("paragraph"),
				}
			).insert()
		traquent.db.commit()
		todo_count = traquent.db.count("ToDo")
		...
		self.assertEqual(self.returncode, 0)
		self.assertEqual(traquent.db.count("ToDo"), todo_count)

	def test_recorder(self):
		traquent.recorder.stop()

		self.execute("bench --site {site} start-recording")
		traquent.local.cache = {}
		self.assertEqual(traquent.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		traquent.local.cache = {}
		self.assertEqual(traquent.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = traquent.get_single("Installed Applications").installed_applications
		if doctype:
	...
	def test_set_password(self):
		from traquent.utils.password import check_password
		...
		original_password = traquent.conf.admin_password or "admin"
	...
	@skipIf(
		not (traquent.conf.root_password and traquent.conf.admin_password and traquent.conf.db_type == "mariadb"),
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--admin-password {traquent.conf.admin_password} "
			f"--db-root-username {traquent.conf.root_login} "
			f"--db-root-password {traquent.conf.root_password} "
	...
	def test_different_db_username(self):
		...
		kwargs = {
			...
			"db_root_username": traquent.conf.root_login,
			"db_root_password": traquent.conf.root_password or "",
		}
		self.execute(
	...
	def test_existing_db_username(self):
		...
		if traquent.conf.db_type == "mariadb":
			from traquent.database.mariadb.setup_db import get_root_connection

			root_conn = get_root_connection()
			root_conn.sql(f"CREATE USER '{user}'@'localhost'")
		else:
			from traquent.database.postgres.setup_db import get_root_connection
...
class TestBackups(BaseTestCommands):
	...
	site_backup_path = traquent.utils.get_site_path("private", "backups")
	...
	@skipIf(
		not (traquent.conf.db_type == "mariadb"),
		"Only for MariaDB",
	)
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			traquent.conf.db_name,
			traquent.conf.db_name,
			traquent.conf.db_password + "INCORRECT PASSWORD",
			db_socket=traquent.conf.db_socket,
			db_host=traquent.conf.db_host,
			db_port=traquent.conf.db_port,
			db_type=traquent.conf.db_type,
		)
	...
	def test_clear_log_table(self):
		d = traquent.get_doc(doctype="Error Log", title="Something").insert()
		d.db_set("creation", "2010-01-01", update_modified=False)
		traquent.db.commit()

		tables_before = traquent.db.get_tables(cached=False)

		self.execute("bench --site {site} clear-log-table --days=30 --doctype='Error Log'")
		self.assertEqual(self.returncode, 0)
		traquent.db.commit()

		self.assertFalse(traquent.db.exists("Error Log", d.name))
		tables_after = traquent.db.get_tables(cached=False)
...
class TestRemoveApp(IntegrationTestCase):
	def test_delete_modules(self):
		from traquent.installer import (
			_delete_doctypes,
			_delete_modules,
		...
		self.assertFalse(traquent.db.exists("Module Def", test_module.module_name))
		self.assertFalse(traquent.db.exists("DocType", module_def_linked_doctype.name))
...
class TestSiteMigration(BaseTestCommands):
	def test_migrate_cli(self):
		with cli(traquent.commands.site.migrate) as result:
...
class TestAddNewUser(BaseTestCommands):
	def test_create_user(self):
		...
		self.assertEqual(self.returncode, 0)
		user = traquent.get_doc("User", "test@gmail.com")
...
class TestBenchBuild(BaseTestCommands):
	def test_build_assets_size_check(self):
		...
		for chunk in default_bundle:
			abs_path = Path.cwd() / traquent.local.sites_path / bundled_asset(chunk)[1:]
...
class TestDBUtils(BaseTestCommands):
	def test_db_add_index(self):
		...
		traquent.db.rollback()
		index_name = traquent.db.get_index_name((field,))
		self.assertTrue(traquent.db.has_index("tabUser", index_name))
		meta = traquent.get_meta("User", cached=False)
...
class TestSchedulerUtils(BaseTestCommands):
	...
	def test_ready_for_migrate(self):
		with cli(traquent.commands.scheduler.ready_for_migration) as result:
			self.assertEqual(result.exit_code, 0)
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		...
		parsed_output = traquent.parse_json(self.stdout)
		self.assertEqual(self.returncode, 0)
