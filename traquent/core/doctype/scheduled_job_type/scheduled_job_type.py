...
from traquent.utils import get_datetime, now_datetime
from traquent.utils.background_jobs import enqueue, is_job_enqueued
...
class ScheduledJobType(Document):
	...
	def validate(self):
		...
		if self.frequency == "Cron":
			if not self.cron_format:
				traquent.throw(_("Cron format is required for job types with Cron frequency."))
			try:
	...
	def enqueue(self, force=False) -> bool:
		...
		if self.is_event_due() or force:
			if not self.is_job_in_queue():
				...
				traquent.logger("scheduler").error(
					f"Skipped queueing {self.method} because it was found in queue for {traquent.local.site}"
				)
	...
	def get_next_execution(self):
		CRON_MAP = {
			...
			"Hourly": "0 * * * *",
			"Hourly Long": "0 * * * *",
			"All": f"*/{(traquent.get_conf().scheduler_interval or 240) // 60} * * * *",
		}
	...
	def execute(self):
		if traquent.job:
			traquent.job.frequency = self.frequency
			traquent.job.cron_format = self.cron_format

		self.scheduler_log = None
		try:
			self.log_status("Start")
			if self.server_script:
				script_name = traquent.db.get_value("Server Script", self.server_script)
				if script_name:
					traquent.get_doc("Server Script", script_name).execute_scheduled_method()
			else:
				traquent.get_attr(self.method)()
			traquent.db.commit()
	...
	def log_status(self, status):
		# log file
		traquent.logger("scheduler").info(f"Scheduled Job {status}: {self.method} for {traquent.local.site}")
	...
	def update_scheduler_log(self, status):
		...
		if not self.scheduler_log:
			self.scheduler_log = traquent.get_doc(
		...
		if status == "Failed":
			self.scheduler_log.db_set("details", traquent.get_traceback(with_context=True))
		if status == "Start":
			self.db_set("last_execution", now_datetime(), update_modified=False)
		traquent.db.commit()
...
def execute_event(doc: str):
	...
	doc = json.loads(doc)
	traquent.get_doc("Scheduled Job Type", doc.get("name")).enqueue(force=True)
	return doc
...
def run_scheduled_job(scheduled_job_type: str, job_type: str | None = None):
	...
	try:
		traquent.get_doc("Scheduled Job Type", scheduled_job_type).execute()
	except Exception:
		print(traquent.get_traceback())


def sync_jobs(hooks: dict | None = None):
	traquent.reload_doc("core", "doctype", "scheduled_job_type")
	scheduler_events = hooks or traquent.get_hooks("scheduler_events")
...
def insert_single_event(frequency: str, event: str, cron_format: str | None = None):
	...
	if not traquent.db.exists("Scheduled Job Type", {"method": event, "frequency": frequency, **cron_expr}):
...
def clear_events(scheduler_events: dict):
	def event_exists(event) -> bool:
		...
		freq = traquent.scrub(event.frequency)
		if freq == "cron":
			return event.method in scheduler_events.get(freq, {}).get(event.cron_format, [])
		else:
			return event.method in scheduler_events.get(freq, [])

	for event in traquent.get_all("Scheduled Job Type", fields=["*"]):
		if not event_exists(event):
			traquent.delete_doc("Scheduled Job Type", event.name)


def on_doctype_update():
	traquent.db.add_unique(
