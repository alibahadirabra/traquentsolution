...
class SignupDisabledError(traquent.PermissionError):
	...


def get_oauth2_providers() -> dict[str, dict]:
	out = {}
	providers = traquent.get_all("Social Login Key", fields=["*"])
...
def get_oauth_keys(provider: str) -> dict[str, str]:
	...
	if keys := traquent.conf.get(f"{provider}_login"):
		return {"client_id": keys["client_id"], "client_secret": keys["client_secret"]}

	return {
		"client_id": traquent.db.get_value("Social Login Key", provider, "client_id"),
...
def get_oauth2_authorize_url(provider: str, redirect_to: str) -> str:
	...
	state = {
		"site": traquent.utils.get_url(),
		"token": traquent.generate_hash(),
...
def get_redirect_uri(provider: str) -> str:
	keys = traquent.conf.get(f"{provider}_login")
	...
	return traquent.utils.get_url(redirect_uri)
...
def get_info_via_oauth(provider: str, code: str, decoder: Callable | None = None, id_token: bool = False):
	...
	if not (info.get("email_verified") or info.get("email")):
		traquent.throw(_("Email not verified with {0}").format(provider.title()))
...
def login_oauth_user(
	...
	*,
	...
	if not (state and state["token"]):
		traquent.respond_as_web_page(_("Invalid Request"), _("Token is missing"), http_status_code=417)
		return
	...
	try:
		...
		return traquent.respond_as_web_page(
			"Signup is Disabled",
	...
	traquent.local.login_manager.user = user
	traquent.local.login_manager.post_login()

	# because of a GET request!
	traquent.db.commit()

	if traquent.utils.cint(generate_login_token):
		login_token = traquent.generate_hash(length=32)
		traquent.cache.set_value(f"login_token:{login_token}", traquent.local.session.sid, expires_in_sec=120)

		traquent.response["login_token"] = login_token

	else:
		redirect_to = state.get("redirect_to")
		redirect_post_login(
			desk_user=traquent.local.response.get("message") == "Logged In",
			redirect_to=redirect_to,
			provider=provider,
		)
...
def get_user_record(user: str, data: dict, provider: str) -> "User":
	from traquent.integrations.doctype.social_login_key.social_login_key import provider_allows_signup

	try:
		return traquent.get_doc("User", user)
	except traquent.DoesNotExistError:
		if not provider_allows_signup(provider):
			raise SignupDisabledError

	user: "User" = traquent.new_doc("User")
	...
	user.update(
		{
			...
			"gender": gender,
			"enabled": 1,
			"new_password": traquent.generate_hash(),
...
def update_oauth_user(user: str, data: dict, provider: str):
	...
	if not user.enabled:
		traquent.respond_as_web_page(_("Not Allowed"), _("User {0} is disabled").format(user.email))
		return False
	...
	if not user.get_social_login_userid(provider):
		...
		match provider:
			...
			case _:
				user_id_property = (
					traquent.db.get_value("Social Login Key", provider, "user_id_property") or "sub"
				)
	...
	if update_user_record:
		...
		if default_role := traquent.db.get_single_value("Portal Settings", "default_role"):
...
def redirect_post_login(desk_user: bool, redirect_to: str | None = None, provider: str | None = None):
	...
	if not redirect_to:
		...
		redirect_to = traquent.utils.get_url(desk_uri if desk_user else "/me")

	traquent.local.response["location"] = redirect_to
