...
from traquent.modules import get_module_path, scrub_dt_dn
from traquent.query_builder import DocType
from traquent.utils import get_datetime, now
...
def import_file_by_path(
	...
	if docs:
		...
		for doc in docs:
			...
			db_modified_timestamp = traquent.db.get_value(doc["doctype"], doc["name"], "modified")
			...
			if not force and db_modified_timestamp:
				...
				if doc["doctype"] == "DocType":
					try:
						stored_hash = traquent.db.get_value(doc["doctype"], doc["name"], "migration_hash")
					except Exception:
						pass
			...
			if doc["doctype"] == "DocType":
				...
				traquent.qb.update(doctype_table).set(doctype_table.migration_hash, calculated_hash).where(
...
def update_modified(original_modified, doc):
	...
	if doc["doctype"] == doc["name"] and doc["name"] != "DocType":
		...
		traquent.qb.update(singles_table).set(singles_table.value, original_modified).where(
		...
		traquent.qb.update(doctype_table).set(doctype_table.modified, original_modified).where(
			doctype_table.name == doc["name"]
		).run()
...
def import_doc(
	...
	if traquent.db.exists(doc.doctype, doc.name):
...
def delete_old_doc(doc, reset_permissions):
	ignore = []
	old_doc = traquent.get_doc(doc.doctype, doc.name)
	...
	# delete old
	traquent.delete_doc(doc.doctype, doc.name, force=1, ignore_doctypes=ignore, for_reload=True)
