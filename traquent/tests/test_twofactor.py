...
import traquent
from traquent.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
from traquent.utils import cint, set_request


class TestTwoFactor(IntegrationTestCase):
	def setUp(self):
		self.http_requests = create_http_request()
		self.login_manager = traquent.local.login_manager
	...
	def tearDown(self):
		traquent.local.response["verification"] = None
		traquent.local.response["tmp_id"] = None
		disable_2fa()
		traquent.clear_cache(user=self.user)
	...
	def test_authenticate_for_2factor(self):
		...
		for k in ["_usr", "_pwd", "_otp_secret"]:
			self.assertTrue(traquent.cache.get(f"{tmp_id}{k}"), f"{k} not available")
	...
	def test_two_factor_is_enabled(self):
		...
		# Scenario 3
		enable_2fa()
		user = traquent.get_doc("User", self.user)
		user.restrict_ip = traquent.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_confirm_otp_token(self):
		...
		traquent.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = traquent.local.response["tmp_id"]
		otp = "wrongotp"
		with self.assertRaises(traquent.AuthenticationError):
		...
		traquent.flags.otp_expiry = None
		if traquent.flags.tests_verbose:
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(traquent.SecurityException):
			confirm_otp_token(self.login_manager, otp=otp, tmp_id=tmp_id)
...
def create_http_request():
	...
	enable_2fa()
	traquent.form_dict["usr"] = "test@example.com"
	traquent.form_dict["pwd"] = "Eastern_43A1W"
	traquent.local.form_dict["cmd"] = "login"
	return HTTPRequest()


def enable_2fa(bypass_two_factor_auth=0, bypass_restrict_ip_check=0):
	"""Enable Two factor in system settings."""
	system_settings = traquent.get_doc("System Settings")
...
def toggle_2fa_all_role(state=None):
	...
	all_role = traquent.get_doc("Role", "All")
