...
from traquent.utils import cint, get_link_to_form
...
def cache_source(function):
	...
	def wrapper(*args, **kwargs):
		if kwargs.get("chart_name"):
			chart = traquent.get_doc("Dashboard Chart", kwargs.get("chart_name"))
		else:
			chart = kwargs.get("chart")
		no_cache = kwargs.get("no_cache")
		if no_cache:
			return function(chart=chart, no_cache=no_cache)
		chart_name = traquent.parse_json(chart).name
		...
		if cint(kwargs.get("refresh")):
			...
			cached_results = traquent.cache.get_value(cache_key)
			if cached_results:
				results = traquent.parse_json(traquent.safe_decode(cached_results))
			else:
				results = generate_and_cache_results(kwargs, function, cache_key, chart)
		return results
...
def generate_and_cache_results(args, function, cache_key, chart):
	try:
		...
		if str(e) == "'NoneType' object is not iterable":
			...
			# ref: https://github.com/traquent/traquent/pull/9403
			traquent.throw(
	...
	if not traquent.flags.read_only:
		traquent.db.set_value(
			"Dashboard Chart", args.chart_name, "last_synced_on", traquent.utils.now(), update_modified=False
		)
	return results
...
def get_dashboards_with_link(docname, doctype):
	...
	if doctype == "Dashboard Chart":
		links = traquent.get_all("Dashboard Chart Link", fields=["parent"], filters={"chart": docname})
	elif doctype == "Number Card":
		links = traquent.get_all("Number Card Link", fields=["parent"], filters={"card": docname})
...
def sync_dashboards(app=None):
	...
	apps = [app] if app else traquent.get_installed_apps()
...
def make_records_in_module(app, module):
	dashboards_path = traquent.get_module_path(module, f"{module}_dashboard")
	charts_path = traquent.get_module_path(module, "dashboard chart")
	cards_path = traquent.get_module_path(module, "number card")
