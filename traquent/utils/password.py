...
from traquent.query_builder import Table
from traquent.utils import cstr, encode
...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	...
	try:
		...
		if traquent.db.is_data_too_long(e):
			traquent.throw(_("Most probably your password is too long."), exc=e)
		raise e


def remove_encrypted_password(doctype, name, fieldname="password"):
	traquent.db.delete("__Auth", {"doctype": doctype, "name": name, "fieldname": fieldname})
...
def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise traquent.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	traquent.cache.hdel("login_failed_count", user)
...
def delete_all_passwords_for(doctype, name):
	try:
		traquent.db.delete("__Auth", {"doctype": doctype, "name": name})
	except Exception as e:
		if not traquent.db.is_missing_column(e):
			raise


def rename_password(doctype, old_name, new_name):
	# NOTE: fieldname is not considered, since the document is renamed
	traquent.qb.update(Auth).set(Auth.name, new_name).where(
...
def encrypt(txt, encryption_key=None):
	...
	try:
		...
		traquent.throw(_("Encryption key is in invalid format!"))
...
def get_encryption_key():
	if "encryption_key" not in traquent.local.conf:
		from traquent.installer import update_site_config

		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		traquent.local.conf.encryption_key = encryption_key

	return traquent.local.conf.encryption_key


def get_password_reset_limit():
	return traquent.get_system_settings("password_reset_limit") or 3
