...
from traquent import _, get_module_path
from traquent.desk.doctype.tag.tag import delete_tags_for_document
from traquent.model.docstatus import DocStatus
from traquent.model.dynamic_links import get_dynamic_link_map
from traquent.model.naming import revert_series_if_last
from traquent.model.utils import is_virtual_doctype
from traquent.utils.file_manager import remove_all
from traquent.utils.global_search import delete_for_document
from traquent.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = traquent.form_dict.get("dt")
		name = traquent.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			traquent.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = traquent.get_doc(doctype, name)
				if not (doc.custom or traquent.conf.developer_mode or traquent.flags.in_patch or force):
					traquent.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				traquent.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", traquent.model.table_fields)}
				)
				traquent.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				traquent.conf.developer_mode
				and not doc.custom
				and not (
					for_reload
			...
			try:
				traquent.db.get_value(doctype, name, for_update=True, wait=False)
			except traquent.QueryTimeoutError:
				traquent.throw(
					_(
						"This document can not be deleted right now as it's being modified by another user. Please try again after some time."
					),
					exc=traquent.QueryTimeoutError,
				)
			doc = traquent.get_doc(doctype, name)
			...
			if not for_reload:
				...
				if not force:
					...
					except traquent.LinkExistsError as e:
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		traquent.db.delete("Singles", {"doctype": name})
	else:
		traquent.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if traquent.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = traquent.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", traquent.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		traquent.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and traquent.session.user != "Administrator"
		...
		traquent.msgprint(
			...
			raise_exception=traquent.PermissionError,
		)
	...
	if doc.meta.is_submittable and doc.docstatus.is_submitted():
		traquent.msgprint(
			_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(
				_(doc.doctype),
				doc.name,
				"<a href='https://docs.traquent.com//docs/user/manual/en/setting-up/articles/delete-submitted-document' target='_blank'>",
				"</a>",
			),
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from traquent.model.rename_doc import get_link_fields
	...
	if method == "Delete":
		ignored_doctypes.update(traquent.get_hooks("ignore_links_on_delete"))
	...
	for lf in link_fields:
		...
		try:
			meta = traquent.get_meta(link_dt)
		except traquent.DoesNotExistError:
		...
		if issingle:
			if traquent.db.get_single_value(link_dt, link_field) == doc.name:
		...
		for item in traquent.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	...
	for df in get_dynamic_link_map().get(doc.doctype, []):
		...
		if df.parent in traquent.get_hooks("ignore_links_on_delete") or (
		...
		if meta.issingle:
			...
			for refdoc in traquent.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in traquent.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	traquent.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", traquent.scrub(doctype))
