...
from traquent.utils import cint, today
...
class SystemSettings(Document):
	...
	def validate(self):
		from traquent.twofactor import toggle_two_factor_auth
		...
		if enable_password_policy and minimum_password_score <= 0:
			traquent.throw(_("Please select Minimum Password Score"))
		...
		if self.session_expiry:
			...
			if len(parts) != 2 or not (cint(parts[0]) or cint(parts[1])):
				traquent.throw(_("Session Expiry must be in format {0}").format("hh:mm"))

		if self.enable_two_factor_auth:
			if self.two_factor_method == "SMS":
				if not traquent.db.get_single_value("SMS Settings", "sms_gateway_url"):
					traquent.throw(
		...
		traquent.flags.update_last_reset_password_date = False
		if self.force_user_to_reset_password and not cint(
			traquent.db.get_single_value("System Settings", "force_user_to_reset_password")
		):
			traquent.flags.update_last_reset_password_date = True
	...
	def validate_user_pass_login(self):
		...
		social_login_enabled = traquent.db.exists("Social Login Key", {"enable_social_login": 1})
		ldap_enabled = traquent.db.get_single_value("LDAP Settings", "enabled")
		login_with_email_link_enabled = traquent.db.get_single_value("System Settings", "login_with_email_link")
	...
	def validate_backup_limit(self):
		if not self.backup_limit or self.backup_limit < 1:
			traquent.msgprint(_("Number of backups must be greater than zero."), alert=True)
			self.backup_limit = 1
	...
	def on_update(self):
		...
		traquent.cache.delete_value("system_settings")
		traquent.cache.delete_value("time_zone")

		if traquent.flags.update_last_reset_password_date:
			update_last_reset_password_date()

	def set_defaults(self):
		from traquent.translate import set_default_language

		for df in self.meta.get("fields"):
			if df.fieldtype not in no_value_fields and self.has_value_changed(df.fieldname):
				traquent.db.set_default(df.fieldname, self.get(df.fieldname))
...
def load():
	...
	if "System Manager" not in traquent.get_roles():
		traquent.throw(_("Not permitted"), traquent.PermissionError)

	all_defaults = traquent.db.get_defaults()
	defaults = {}

	for df in traquent.get_meta("System Settings").get("fields"):
