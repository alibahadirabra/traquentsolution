...
import traquent
from traquent.exceptions import MandatoryError, ValidationError
...
class LDAP_TestCase:
	...
	def clean_test_users():
		with contextlib.suppress(Exception):
			traquent.get_doc("User", "posix.user1@unit.testing").delete()
		with contextlib.suppress(Exception):
			traquent.get_doc("User", "posix.user2@unit.testing").delete()
		with contextlib.suppress(Exception):
			traquent.get_doc("User", "website_ldap_user@test.com").delete()

	@classmethod
	def setUpClass(cls):
		cls.clean_test_users()
		# Save user data for restoration in tearDownClass()
		cls.user_ldap_settings = traquent.get_doc("LDAP Settings")
		...
		user = traquent.get_doc(cls.user1doc)
		...
		user = traquent.get_doc(cls.user2doc)
		user.insert(ignore_permissions=True)
	...
	def tearDownClass(cls):
		with contextlib.suppress(Exception):
			traquent.get_doc("LDAP Settings").delete()
	...
	def test_update_user_fields(self):
		...
		test_user = traquent.get_doc("User", test_user_data["email"])
		self.test_class.update_user_fields(test_user, test_user_data)
		updated_user = traquent.get_doc("User", test_user_data["email"])
		...
		self.assertIn(self.test_class.default_role, traquent.get_roles(updated_user.name))
	...
	def test_create_website_user(self):
		...
		new_user = traquent.get_doc("User", new_test_user_data["email"])
	...
	def test_sync_roles(self):
		...
		role_to_group_map = {
			self.doc["ldap_groups"][0]["traquent_role"]: self.doc["ldap_groups"][0]["ldap_group"],
			self.doc["ldap_groups"][1]["traquent_role"]: self.doc["ldap_groups"][1]["ldap_group"],
			self.doc["ldap_groups"][2]["traquent_role"]: self.doc["ldap_groups"][2]["ldap_group"],
		...
		traquent.get_doc("User", "posix.user1@unit.testing").delete()
		user = traquent.get_doc(self.user1doc)
		user.insert(ignore_permissions=True)

		for test_user in test_user_data:
			test_user_doc = traquent.get_doc("User", f"{test_user}@unit.testing")
			test_user_roles = traquent.get_roles(f"{test_user}@unit.testing")
			...
			updated_user_roles = traquent.get_roles(f"{test_user}@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		traquent.get_doc("User", f"{test_user}@unit.testing").delete()

		with self.assertRaises(
			traquent.exceptions.DoesNotExistError
	...
	def test_authenticate(self):
		...
		for username, password in enumerate(invalid_users):
			with self.assertRaises(traquent.exceptions.ValidationError) as display_massage:
	...
	DOCUMENT_GROUP_MAPPINGS: typing.ClassVar[list] = [
		{
			...
			"traquent_role": "System Manager",
		},
		{"doctype": "LDAP Group Mapping", "ldap_group": "Users", "traquent_role": "Blogger"},
		{"doctype": "LDAP Group Mapping", "ldap_group": "Group3", "traquent_role": "Accounts User"},
	]
...
class Test_ActiveDirectory(LDAP_TestCase, TestCase):
	...
	DOCUMENT_GROUP_MAPPINGS: typing.ClassVar[list] = [
		...
		{"doctype": "LDAP Group Mapping", "ldap_group": "Domain Users", "traquent_role": "Blogger"},
		{
			"doctype": "LDAP Group Mapping",
			"ldap_group": "Enterprise Administrators",
			"traquent_role": "Accounts User",
		},
	]
