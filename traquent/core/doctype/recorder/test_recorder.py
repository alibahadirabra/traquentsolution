...
from traquent.core.doctype.recorder.recorder import _optimize_query, serialize_request
...
class TestRecorder(IntegrationTestCase):
	...
	def test_recorder_list(self):
		traquent.get_all("User")  # trigger one query
		self.stop_recorder()
		requests = traquent.get_all("Recorder")
		self.assertGreaterEqual(len(requests), 1)
		request = traquent.get_doc("Recorder", requests[0].name)
	...
	def test_recorder_list_filters(self):
		...
		traquent.qb.from_(user).select("name").run()
		self.stop_recorder()
		...
		requests = traquent.get_list(
			"Recorder", filters={"path": ("like", "/api/method/ping"), "number_of_queries": 1}
		)
		self.assertGreaterEqual(len(requests), 1)
		requests = traquent.get_list("Recorder", filters={"path": ("like", "/api/method/test")})
		self.assertEqual(len(requests), 0)

		requests = traquent.get_list("Recorder", filters={"method": "GET"})
		self.assertGreaterEqual(len(requests), 1)
		requests = traquent.get_list("Recorder", filters={"method": "POST"})
		self.assertEqual(len(requests), 0)

		requests = traquent.get_list("Recorder", order_by="path desc")
