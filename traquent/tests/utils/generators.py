...
from traquent.modules import get_doctype_module, get_module_path, load_doctype_module

if TYPE_CHECKING:
	from traquent.model.document import Document

logger = logging.getLogger(__name__)
testing_logger = logging.getLogger("traquent.testing.generators")
...
def get_modules(doctype) -> (str, ModuleType):
	...
	module = traquent.db.get_value("DocType", doctype, "module")
	try:
...
def get_missing_records_doctypes(doctype, visited=None) -> list[str]:
	...
	for df in meta.get_table_fields():
		link_fields.extend(traquent.get_meta(df.options).get_link_fields())
...
def get_missing_records_module_overrides(module) -> [list, list]:
	...
	if hasattr(module, "test_dependencies"):
		...
		deprecation_warning(
			"2024-10-09",
			"v17",
			"""test_dependencies was clarified to EXTRA_TEST_RECORD_DEPENDENCIES; migration script: https://github.com/traquent/traquent/pull/28060""",
		)
	...
	if hasattr(module, "test_ignore"):
		...
		deprecation_warning(
			"2024-10-09",
			"v17",
			"""test_ignore was clarified to IGNORE_TEST_RECORD_DEPENDENCIES; migration script: https://github.com/traquent/traquent/pull/28060""",
		)
...
def load_test_records_for(index_doctype) -> dict[str, Any] | list:
	module_path = get_module_path(get_doctype_module(index_doctype), "doctype", traquent.scrub(index_doctype))

	json_path = os.path.join(module_path, "test_records.json")
	if os.path.exists(json_path):
		if not traquent.flags.deprecation_dumpster_invoked:
			from traquent.deprecation_dumpster import deprecation_warning

			deprecation_warning(
				"2024-10-09",
				"v18",
				"Use TOML files for test records; migration script: https://github.com/traquent/traquent/pull/28065",
			)
...
def _generate_all_records_towards(
	...
	visited = set(traquent.local.test_objects.keys())
...
def _generate_records_for(
	...
	if hasattr(test_module, "_make_test_records"):
		...
		if not test_records:
			...
			traquent.local.test_objects[index_doctype] = []  # avoid noisy retries on multiple invocations
...
def _sync_records(
	...
	def _load(do_create=True):
		...
		for _sub_doctype, records in test_records.items():
			for record in records:
				...
				traquent.local.test_objects[index_doctype].append(MappingProxyType(record))
	...
	if index_doctype in test_record_manager_instance.get():
		...
		if reset:
			...
			traquent.local.test_objects[index_doctype] = []
			...
			if index_doctype not in traquent.local.test_objects:
...
def _try_create(record, reset=False, commit=False) -> tuple["Document", bool]:
	...
	if not reset:
		traquent.db.savepoint("creating_test_record")

	d = traquent.copy_doc(record)
	...
	if traquent.db.exists(d.doctype, d.name) and not reset:
		traquent.db.rollback(save_point="creating_test_record")
		# do not create test records, if already exists
		return traquent.get_doc(d.doctype, d.name), False
...
class TestRecordManager:
	def __init__(self):
		...
		self.log_file = Path(traquent.get_site_path(PERSISTENT_TEST_LOG_FILE))
		self._log = None
	...
	def _append_to_log(self, index_doctype, records: list["Document"]):
		...
		with self.log_file.open("a") as f:
			f.write(traquent.as_json(entry, indent=None) + "\n")
			...
			with self.log_file.open() as f:
				for line in f:
					...
					log.setdefault(index_doctype, []).extend(
						traquent.get_doc(r["doctype"], r["name"]) for r in records
					)
		return log
...
def _after_install_clear_test_log():
	log_file_path = traquent.get_site_path(PERSISTENT_TEST_LOG_FILE)
