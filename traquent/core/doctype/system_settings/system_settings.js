traquent.ui.form.on("System Settings", {
	refresh: function (frm) {
		traquent.call({
			method: "traquent.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				traquent.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", traquent.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					traquent.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					traquent.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, traquent.boot.sysdefaults.language, traquent.boot.user.language],
			[frm.doc.rounding_method, traquent.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == traquent.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		traquent.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", traquent.boot.sysdefaults.rounding_method);
			}
		);
	},
