traquent.ui.form.ControlCurrency = class ControlCurrency extends traquent.ui.form.ControlFloat {
	...
	get_precision() {
		...
		if (typeof this.df.precision != "number" && !this.df.precision) {
			if (traquent.boot.sysdefaults.currency_precision) {
				this.df.precision = traquent.boot.sysdefaults.currency_precision;
			} else {
