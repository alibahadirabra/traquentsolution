...
EXCLUDE_EXCEPTIONS = (
	...
	traquent.CSRFTokenError,  # CSRF covers OAuth too
	traquent.SecurityException,
	traquent.InReadOnlyMode,
)
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from traquent.utils.sentry import capture_exception
	...
	traceback = traquent.as_unicode(traceback or traquent.get_traceback(with_context=True))
	...
	if traquent.flags.read_only or defer_insert:
...
def log_error_snapshot(exception: Exception):
	...
	logger = traquent.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				traquent.throw(error_message, error_type or Exception)
			return response
