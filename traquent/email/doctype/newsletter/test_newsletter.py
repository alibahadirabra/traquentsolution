...
import traquent
from traquent.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from traquent.email.doctype.newsletter.newsletter import (
	Newsletter,
...
from traquent.utils import add_days, getdate
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			traquent.delete_doc("Newsletter", newsletter)
			traquent.db.delete("Newsletter Email Group", {"parent": newsletter})
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not traquent.db.exists("Email Group", "_Test Email Group"):
			traquent.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				traquent.db.set_value(doctype, email_filters, "unsubscribed", 0)
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = traquent.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			traquent.delete_doc(doctype, similar_newsletter)

		newsletter = traquent.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
		...
		attached_files = traquent.get_all(
			"File",
			{
...
class TestNewsletter(TestNewsletterMixin, IntegrationTestCase):
	def test_send(self):
		...
		email_queue_list = [traquent.get_doc("Email Queue", e.name) for e in traquent.get_all("Email Queue")]
	...
	def test_unsubscribe(self):
		...
		group = traquent.get_all("Newsletter Email Group", filters={"parent": name}, fields=["email_group"])
	...
	def test_retry_partially_sent_newsletter(self):
		...
		# retry
		newsletter.send_emails()
		self.assertEqual(traquent.db.count("Email Queue"), 4)
