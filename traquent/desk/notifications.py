...
def get_notifications():
	...
	if traquent.flags.in_install or not traquent.get_system_settings("setup_complete"):
		return out
	...
	for name in groups:
		count = traquent.cache.hget("notification_count:" + name, traquent.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = traquent.get_user().get_can_read()
	open_count_doctype = {}
		...
		if d in can_read:
			...
			if d in notification_count:
				...
				# traquent.msgprint("Permission Error in notifications for {0}".format(d))
					...
					traquent.cache.hset("notification_count:" + d, traquent.session.user, result)
...
def clear_notifications(user=None):
	...
	if user:
		traquent.cache.hdel_names([f"notification_count:{name}" for name in groups], user)
	else:
		traquent.cache.delete_value([f"notification_count:{name}" for name in groups])


def clear_notification_config(user):
	traquent.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	traquent.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(traquent.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	...
	return traquent.cache.hget("notification_config", user, _get)
...
def get_open_count(doctype: str, name: str, items=None):
	...
	if traquent.flags.in_migrate or traquent.flags.in_install:
		return {"count": []}
	...
	if not meta.custom:
		module = traquent.get_meta_module(doctype)
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(traquent.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			traquent.bold(sender_fullname), traquent.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = traquent.get_cached_doc("User Group", mention["data-id"])
