...
def setup_database(force, verbose=None, mariadb_user_host_login_scope=None):
	...
	if traquent.conf.db_type == "postgres":
		import traquent.database.postgres.setup_db

		return traquent.database.postgres.setup_db.setup_database()
	else:
		import traquent.database.mariadb.setup_db

		return traquent.database.mariadb.setup_db.setup_database(force, verbose, mariadb_user_host_login_scope)
...
def bootstrap_database(verbose=None, source_sql=None):
	...
	if traquent.conf.db_type == "postgres":
		...
		return traquent.database.postgres.setup_db.bootstrap_database(verbose, source_sql)
	else:
		import traquent.database.mariadb.setup_db

		return traquent.database.mariadb.setup_db.bootstrap_database(verbose, source_sql)
...
def drop_user_and_database(db_name, db_user):
	...
	if traquent.conf.db_type == "postgres":
		...
		return traquent.database.postgres.setup_db.drop_user_and_database(db_name, db_user)
	else:
		import traquent.database.mariadb.setup_db

		return traquent.database.mariadb.setup_db.drop_user_and_database(db_name, db_user)
...
def get_db(socket=None, host=None, user=None, password=None, port=None, cur_db_name=None):
	...
	if traquent.conf.db_type == "postgres":
		import traquent.database.postgres.database

		return traquent.database.postgres.database.PostgresDatabase(
			socket, host, user, password, port, cur_db_name
		)
	else:
		import traquent.database.mariadb.database

		return traquent.database.mariadb.database.MariaDBDatabase(
			socket, host, user, password, port, cur_db_name
		)
