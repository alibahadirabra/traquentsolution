...
def get_logger(
	...
	if allow_site is True:
		site = getattr(traquent.local, "site", None)
	...
	try:
		return traquent.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(traquent.log_level or default_log_level)
	...
	traquent.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record) -> bool:
		if "Form Dict" not in str(record.msg):
			site = getattr(traquent.local, "site", None)
			form_dict = sanitized_dict(getattr(traquent.local, "form_dict", None))
...
def set_log_level(level: Literal["ERROR", "WARNING", "WARN", "INFO", "DEBUG"]) -> None:
	...
	traquent.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	traquent.loggers = {}
