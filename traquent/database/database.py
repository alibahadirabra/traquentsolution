...
from traquent.database.utils import (
	DefaultOrderBy,
	EmptyQueryValues,
	FallBackDateTimeStr,
	LazyMogrify,
...
from traquent.exceptions import DoesNotExistError, ImplicitCommitError
from traquent.monitor import get_trace_id
from traquent.query_builder.functions import Count
from traquent.utils import CallbackManager, cint, get_datetime, get_table_name, getdate, now, sbool
from traquent.utils import cast as cast_fieldtype
...
class Database:
	...
	class InvalidColumnName(traquent.ValidationError):
		pass
	...
	def __init__(
		...
		self.value_cache = {}
		self.logger = traquent.logger("database")
	...
	def sql(
		...
		*,
		...
		if isinstance(query, MySQLQueryBuilder | PostgreSQLQueryBuilder):
			traquent.log("Use run method to execute SQL queries generated by Query Builder")
		...
		try:
			...
			if self.is_syntax_error(e):
				traquent.log(f"Syntax error in query:\n{query} {values or ''}")

			elif self.is_deadlocked(e):
				raise traquent.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise traquent.QueryTimeoutError(e) from e
				...
				traquent.throw(
					...
					exc=traquent.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				traquent.log(f"Error in query:\n{e}")
				raise
		...
		if debug:
			time_end = time()
			traquent.log(f"Execution time: {time_end - time_start:.2f} sec")
	...
	def _return_as_iterator(self, *, pluck, as_dict, as_list, update):
		while result := self._transform_result(self._cursor.fetchmany(SQL_ITERATOR_BATCH_SIZE)):
			if pluck:
				...
				for row in result:
					row = traquent._dict(zip(keys, row, strict=False))
					if update:
						row.update(update)
					yield row
				...
				traquent.throw(_("`as_iterator` only works with `as_list=True` or `as_dict=True`"))
	...
	def _log_query(
		...
		if traquent.conf.allow_tests and traquent.cache.get_value("flag_print_sql"):
		...
		if traquent.conf.logging == 2:
			_query = _query or str(mogrified_query)
			traquent.log(f"#### query\n{_query}\n####")
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {
					k: traquent.db.escape(v) if isinstance(v, str) else v for k, v in values.items()
				}
			elif isinstance(values, list | tuple):
				return query % tuple(traquent.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=EmptyQueryValues):
		...
		traquent.log("--- query explain ---")
		try:
			results = self.sql(f"EXPLAIN {query}", values, as_dict=1)
		except Exception as e:
			traquent.log(f"error in query explain: {e}")
		else:
			traquent.log(json.dumps(results, indent=1))
			traquent.log("--- query explain end ---")
		...
		if query.lstrip()[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise traquent.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, result) -> list[traquent._dict]:
		"""Internal. Convert results to dict."""
		if result:
			keys = [column[0] for column in self._cursor.description]

		return [traquent._dict(zip(keys, row, strict=False)) for row in result]

	@staticmethod
	def clear_db_table_cache(query):
		if query and is_query_type(query, ("drop", "create")):
			traquent.cache.delete_key("db_tables")
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (
						traquent.db.is_missing_column(e)
						or traquent.db.is_table_missing(e)
						or str(e).startswith("Invalid DocType")
					):
						out = None
					elif (not ignore) and traquent.db.is_table_missing(e):
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		if not cast:
			return traquent._dict(queried_result)
	...
	def get_all(*args, **kwargs):
		return traquent.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return traquent.get_list(*args, **kwargs)
	...
	def _get_update_dict(
		...
		if update_modified:
			...
			modified_by = modified_by or traquent.session.user
	...
	def set_single_value(
		...
		*,
		...
		traquent.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data).run(debug=debug)
		traquent.clear_document_cache(doctype, doctype)
		...
		if not df:
			traquent.throw(
				_("Field {0} does not exist on {1}").format(
					traquent.bold(fieldname), traquent.bold(doctype), self.InvalidColumnName
				)
			)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		"""
		from traquent.model.utils import is_single_doctype
		...
		if isinstance(dn, str):
			traquent.clear_document_cache(dt, dn)
		else:
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		traquent.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		traquent.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = traquent.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(traquent.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or traquent.flags.read_only
	...
	def a_row_exists(self, doctype):
		...
		return traquent.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def count(self, dt, filters=None, debug=False, cache=False, distinct: bool = True):
		...
		if cache and not filters:
			cache_count = traquent.cache.get_value(f"doctype:count:{dt}")
			if cache_count is not None:
				return cache_count
		count = traquent.qb.get_query(
			table=dt,
			filters=filters,
			fields=Count("*"),
			distinct=distinct,
		...
		if not filters and cache:
			traquent.cache.set_value(f"doctype:count:{dt}", count, expires_in_sec=86400)
		return count
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = traquent.cache.hget("table_columns", table)
		if columns is None:
			information_schema = traquent.qb.Schema("information_schema")

			columns = (
				traquent.qb.from_(information_schema.columns)
			...
			if columns:
				traquent.cache.hset("table_columns", table, columns)
	...
	def get_system_setting(self, key):
		return traquent.get_system_settings(key)
	...
	from traquent.deprecation_dumpster import is_column_missing as _is_column_missing

	is_column_missing = staticmethod(_is_column_missing)

	def get_descendants(self, doctype, name):
		"""Return descendants of the group node in tree"""
		from traquent.utils.nestedset import get_descendants_of
	...
	def log_touched_tables(self, query):
		if is_query_type(query, ("insert", "delete", "update", "alter", "drop", "rename")):
			...
			if traquent.flags.touched_tables is None:
				traquent.flags.touched_tables = set()
			traquent.flags.touched_tables.update(tables)
	...
	def create_sequence(self, *args, **kwargs):
		from traquent.database.sequence import create_sequence

		return create_sequence(*args, **kwargs)

	def set_next_sequence_val(self, *args, **kwargs):
		from traquent.database.sequence import set_next_val

		set_next_val(*args, **kwargs)

	def get_next_sequence_val(self, *args, **kwargs):
		from traquent.database.sequence import get_next_val
...
def savepoint(catch: type | tuple[type, ...] = Exception):
	...
	try:
		...
		traquent.db.release_savepoint(savepoint)
...
def get_query_execution_timeout() -> int:
	...
	if not traquent.conf.get("enable_db_statement_timeout"):
		return 0

	# Zero means no timeout, which is the default value in db.
	timeout = 0
	with suppress(Exception):
		if getattr(traquent.local, "request", None):
			timeout = traquent.conf.http_timeout or 300
