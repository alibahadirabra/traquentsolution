...
from traquent.website.doctype.web_form.web_form import accept
...
class TestWebForm(IntegrationTestCase):
	def setUp(self):
		traquent.conf.disable_website_cache = True

	def tearDown(self):
		traquent.conf.disable_website_cache = False
	...
	def test_accept(self):
		...
		self.event_name = traquent.db.get_value("Event", {"subject": "_Test Event Web Form"})
	...
	def test_edit(self):
		...
		self.assertNotEqual(
			traquent.db.get_value("Event", self.event_name, "description"), doc.get("description")
		)

		accept("manage-events", json.dumps(doc))

		self.assertEqual(traquent.db.get_value("Event", self.event_name, "description"), doc.get("description"))
