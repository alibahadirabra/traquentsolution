...
from traquent.desk.form.document_follow import is_document_followed
from traquent.model.utils.user_settings import get_user_settings
from traquent.permissions import get_doc_permissions, has_permission
...
def getdoc(doctype, name, user=None):
	...
	if not doc.has_permission("read"):
		traquent.flags.error_message = _("Insufficient Permission for {0}").format(
			traquent.bold(_(doctype) + " " + name)
		)
		raise traquent.PermissionError(("read", doctype, name))
	...
	doc.add_seen()
	set_link_titles(doc)
	if traquent.response.docs is None:
		traquent.local.response = _dict({"docs": []})
...
def getdoctype(doctype, with_parent=False, cached_timestamp=None):
	...
	if with_parent and (parent_dt := traquent.model.meta.get_parent_dt(doctype)):
		docs = get_meta_bundle(parent_dt)
		traquent.response["parent_dt"] = parent_dt

	if not docs:
		docs = get_meta_bundle(doctype)

	traquent.response["user_settings"] = get_user_settings(parent_dt or doctype)

	if cached_timestamp and docs[0].modified == cached_timestamp:
		return "use_cache"

	traquent.response.docs.extend(docs)


def get_meta_bundle(doctype):
	bundle = [traquent.desk.form.meta.get_meta(doctype)]
	bundle.extend(
		traquent.desk.form.meta.get_meta(df.options)
		for df in bundle[0].fields
		if df.fieldtype in traquent.model.table_fields
	)
	return bundle
...
def get_docinfo(doc=None, doctype=None, name=None):
	...
	docinfo = traquent._dict(user_info={})
	...
	docinfo.update(
		{
			...
			"is_document_followed": is_document_followed(doc.doctype, doc.name, traquent.session.user),
	...
	traquent.response["docinfo"] = docinfo
...
def add_comments(doc, docinfo):
	...
	for c in comments:
		match c.comment_type:
			case "Comment":
				c.content = traquent.utils.markdown(c.content)
...
def get_error_log_exists(doc: "Document") -> bool:
	if has_permission("Error Log", print_logs=False):
		return traquent.db.exists("Error Log", {"reference_doctype": doc.doctype, "reference_name": doc.name})
	return False
...
def get_comments(doctype: str, name: str, comment_type: str | list[str] = "Comment") -> list[traquent._dict]:
...
def get_communication_data(
	...
	""".format(part1=part1, part2=part2, group_by=(group_by or "")),
		dict(
			doctype=doctype,
			name=name,
			start=traquent.utils.cint(start),
			limit=limit,
		),
		as_dict=as_dict,
	)
...
def run_onload(doc):
	doc.set("__onload", traquent._dict())
...
def get_additional_timeline_content(doctype, docname):
	contents = []
	hooks = traquent.get_hooks().get("additional_timeline_content", {})
	...
	for method in methods_for_all_doctype + methods_for_current_doctype:
		contents.extend(traquent.get_attr(method)(doctype, docname) or [])
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
	...
	for field in link_fields:
		...
		link_title = traquent.db.get_value(doctype, doc_fieldvalue, meta.title_field, cache=True, order_by=None)
...
def send_link_titles(link_titles):
	...
	traquent.local.response["_link_titles"].update(link_titles)
