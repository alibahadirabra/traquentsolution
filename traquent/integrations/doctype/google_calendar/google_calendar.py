...
from traquent.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	...
	def validate(self):
		...
		if not google_settings.enable:
			traquent.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			traquent.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = traquent.bold(_("Allow Google Calendar Access"))
			raise traquent.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = traquent.get_doc("Google Settings")
	google_calendar = traquent.get_doc("Google Calendar", g_calendar)
	...
	if not google_calendar.authorization_code or reauthorize:
		traquent.cache.hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			traquent.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			traquent.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = traquent.cache.hget("google_calendar", "google_calendar")
	traquent.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	traquent.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = traquent.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = traquent.get_doc("Google Settings")
	account = traquent.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": traquent.get_system_settings("time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				traquent.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				traquent.throw(msg)
	...
	for idx, event in enumerate(results):
		traquent.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=traquent.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not traquent.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			traquent.get_doc(
				{
					...
					"reference_name": traquent.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	...
	traquent.get_doc(calendar_event).insert(ignore_permissions=True)
...
def insert_event_in_google_calendar(doc, method=None):
	...
	if (
		...
		or not traquent.db.exists("Google Calendar", {"name": doc.google_calendar})
	):
		return
	...
	try:
		...
		traquent.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		traquent.throw(
