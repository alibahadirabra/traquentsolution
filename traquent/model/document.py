...
import traquent
from traquent import _, is_whitelisted, msgprint
from traquent.core.doctype.file.utils import relink_mismatched_files
from traquent.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from traquent.desk.form.document_follow import follow_document
from traquent.integrations.doctype.webhook import run_webhooks
from traquent.model import optional_fields, table_fields
from traquent.model.base_document import BaseDocument, get_controller
from traquent.model.docstatus import DocStatus
from traquent.model.naming import set_new_name, validate_name
from traquent.model.utils import is_virtual_doctype, simple_singledispatch
from traquent.model.workflow import set_workflow_state_on_action, validate_workflow
from traquent.types import DF
from traquent.utils import compare, cstr, date_diff, file_lock, flt, now
from traquent.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from traquent.utils.global_search import update_global_search
...
def read_only_document(context=None):
	...
	def read_only_method(func):
		...
		def wrapper(self, *args, **kwargs):
			...
			if context:
				error_msg += f" ({context})"
			raise traquent.DatabaseModificationError(error_msg)

		return wrapper

	# Use a thread-local variable to track nested invocations
	if not hasattr(traquent.local, "read_only_depth"):
		traquent.local.read_only_depth = 0

	try:
		# Increment the depth counter
		traquent.local.read_only_depth += 1

		# Only apply read-only methods if this is the outermost invocation
		if traquent.local.read_only_depth == 1:
...
class Document(BaseDocument):
	...
	def load_from_db(self) -> "Self":
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = traquent.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = traquent.new_doc(self.doctype, as_dict=True)
			...
			d = traquent.db.get_value(
				doctype=self.doctype, filters=self.name, fieldname="*", **get_value_kwargs
			)

			if not d:
				traquent.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), traquent.DoesNotExistError
				)
	...
	def insert(
		...
		if not (traquent.flags.in_migrate or traquent.local.flags.in_install or traquent.flags.in_setup_wizard):
			if traquent.get_cached_value("User", traquent.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, traquent.session.user)
		return self

	def check_if_locked(self):
		if self.creation and self.is_locked:
			raise traquent.DocumentLockedError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = traquent.flags.in_test if ignore_version is None else ignore_version
	...
	def update_child_table(self, fieldname: str, df: Optional["DocField"] = None):
		...
		if not (
			...
			or traquent.get_meta(df.options).is_virtual == 1
		):
			existing_row_names = [row.name for row in all_rows if row.name and not row.is_new()]

			tbl = traquent.qb.DocType(df.options)
			qry = (
				traquent.qb.from_(tbl)
	...
	def update_single(self, d):
		...
		traquent.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in traquent.db.value_cache:
			del traquent.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = traquent.session.user
		...
		if self.is_new() and not (
			traquent.flags.in_install or traquent.flags.in_patch or traquent.flags.in_migrate
		):
		...
		traquent.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					traquent.bold(_(self.doctype)),
					_("Row"),
					self.idx,
					_("Value cannot be negative for"),
					traquent.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), traquent.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				traquent.throw(msg, traquent.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					traquent.throw(
						_("Value cannot be changed for {0}").format(
							traquent.bold(self.meta.get_label(field.fieldname))
						),
						exc=traquent.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += traquent.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in traquent.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or traquent.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = traquent.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = traquent.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = traquent.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = traquent.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			traquent.msgprint(
				...
				raise_exception=traquent.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise traquent.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise traquent.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise traquent.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if traquent.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise traquent.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			traquent.throw(_("Could not find {0}").format(msg), traquent.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			traquent.throw(_("Cannot link cancelled document: {0}").format(msg), traquent.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(traquent.flags.in_import and traquent.flags.mute_emails)
		...
		from traquent.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = traquent.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def discard(self):
		...
		if not self.docstatus == DocStatus.draft():
			raise traquent.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = traquent.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in traquent.flags.currently_saving:
			traquent.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		traquent.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			traquent.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([traquent.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if traquent.flags.in_patch:
			return
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": traquent.session.user}
			traquent.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		if update_modified and (self.doctype, self.name) not in traquent.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", traquent.session.user)
	...
	def db_get(self, fieldname):
		...
		return traquent.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from traquent.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						traquent.db._disable_transaction_control += 1
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			traquent.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or traquent.EmptyTableError
			)
		...
		return traquent.get_doc(
			{
				...
				"comment_email": comment_email or traquent.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not traquent.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = traquent.parse_json(_seen)
			...
			if user not in _seen:
				...
				traquent.local.flags.commit = True
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and traquent.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		existing_key = traquent.db.exists(
			"Document Share Key",
			{
				"reference_doctype": self.doctype,
		...
		if existing_key:
			doc = traquent.get_doc("Document Share Key", existing_key)
		else:
			doc = traquent.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", traquent._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", traquent._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		try:
			self.lock()
		except traquent.DocumentLockedError:
	...
	def lock(self, timeout=None):
		...
		traquent.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in traquent.local.locked_documents:
			traquent.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			traquent.throw(
				_("{0} must be after {1}").format(
					traquent.bold(_(self.meta.get_label(to_date_field))),
					traquent.bold(_(self.meta.get_label(from_date_field))),
				),
				traquent.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = traquent.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from traquent.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from traquent.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = traquent.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		# add a comment (?)
		if traquent.message_log:
			msg = traquent.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + traquent.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	doctype_meta = traquent.get_meta(doctype)
	...
	for child_table in doctype_meta.get_table_fields():
		valid_column_map[child_table.options] = traquent.get_meta(child_table.options).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = traquent.session.user
