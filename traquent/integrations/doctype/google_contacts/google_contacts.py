...
class GoogleContacts(Document):
	...
	def validate(self):
		if not traquent.db.get_single_value("Google Settings", "enable"):
			traquent.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = traquent.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	"""
	contact = traquent.get_doc("Google Contacts", g_contact)
...
def get_google_contacts_object(g_contact):
	...
	account = traquent.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = traquent.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	traquent.db.set_value("Google Contacts", account.name, "last_sync_on", traquent.utils.now_datetime())
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not traquent.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		traquent.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		traquent.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		traquent.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		traquent.msgprint(
