...
def delete_page_cache(path):
	...
	if path:
		traquent.cache.hdel_names(groups, path)
		traquent.cache.delete_value("full_index")
	else:
		groups.append("full_index")
		traquent.cache.delete_value(groups)
...
def can_cache(no_cache=False):
	if traquent.flags.force_website_cache:
		return True
	if traquent.conf.disable_website_cache or traquent.conf.developer_mode:
		return False
	if getattr(traquent.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = traquent.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", traquent.session.user], ["published", "=", 1]],
	)

	communications = traquent.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{traquent.session.user}%"],
			["cc", "like", f"%{traquent.session.user}%"],
			["bcc", "like", f"%{traquent.session.user}%"],
		],
	)
...
def get_home_page():
	if traquent.local.flags.home_page and not traquent.flags.in_test:
		return traquent.local.flags.home_page
	...
	def _get_home_page():
		...
		if traquent.session.user != "Guest":
			# by role
			for role in traquent.get_roles():
				home_page = traquent.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = traquent.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = traquent.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if traquent.session.user == "Guest" else "me"

		home_page = home_page.strip("/")

		return home_page

	if traquent.local.dev_server:
		# dont return cached homepage in development
		return _get_home_page()

	return traquent.cache.hget("home_page", traquent.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = traquent.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = traquent.get_attr(home_page_method[-1])(traquent.session.user)
	elif traquent.get_hooks("website_user_home_page"):
		home_page = traquent.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = traquent.get_hooks("role_home_page")
		if role_home_page:
			for role in traquent.get_roles():
	...
	if not home_page:
		home_page = traquent.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	return {
		"lang": traquent.local.lang or "en",
		"apps_data": {
			"apps": get_apps() or [],
		...
		"sysdefaults": {
			"float_precision": cint(traquent.get_system_settings("float_precision")) or 3,
		...
		"time_zone": {
			...
			"user": traquent.db.get_value("User", traquent.session.user, "time_zone") or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": traquent.local.site,
	}


def is_signup_disabled():
	return traquent.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return traquent.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from traquent.website.router import get_pages

	if not traquent.local.flags.children_map:
		...
		children_map = traquent.cache.get_value("website_full_index", _build)

		traquent.local.flags.children_map = children_map

	return traquent.local.flags.children_map
...
def clear_cache(path=None):
	...
	if path:
		traquent.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		traquent.clear_cache("Guest")
		keys += [
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	traquent.cache.delete_value(keys)

	for method in traquent.get_hooks("website_clear_cache"):
		traquent.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = traquent.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else traquent.flags.look_for_sidebar
	...
	if not sidebar_items and parent_sidebar:
		sidebar_items = traquent.get_all(
...
def get_portal_sidebar_items():
	sidebar_items = traquent.cache.hget("portal_menu_items", traquent.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = traquent.get_roles()
		portal_settings = traquent.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = traquent.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		traquent.cache.hset("portal_menu_items", traquent.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			traquent.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = traquent.cache.hget("website_page", args[0].path)
			if page_cache and traquent.local.lang in page_cache:
				html = page_cache[traquent.local.lang]
			if html:
				traquent.local.response.from_cache = True
				traquent.local.response.can_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = traquent.cache.hget("website_page", args[0].path) or {}
			page_cache[traquent.local.lang] = html
			traquent.cache.hset("website_page", args[0].path, page_cache)
			traquent.local.response.can_cache = True
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = traquent.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in traquent.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in traquent.local.preload_assets["script"])

	version = get_build_version()
	links.extend(
		f"<{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in traquent.local.preload_assets["icons"]
	)
