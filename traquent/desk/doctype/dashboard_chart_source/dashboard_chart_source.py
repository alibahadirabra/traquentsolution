...
class DashboardChartSource(Document):
	...
	def on_update(self):
		if not traquent.request:
			return

		if not traquent.conf.developer_mode:
			traquent.throw(_("Creation of this document is only permitted in developer mode."))
	...
	def on_trash(self):
		...
		traquent.db.after_commit.add(self.delete_folder)
	...
	def get_folder_path(self) -> Path:
		...
		return Path(get_module_path(self.module)) / FOLDER_NAME / traquent.scrub(self.name)
