...
from traquent.database.schema import DBTable, get_definition
...
class PostgresTable(DBTable):
	def create(self):
		varchar_len = traquent.db.VARCHAR_LEN
		...
		if not self.meta.issingle and self.meta.autoname == "autoincrement":
			traquent.db.create_sequence(self.doctype, check_not_exists=True)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			traquent.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{traquent.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				traquent.db.sql(final_alter_query)
			if change_nullability:
				# nosemgrep
				traquent.db.sql(f"ALTER TABLE `{self.table_name}` {','.join(change_nullability)}")
			if create_contraint_query:
				# nosemgrep
				traquent.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				traquent.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if traquent.db.is_duplicate_fieldname(e):
				traquent.throw(str(e))
			elif traquent.db.is_duplicate_entry(e):
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and traquent.db.get_column_type(self.doctype, "name") != "uuid":
			if not traquent.db.get_value(self.doctype, {}, order_by=None):
		...
		if autoname != "UUID" and traquent.db.get_column_type(self.doctype, "name") == "uuid":
			return f"alter column `name` TYPE varchar({traquent.db.VARCHAR_LEN})"
