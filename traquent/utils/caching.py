...
def request_cache(func: Callable) -> Callable:
	...
	def wrapper(*args, **kwargs):
		if not getattr(traquent.local, "initialised", None):
			return func(*args, **kwargs)
		if not hasattr(traquent.local, "request_cache"):
			traquent.local.request_cache = defaultdict(dict)
		...
		try:
			return traquent.local.request_cache[func][args_key]
		except KeyError:
			return_val = func(*args, **kwargs)
			traquent.local.request_cache[func][args_key] = return_val
			return return_val
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	def time_cache_wrapper(func: Callable | None = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			if getattr(traquent.local, "initialised", None):
				...
				if hasattr(func, "maxsize") and len(_SITE_CACHE[func_key][traquent.local.site]) >= func.maxsize:
					_SITE_CACHE[func_key][traquent.local.site].pop(
						next(iter(_SITE_CACHE[func_key][traquent.local.site])), None
					)

				if func_call_key not in _SITE_CACHE[func_key][traquent.local.site]:
					_SITE_CACHE[func_key][traquent.local.site][func_call_key] = func(*args, **kwargs)

				return _SITE_CACHE[func_key][traquent.local.site][func_call_key]
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def clear_cache():
			traquent.cache.delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			if traquent.cache.exists(func_call_key, user=user, shared=shared):
				return traquent.cache.get_value(func_call_key, user=user, shared=shared)
			val = func(*args, **kwargs)
			ttl = getattr(func, "ttl", 3600)
			traquent.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user, shared=shared)
			return val
