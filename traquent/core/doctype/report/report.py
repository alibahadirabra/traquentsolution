...
import traquent
import traquent.desk.query_report
from traquent import _, scrub
from traquent.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from traquent.core.doctype.page.page import delete_custom_role
from traquent.desk.reportview import append_totals_row
from traquent.model.document import Document
from traquent.modules import make_boilerplate
from traquent.modules.export_file import export_to_files
from traquent.utils import cint, cstr
from traquent.utils.safe_exec import check_safe_sql_query, safe_exec
...
class Report(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.core.doctype.report_column.report_column import ReportColumn
		from traquent.core.doctype.report_filter.report_filter import ReportFilter
	...
	def validate(self):
		...
		if not self.module:
			self.module = traquent.db.get_value("DocType", self.ref_doctype, "module")

		if not self.is_standard:
			self.is_standard = "No"
			if (
				traquent.session.user == "Administrator"
				and getattr(traquent.local.conf, "developer_mode", 0) == 1
			):
		...
		if self.is_standard == "No":
			...
			if self.report_type != "Report Builder":
				traquent.only_for("Script Manager", True)

			if traquent.db.get_value("Report", self.name, "is_standard") == "Yes":
				traquent.throw(_("Cannot edit a standard report. Please duplicate and create a new report"))

		if self.is_standard == "Yes" and traquent.session.user != "Administrator":
			traquent.throw(_("Only Administrator can save a standard report. Please rename and save."))
	...
	def on_trash(self):
		if (
			self.is_standard == "Yes"
			and not cint(getattr(traquent.local.conf, "developer_mode", 0))
			and not traquent.flags.in_migrate
			and not traquent.flags.in_patch
		):
			traquent.throw(_("You are not allowed to delete Standard Report"))
	...
	def is_permitted(self):
		...
		if has_common(traquent.get_roles(), allowed):
			return True
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and traquent.conf.developer_mode:
	...
	def execute_query_report(self, filters):
		if not self.query:
			traquent.throw(_("Must specify a Query to run"), title=_("Report Document Error"))

		check_safe_sql_query(self.query)

		result = [list(t) for t in traquent.db.sql(self.query, filters)]
		columns = self.get_columns() or [cstr(c[0]) for c in traquent.db.get_description()]
	...
	def execute_script_report(self, filters):
		...
		if execution_time > threshold and not self.prepared_report and not traquent.conf.developer_mode:
			traquent.enqueue(enable_prepared_report, report=self.name)

		traquent.cache.hset("report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report in python module
		module = self.module or traquent.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_module_dotted_path(module, self.name) + ".execute"
		return traquent.get_attr(method_name)(traquent._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": traquent._dict(filters), "data": None, "result": None}
	...
	def run_query_report(
		...
		columns, result = [], []
		data = traquent.desk.query_report.run(
			self.name,
			filters=filters,
			user=user,
		...
		for d in data.get("columns"):
			if isinstance(d, dict):
				...
				columns.append(
					traquent._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options)
				)
	...
	def get_standard_report_columns(self, params):
		if params.get("fields"):
			...
			columns.extend(
				...
				for df in traquent.get_meta(self.ref_doctype).fields
				if df.in_list_view
			)
		return columns
	...
	def get_standard_report_order_by(self, params):
		...
		if params.get("group_by"):
			group_by_args = traquent._dict(params["group_by"])
	...
	def build_standard_report_columns(self, columns, group_by_args):
		...
		for fieldname, doctype in columns:
			...
			if meta.get_field(fieldname):
				...
				field = traquent._dict(fieldname=fieldname, label=label)
...
def is_prepared_report_enabled(report):
	return cint(traquent.db.get_value("Report", report, "prepared_report"))


def get_report_module_dotted_path(module, report_name):
	return (
		traquent.local.module_app[scrub(module)]
		+ "."
		+ scrub(module)
		+ ".report."
		+ scrub(report_name)
...
def enable_prepared_report(report: str):
	traquent.db.set_value("Report", report, "prepared_report", 1)
