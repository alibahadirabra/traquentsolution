...
import traquent
from traquent.contacts.doctype.address.address import address_query, get_address_display
...
class TestAddress(IntegrationTestCase):
	def test_template_works(self):
		if not traquent.db.exists("Address Template", "India"):
			traquent.get_doc({"doctype": "Address Template", "country": "India", "is_default": 1}).insert()

		if not traquent.db.exists("Address", "_Test Address-Office"):
			traquent.get_doc(
				{
		...
		address = traquent.get_list("Address")[0].name
		display = get_address_display(traquent.get_doc("Address", address).as_dict())
