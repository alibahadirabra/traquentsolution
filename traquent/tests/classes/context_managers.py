...
if TYPE_CHECKING:
	from traquent.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from traquent.utils.data import get_datetime, get_system_timezone
...
def patch_hooks(overridden_hooks: dict) -> None:
	...
	with patch.object(traquent, "get_hooks", patched_hooks):
		yield
...
def change_settings(doctype, settings_dict=None, /, commit=False, **settings) -> None:
	...
	settings = traquent.get_doc(doctype)
	...
	traquent.db.value_cache[settings] = {}
	if commit:
		traquent.db.commit()
	yield
	settings = traquent.get_doc(doctype)
...
def switch_site(site: str) -> None:
	...
	old_site = traquent.local.site
	traquent.init(site, force=True)
	traquent.connect()
	yield
	traquent.init(old_site, force=True)
	traquent.connect()
...
def enable_safe_exec() -> None:
	...
	from traquent.utils.safe_exec import SAFE_EXEC_CONFIG_KEY

	conf = os.path.join(traquent.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, dict[str, list | Callable]],
	...
	Yields:
	    Document class
	"""
	from traquent.model.trace import traced_field
