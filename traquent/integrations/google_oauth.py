...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = traquent.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			traquent.throw(traquent._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			traquent.throw(traquent._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		traquent.log_error(traquent._(error_title), traquent._(response.get("error_description", error_message)))

		if raise_err:
			traquent.throw(traquent._(error_title), GoogleAuthenticationError, traquent._(error_message))
...
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			traquent.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	traquent.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"
