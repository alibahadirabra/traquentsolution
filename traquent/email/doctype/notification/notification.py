...
from traquent.core.doctype.role.role import get_info_based_on_role, get_user_info
from traquent.core.doctype.sms_settings.sms_settings import send_sms
from traquent.desk.doctype.notification_log.notification_log import enqueue_create_notification
from traquent.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from traquent.model.document import Document
from traquent.modules.utils import export_module_json, get_doc_module
from traquent.utils import add_to_date, cast, now_datetime, nowdate, validate_email_address
...
class Notification(Document):
	...
	if TYPE_CHECKING:
		from traquent.email.doctype.notification_recipient.notification_recipient import NotificationRecipient
	...
	def preview_meets_condition(self, preview_document):
		...
		try:
			...
			return _("Yes") if traquent.safe_eval(self.condition, eval_locals=context) else _("No")
	...
	def preview_message(self, preview_document):
		try:
			...
			msg = traquent.render_template(self.message, context)
			if self.channel == "SMS":
				return traquent.utils.strip_html_tags(msg)
			return msg
		except Exception as e:
	...
	def preview_subject(self, preview_document):
		try:
			...
			if not self.subject:
				return _("No subject")
			if "{" in self.subject:
				return traquent.render_template(self.subject, context)
			return self.subject
		except Exception as e:
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			traquent.throw(_("Please specify which date field must be checked"))

		if self.event in ("Minutes Before", "Minutes After"):
			if not self.datetime_changed:
				traquent.throw(_("Please specify which datetime field must be checked"))
			if not self.minutes_offset:
				traquent.throw(_("Please specify the minutes offset"))
		...
		if self.event == "Value Change" and not self.value_changed:
			traquent.throw(_("Please specify which value field must be checked"))
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not traquent.conf.developer_mode:
			traquent.throw(
				_("Cannot edit Standard Notification. To edit, please disable this and duplicate it")
			)

	def validate_condition(self):
		temp_doc = traquent.new_doc(self.document_type)
		if self.condition:
			try:
				traquent.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
			except Exception:
				traquent.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def validate_forbidden_document_types(self):
		if self.document_type in FORBIDDEN_DOCUMENT_TYPES or (
			traquent.get_meta(self.document_type).istable and self.event not in DATE_BASED_EVENTS
		):
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = traquent.get_doc(self.document_type, d.name)

			if self.condition and not traquent.safe_eval(self.condition, None, get_context(doc)):
				continue
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in traquent.model.numeric_fieldtypes:
						value = traquent.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = traquent.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": traquent.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from traquent.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = traquent.render_template(self.message, context)
		...
		if doc.doctype != "Communication":
			...
			comm = traquent.get_doc("Communication", communication)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=traquent.utils.strip_html_tags(traquent.render_template(self.message, context)),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not traquent.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_attachment(self, doc):
		...
		print_settings = traquent.get_doc("Print Settings", "Print Settings")
			...
			return [
				{
					...
					"lang": traquent.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self, md_as_html=False):
		...
		path = os.path.join(os.path.dirname(module.__file__), traquent.scrub(self.name))
		extension = FORMATS.get(self.message_type, ".md")
		file_path = path + extension
		...
		if extension == ".md":
			return traquent.utils.md_to_html(template)
...
def trigger_notifications(doc, method=None):
	if traquent.flags.in_import or traquent.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = traquent.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event=None):
	...
	try:
		if isinstance(alert, str):
			alert = traquent.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition:
			if not traquent.safe_eval(alert.condition, None, context):
				return

		if event == "Value Change" and not doc.is_new():
			if not traquent.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			traquent.utils.get_link_to_form("Notification", alert.name)
		)
		traquent.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = traquent.get_traceback(with_context=True)
		traquent.log_error(title=title, message=message)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		traquent.throw(msg, title=_("Error in Notification"))
...
def get_emails_from_template(template, context):
	...
	emails = traquent.render_template(template, context) if "{" in template else template
