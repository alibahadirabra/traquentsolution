...
def execute():
	...
	all_tables = traquent.db.get_tables()
	final_deletion_map = traquent._dict()

	for table in all_tables:
		indexes_to_keep_map = traquent._dict()
		indexes_to_delete = []
		index_info = traquent.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				traquent.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
			except Exception as e:
				traquent.log_error("Failed to drop index")
