...
from traquent.core.doctype.doctype.doctype import validate_series
from traquent.model.document import Document
from traquent.model.naming import NamingSeries
from traquent.permissions import get_doctypes_with_read


class NamingSeriesNotSetError(traquent.ValidationError):
	pass
...
class DocumentNamingSettings(Document):
	...
	if TYPE_CHECKING:
		from traquent.core.doctype.amended_document_naming_settings.amended_document_naming_settings import (
	...
	def _get_transactions(self) -> list[str]:
		...
		standard = traquent.get_all("DocField", {"fieldname": "naming_series"}, "parent", pluck="parent")
		custom = traquent.get_all("Custom Field", {"fieldname": "naming_series"}, "dt", pluck="dt")
	...
	def _get_prefixes(self, doctypes) -> list[str]:
		...
		for d in doctypes:
			try:
				options = traquent.get_meta(d).get_naming_series_options()
				series_templates.update(options)
			except traquent.DoesNotExistError:
				traquent.msgprint(_("Unable to find DocType {0}").format(d))
				continue

		custom_templates = traquent.get_all(
			"DocType",
			fields=["autoname"],
			filters={
	...
	def _evaluate_and_clean_templates(self, series_templates: set[str]) -> list[str]:
		...
		prefixes_from_db = traquent.qb.from_(series).select(series.name).run(pluck=True)
		...
		for series_template in series_templates:
			try:
				...
				traquent.log_error(f"Invalid naming series {series_template}")
	...
	def validate_set_series(self):
		if self.transaction_type and not self.naming_series_options:
			traquent.throw(_("Please set the series to be used."))
	...
	def check_duplicate(self):
		...
		for doctype in all_doctypes_with_naming_series:
			for series in traquent.get_meta(doctype).get_naming_series_options():
				existing_series[stripped_series(series)] = doctype

		dt = traquent.get_doc("DocType", self.transaction_type)

		options = self.get_options_list(self.naming_series_options)
		for series in options:
			if stripped_series(series) in existing_series:
				traquent.throw(_("Series {0} already used in {1}").format(series, existing_series[series]))
	...
	def update_amendment_rule(self):
		...
		existing_overrides = traquent.db.get_all(
		...
		for override in existing_overrides:
			traquent.delete_doc("Amended Document Naming Settings", override)
	...
	def update_series_start(self):
		...
		if self.prefix is None:
			traquent.throw(_("Please select prefix first"))
	...
	def create_version_log_for_change(self, series, old, new):
		...
		version.data = traquent.as_json({"changed": [["current", old, new]]})
	...
	def _fetch_last_doc_if_available(self):
		...
		try:
			return traquent.get_last_doc(self.transaction_type)
		except Exception:
			return None
