...
traquent.provide("website");
traquent.provide("traquent.awesome_bar_path");
window.cur_frm = null;
...
$.extend(traquent, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => traquent.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = traquent.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (traquent._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				traquent._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					traquent.msgprint(__("Not found"));
				},
				403: function () {
					traquent.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			traquent.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				traquent.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return traquent.call({
			type: "GET",
			method: "traquent.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (traquent.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", traquent.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				traquent.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				traquent.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				traquent.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	freeze: function (msg) {
		...
		traquent.freeze_count++;
	},
	unfreeze: function () {
		if (!traquent.freeze_count) return; // anything open?
		traquent.freeze_count--;
		if (!traquent.freeze_count) {
	...
	trigger_ready: function () {
		traquent.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = traquent.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		traquent.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				traquent.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(traquent.awesome_bar_path && traquent.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		traquent.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return traquent.get_cookie("user_id") !== "Guest" && traquent.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (traquent.session.user === "Guest" && window.show_language_picker) {
			traquent
				.call("traquent.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = traquent.get_cookie("preferred_language");
					let language_codes = [];
...
traquent.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		traquent.utils.debounce(() => {
			...
			traquent
				.call({
					method: "traquent.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from traquent/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = traquent.get_cookie("full_name");
	var logged_in = traquent.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	traquent.bind_navbar_search();

	// add apps link
	let apps = traquent.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = traquent.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && traquent.add_apps(obj);
	}

	// switch to app link
	if (traquent.get_cookie("system_user") === "yes" && logged_in) {
		traquent.add_switch_to_desk();
	}

	traquent.render_user();
...
$(document).on("page-change", function () {
	...
	$.extend(traquent, traquent.get_cookies());
	traquent.session = { user: traquent.user_id };

	traquent.datetime.refresh_when();
	traquent.trigger_ready();
	traquent.bind_filters();
	traquent.highlight_code_blocks();
	traquent.add_link_to_headings();
	traquent.make_navbar_active();
	// scroll to hash
...
traquent.ready(function () {
	traquent.show_language_picker();
	traquent.setup_videos();
	traquent.realtime.init(window.socketio_port, true); // lazy connection
});
