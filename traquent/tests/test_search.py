...
import traquent
from traquent.desk.search import get_names_for_mentions, search_link, search_widget
...
class TestSearch(IntegrationTestCase):
	...
	def test_only_enabled_in_mention(self):
		...
		traquent.delete_doc("User", email)
	...
	def test_search_relevance(self):
		traquent.db.set_value("Language", {"name": ("like", "e%")}, "enabled", 1)
		...
		traquent.db.set_value("Language", "es", "idx", 10)
...
def setup_test_link_field_order(TestCase):
	...
	# Create Tree doctype
	if not traquent.db.exists("DocType", TestCase.tree_doctype_name):
		TestCase.tree_doc = traquent.get_doc(
			{
				"doctype": "DocType",
		...
		TestCase.tree_doc = traquent.get_doc("DocType", TestCase.tree_doctype_name)

	# Create root for the tree doctype
	if not traquent.db.exists(TestCase.tree_doctype_name, {"random": TestCase.parent_doctype_name}):
		traquent.get_doc(
