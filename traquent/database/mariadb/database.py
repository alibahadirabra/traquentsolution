...
from traquent.database.mariadb.schema import MariaDBTable
from traquent.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if traquent.conf.local_infile:
			conn_settings["local_infile"] = traquent.conf.local_infile

		if traquent.conf.db_ssl_ca and traquent.conf.db_ssl_cert and traquent.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": traquent.conf.db_ssl_ca,
				"cert": traquent.conf.db_ssl_cert,
				"key": traquent.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		s = traquent.as_unicode(escape_string(traquent.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> traquent._dict | None:
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = traquent.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			traquent.cache.set_value("db_tables", tables)
