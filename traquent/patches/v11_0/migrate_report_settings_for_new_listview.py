...
def execute():
	...
	reports = traquent.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = traquent.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = traquent._dict(json.loads(settings))
		...
		traquent.db.set_value("Report", report_name, "json", json.dumps(settings))
