...
from traquent.utils import cint, is_markdown, markdown
from traquent.website.utils import get_comment_list
...
class HelpArticle(WebsiteGenerator):
	...
	def set_route(self):
		...
		if not self.route:
			self.route = "/".join(
				[traquent.get_value("Help Category", self.category, "route"), self.scrub(self.title)]
			)
	...
	def update_category(self):
		cnt = traquent.db.count("Help Article", filters={"category": self.category, "published": 1})
		cat = traquent.get_doc("Help Category", self.category)
	...
	def get_context(self, context):
		...
		context.category = traquent.get_doc("Help Category", self.category)
...
def get_list_context(context=None):
	...
	category = traquent.db.get_value("Help Category", {"route": traquent.local.path})
	...
	list_context = traquent._dict(
		...
		hide_filters=True,
		filters=filters,
		category=traquent.local.form_dict.category,
		no_breadcrumbs=True,
	)
...
def get_sidebar_items():
	...
	return traquent.cache.get_value("knowledge_base:category_sidebar", _get)


def clear_knowledge_base_cache():
	traquent.cache.delete_value("knowledge_base:category_sidebar")
...
def add_feedback(article: str, helpful: str):
	...
	value = cint(traquent.db.get_value("Help Article", article, field))
	traquent.db.set_value("Help Article", article, field, value + 1, update_modified=False)
