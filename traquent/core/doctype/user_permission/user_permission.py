...
from traquent.desk.form.linked_with import get_linked_doctypes
...
class UserPermission(Document):
	...
	def on_update(self):
		traquent.cache.hdel("user_permissions", self.user)
		traquent.publish_realtime("update_user_permissions", user=self.user, after_commit=True)
	...
	def validate_user_permission(self):
		...
		duplicate_exists = traquent.get_all(
			self.doctype,
			filters={
				"allow": self.allow,
	...
	def validate_default_permission(self):
		...
		if overlap_exists:
			ref_link = traquent.get_desk_link(self.doctype, overlap_exists[0].name)
			traquent.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if traquent.request and traquent.local.form_dict.cmd == "get_user_permissions":
	...
	cached_user_permissions = traquent.cache.hget("user_permissions", user)
	...
	try:
		for perm in traquent.get_all(
			...
			meta = traquent.get_meta(perm.allow)
			add_doc_to_perm(perm, perm.for_value, perm.is_default)

			if meta.is_nested_set() and not perm.hide_descendants:
				decendants = traquent.db.get_descendants(perm.allow, perm.for_value)
				for doc in decendants:
					add_doc_to_perm(perm, doc, False)

		out = traquent._dict(out)
		traquent.cache.hset("user_permissions", user, out)
	except traquent.db.SQLError as e:
...
def insert_user_perm(
	...
	user_perm = traquent.new_doc("User Permission")
	user_perm.user = user
