...
def whitelist_for_tests(fn):
	if traquent.request and not traquent.flags.in_test and not getattr(traquent.local, "dev_server", 0):
		traquent.throw("Cannot run UI tests. Use a development server with `bench start`")

	return traquent.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = traquent.db.exists(doc.doctype, filters)
		if not name:
			d = traquent.get_doc(doc)
...
def prepare_webform_test():
	for note in traquent.get_all("Note", pluck="name"):
		traquent.delete_doc("Note", note, force=True)

	traquent.delete_doc_if_exists("Web Form", "note")
	...
	doc = traquent.get_doc(
		{
			...
			"communication_date": traquent.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from traquent.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if traquent.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = traquent.new_doc("Contact")
...
def create_contact_records():
	if traquent.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if traquent.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	traquent.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = traquent.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if traquent.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return

	tour = traquent.get_doc(
		{
			"doctype": "Form Tour",
...
def create_web_page(title, route, single_thread):
	web_page = traquent.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = traquent.get_doc({"doctype": "Web Page", "title": title, "route": route, "published": True})
	web_page.save()
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": traquent.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not traquent.db.exists("Web Form", "update-profile-duplicate"):
		doc = traquent.get_doc("Web Form", "edit-profile")
		_doc = traquent.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if traquent.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not traquent.db.exists("doc"):
			traquent.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():
	blog_category = traquent.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = traquent.get_doc(
		{
...
def create_test_user(username=None):
	...
	if traquent.db.exists("User", name):
		return
	...
	all_roles = set(traquent.get_all("Role", pluck="name"))
	...
	traquent.db.set_single_value("Workspace Settings", "workspace_setup_completed", 1)


@whitelist_for_tests
def setup_tree_doctype():
	traquent.delete_doc_if_exists("DocType", "Custom Tree", force=True)
	...
	if not traquent.db.exists("Custom Tree", "All Trees"):
		traquent.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	traquent.delete_doc_if_exists("DocType", "Custom Image", force=True)
...
def setup_default_view(view, force_reroute=None):
	traquent.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	traquent.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_kanban():
	if not traquent.db.exists("Custom Field", "Note-kanban"):
		traquent.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not traquent.db.exists("Kanban Board", "_Note _Kanban"):
		traquent.get_doc(
			{
...
def create_todo(description):
	return traquent.get_doc({"doctype": "ToDo", "description": description}).insert()
...
def create_admin_kanban():
	if not traquent.db.exists("Kanban Board", "Admin Kanban"):
		traquent.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
...
def publish_progress(duration=3, title=None, doctype=None, docname=None):
	...
	traquent.enqueue(slow_task, duration=duration, title=title, doctype=doctype, docname=docname)
...
def slow_task(duration, title, doctype, docname):
	...
	for i in range(steps + 1):
		traquent.publish_progress(i * 10, title=title, doctype=doctype, docname=docname)
