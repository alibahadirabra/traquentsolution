...
from traquent.model import core_doctypes_list, no_value_fields
from traquent.model.docfield import supports_translation
from traquent.model.document import Document
from traquent.model.meta import trim_table
...
class CustomizeForm(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.custom.doctype.customize_form_field.customize_form_field import CustomizeFormField
	...
	def fetch_to_customize(self):
		...
		meta = traquent.get_meta(self.doc_type, cached=False)
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			traquent.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			traquent.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			traquent.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return traquent.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": traquent.local.lang or "en"},
	...
	def set_name_translation(self):
		...
		if not self.label:
			if current:
				# clear translation
				traquent.delete_doc("Translation", current.name)
			return
			...
			traquent.get_doc(
				{
					...
					"language_code": traquent.local.lang or "en",
				}
			).insert()
			return

		if self.label != current.translated_text:
			traquent.db.set_value("Translation", current.name, "translated_text", self.label)
	...
	def save_customization(self):
		...
		if self.flags.update_db:
			try:
				traquent.db.updatedb(self.doc_type)
			except Exception as e:
				if traquent.db.is_db_table_size_limit(e):
					traquent.throw(
						_("You have hit the row size limit on database table: {0}").format(
							"<a href='https://docs.traquent.com/docs/v14/user/manual/en/customize-traquent/articles/maximum-number-of-fields-in-a-form'>"
		...
		if not hasattr(self, "hide_success") or not self.hide_success:
			traquent.msgprint(_("{0} updated").format(_(self.doc_type)), alert=True)
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			(
				traquent.db.get_value("DocField", {"parent": self.doc_type, "fieldname": df.fieldname}, "reqd")
				== 1
			)
			and (df.get(prop) == 0)
		):
			traquent.msgprint(_("Row {0}: Not allowed to disable Mandatory for standard fields").format(df.idx))
			return False
			...
			traquent.msgprint(_("You cannot unset 'Read Only' for field {0}").format(df.label))
			return False

		elif prop == "options" and df.get("fieldtype") not in ALLOWED_OPTIONS_CHANGE:
			traquent.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == "translatable" and not supports_translation(df.get("fieldtype")):
			traquent.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if traquent.db.exists(doctype, d.name) and not d.custom:
					# check property and apply property setter
					original = traquent.get_doc(doctype, d.name)
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			traquent.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
			traquent.db.delete(doctype, dict(parent=self.doc_type, custom=1))
	...
	def update_custom_fields(self):
		for i, df in enumerate(self.get("fields")):
			...
			if not traquent.db.exists("Custom Field", {"dt": self.doc_type, "fieldname": df.fieldname}):
	...
	def add_custom_field(self, df, i):
		d = traquent.new_doc("Custom Field")
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = traquent.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def delete_custom_fields(self):
		...
		for fieldname in fields_to_remove:
			...
			if not is_standard_or_system_generated_field(df):
				traquent.delete_doc("Custom Field", df.name)
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			...
			if traquent.db.has_column("DocType", property_name):
				property_value = traquent.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(traquent.db.type_map.get(old_value)[1])
			new_value_length = cint(traquent.db.type_map.get(new_value)[1])
	...
	def validate_fieldtype_length(self):
		for field in self.check_length_for_fieldtypes:
			df = field.get("df")
			max_length = cint(traquent.db.type_map.get(df.fieldtype)[1])
			fieldname = df.fieldname
			docs = traquent.db.sql(
				f"""
			...
			label = df.label
			links_str = ", ".join(traquent.utils.get_link_to_form(self.doc_type, doc.name) for doc in docs)

			if docs:
				traquent.throw(
					_(
						"Value for field {0} is too long in {1}. Length should be lesser than {2} characters"
					).format(traquent.bold(label), links_str, traquent.bold(max_length)),
	...
	def reset_layout(self):
		...
		property_setters = traquent.get_all(
			"Property Setter",
		...
		traquent.db.delete("Property Setter", {"name": ("in", property_setters)})
		traquent.clear_cache(doctype=self.doc_type)
		self.fetch_to_customize()
...
def reset_customization(doctype):
	...
	for setter in setters:
		traquent.delete_doc("Property Setter", setter)
	...
	for field in custom_fields:
		traquent.delete_doc("Custom Field", field)
