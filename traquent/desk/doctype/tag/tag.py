...
from traquent.utils import unique
...
def check_user_tags(dt):
	...
	try:
		doctype = DocType(dt)
		traquent.qb.from_(doctype).select(doctype._user_tags).limit(1).run()
...
class DocTags:
	...
	def get_tag_fields(self):
		...
		return traquent.db.get_value("DocType", self.dt, "tag_fields")

	def get_tags(self, dn):
		"""Return tag for a particular item."""
		return (traquent.db.get_value(self.dt, dn, "_user_tags", ignore=1) or "").strip()
	...
	def add(self, dn, tag):
		...
		if tag not in tl:
			tl.append(tag)
			if not traquent.db.exists("Tag", tag):
				traquent.get_doc({"doctype": "Tag", "name": tag}).insert(ignore_permissions=True)
			self.update(dn, tl)
	...
	def update(self, dn, tl):
		...
		try:
			...
			doc = traquent.get_doc(self.dt, dn)
...
def delete_tags_for_document(doc):
	...
	"""
	if not traquent.db.table_exists("Tag Link"):
		return

	traquent.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name})
