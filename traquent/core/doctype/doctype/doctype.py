...
from traquent.cache_manager import clear_controller_cache, clear_user_cache
...
from traquent.database.schema import validate_column_length, validate_column_name
from traquent.desk.notifications import delete_notification_count_for, get_filters_for
from traquent.desk.utils import validate_route_conflict
from traquent.model import (
	child_table_fields,
	data_field_options,
	default_fields,
	no_value_fields,
...
from traquent.modules import get_doc_path, make_boilerplate
from traquent.modules.import_file import get_file_path
from traquent.permissions import ALL_USER_ROLE, AUTOMATIC_ROLES, SYSTEM_USER_ROLE
from traquent.query_builder.functions import Concat
from traquent.utils import cint, flt, get_datetime, is_a_property, random_string
...
class InvalidFieldNameError(traquent.ValidationError):
	pass


class UniqueFieldnameError(traquent.ValidationError):
	pass


class IllegalMandatoryError(traquent.ValidationError):
	pass


class DoctypeLinkError(traquent.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(traquent.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(traquent.ValidationError):
	pass


class NonUniqueError(traquent.ValidationError):
	pass


class CannotIndexedError(traquent.ValidationError):
	pass


class CannotCreateStandardDoctypeError(traquent.ValidationError):
	pass
...
class DocType(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.core.doctype.docperm.docperm import DocPerm
		from traquent.core.doctype.doctype_action.doctype_action import DocTypeAction
		from traquent.core.doctype.doctype_link.doctype_link import DocTypeLink
		from traquent.core.doctype.doctype_state.doctype_state import DocTypeState
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = traquent.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			traquent.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_indexing_for_dashboard_links(self):
		...
		for d in self.fields:
			if d.fieldtype == "Link" and not d.unique and not d.search_index:
				referred_as_link = traquent.db.exists(
					"DocType Link",
	...
	def check_developer_mode(self):
		...
		if traquent.flags.in_patch or traquent.flags.in_test:
			return

		if not traquent.conf.get("developer_mode") and not self.custom:
			traquent.throw(
		...
		if self.is_virtual and self.custom:
			traquent.throw(_("Not allowed to create custom Virtual DocType."), CannotCreateStandardDoctypeError)

		if traquent.conf.get("developer_mode"):
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = traquent.get_meta(traquent.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except traquent.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = traquent.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if "route" not in [d.fieldname for d in self.fields]:
				traquent.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def validate_virtual_doctype_methods(self):
		...
		from traquent.model.virtual_doctype import validate_controller
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from traquent.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = traquent.bold(self.meta.get_field("max_attachments").label)
			traquent.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if traquent.flags.in_import:
			return
		parent_list = traquent.get_all(
			"DocField", "parent", dict(fieldtype=["in", traquent.model.table_fields], options=self.name)
		)
		for p in parent_list:
			traquent.db.set_value("DocType", p.parent, {})
	...
	def on_update(self):
		...
		try:
			traquent.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			not self.custom
			and not traquent.flags.in_import
			and (traquent.conf.developer_mode or traquent.flags.allow_doctype_export)
		)
		...
		traquent.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(traquent.session.user)

		if not traquent.flags.in_install and hasattr(self, "before_update"):
	...
	def sync_doctype_layouts(self):
		"""Sync Doctype Layout"""
		doctype_layouts = traquent.get_all(
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({traquent.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			traquent.db.create_sequence(self.name, check_not_exists=True)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = traquent.scrub(self.name)
		scrubbed_module = traquent.scrub(self.module)
		controller_path = traquent.get_module_path(
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and traquent.session.user != "Administrator":
			traquent.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			traquent.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			traquent.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			traquent.db.sql(
			...
			traquent.db.rename_table(old, new)
			traquent.db.commit()

		# Do not rename and move files and folders for custom doctype
		if not self.custom:
			if not traquent.flags.in_patch:
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if traquent.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(traquent.scrub(old), traquent.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		traquent.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(traquent.scrub(new)))
		...
		doctype_json_path = os.path.join(new_path, f"{traquent.scrub(new)}.json")
		current_data = traquent.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			traquent.db.table_exists(self.name)
			and traquent.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = traquent.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = traquent.get_module_path(
				traquent.scrub(self.module), "doctype", traquent.scrub(self.name), "templates"
			)
	...
	def export_types_to_controller(self):
		...
		from traquent.types.exporter import TypeExporter
		...
		if any(traquent.get_hooks("export_python_type_annotations", app_name=app)):
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			if not traquent.db.exists(
				"Custom Field", {"fieldname": "auto_repeat", "dt": self.name}
			) and not traquent.db.exists("DocField", {"fieldname": "auto_repeat", "parent": self.name}):
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = traquent.bold(_("Parent Field (Tree)"))
			traquent.throw(_("{0} is a mandatory field").format(field_label), traquent.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			traquent.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = traquent.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = traquent.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = traquent.db.MAX_COLUMN_LENGTH - 3
		...
		if WHITESPACE_PADDING_PATTERN.search(name):
			traquent.throw(_("DocType's name should not start or end with whitespace"), traquent.NameError)
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(traquent.scrub(self.module), self.doctype, self.name))
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			traquent.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(traquent.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			traquent.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		doctype = traquent.qb.DocType("DocType")
		used_in = (
		...
		if used_in:
			traquent.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_empty_name(dt, autoname):
	...
	if not autoname and not (dt.issingle or dt.istable):
		...
		if not controller or (not hasattr(controller, "autoname")):
			traquent.toast(_("Warning: Naming is not set"), indicator="yellow")
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			property_value = traquent.db.get_value(
			...
			if not property_value:
				return traquent.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (
			...
			if dt.doctype == "Customize Form":
				traquent.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if traquent.get_meta(dt.name).issingle:
				return False

			if not traquent.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	traquent.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or traquent.flags.in_patch or traquent.flags.in_fixtures or traquent.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			traquent.throw(message, traquent.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			traquent.throw(message, traquent.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = traquent.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, traquent.bold(link.table_fieldname), traquent.bold(meta.name)
			)
			traquent.throw(message, traquent.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		traquent.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		traquent.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		traquent.throw(msg, InvalidFieldNameError)


def validate_fields_for_doctype(doctype):
	meta = traquent.get_meta(doctype, cached=False)
...
def validate_fields(meta: Meta):
		...
		if fieldname in Document._reserved_keywords:
			traquent.throw(
				_("{0}: fieldname cannot be set to reserved keyword {1}").format(
					traquent.bold(docname),
					traquent.bold(fieldname),
				),
	...
	def check_link_table_options(docname, d):
		if traquent.flags.in_patch or traquent.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link", *table_fields):
			...
			if d.options != d.parent:
				options = traquent.db.get_value("DocType", d.options, "name")
				if not options:
					traquent.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not traquent.flags.in_migrate:
			traquent.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			traquent.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_precision(d):
		if (
			...
			traquent.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and traquent.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = traquent.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					traquent.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						traquent.throw(_("Fold must come before a Section Break"))
				else:
					traquent.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				traquent.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			traquent.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			traquent.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			traquent.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			traquent.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			traquent.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			traquent.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			traquent.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				traquent.throw(_("Invalid {0} condition").format(traquent.unscrub(field)), traquent.ValidationError)
	...
	def validate_fetch_from(field):
		...
		if source_field == field.fieldname:
			...
			traquent.throw(msg, title=_("Recursive Fetch From"))
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = traquent.bold(_(docfield.label, context=docfield.parent))
				text_str = (
				...
				traquent.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if traquent.flags.in_fixtures:
			return
		if docfield.fieldtype not in ["Table MultiSelect", "Table"]:
			return

		doctype = docfield.options
		child_doctype_meta = traquent.get_meta(doctype)

		if not child_doctype_meta.istable:
			traquent.throw(
				_("Option {0} for field {1} is not a child table").format(
					traquent.bold(doctype), traquent.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			traquent.throw(f"Max for {traquent.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				traquent.throw(_("Options for Rating field can range from 3 to 10"))
	...
	for d in fields:
		...
		if not traquent.flags.in_migrate or in_ci:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = traquent.get_doc("DocType", doctype)
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		traquent.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if not d.select and not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			traquent.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			traquent.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			traquent.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			traquent.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			traquent.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			traquent.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
	...
	def check_if_submittable(d):
		if d.submit and not issubmittable:
			traquent.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			traquent.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			traquent.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
		...
		if doctype.custom:
			if d.role in AUTOMATIC_ROLES:
				traquent.throw(
					...
					).format(d.idx, traquent.bold(_(d.role))),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in traquent.get_all("Role", filters={"is_custom": 1})]
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not traquent.db.exists("Domain", doc.restrict_to_domain)
		):
			traquent.get_doc(doctype="Domain", domain=doc.restrict_to_domain).insert()

		if "tabModule Def" in traquent.db.get_tables() and not traquent.db.exists("Module Def", doc.module):
			m = traquent.get_doc({"doctype": "Module Def", "module_name": doc.module})
		...
		for role in list(set(roles)):
			if traquent.db.table_exists("Role", cached=False) and not traquent.db.exists("Role", role):
				r = traquent.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
		...
		pass
	except traquent.db.ProgrammingError as e:
		if traquent.db.is_table_missing(e):
			pass
		else:
			raise
...
def check_fieldname_conflicts(docfield):
	...
	doc = traquent.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		traquent.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	traquent.cache.delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		traquent.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		traquent.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		traquent.throw(_("Select a valid Sender Field for creating documents from Email"))

	if sender_field.options != "Email":
		traquent.throw(_("Sender Field should have Email in options"))
...
def get_row_size_utilization(doctype: str) -> float:
	...
	try:
		return flt(traquent.db.get_row_size(doctype) / traquent.db.MAX_ROW_SIZE_LIMIT * 100, 2)
	except Exception:
		return 0.0
