...
from traquent.desk.query_report import build_xlsx_data
...
from traquent.utils.csvutils import to_csv
...
class AutoEmailReport(Document):
	...
	def autoname(self):
		...
		if traquent.db.exists("Auto Email Report", self.name):
	...
	def sender_email(self):
		return traquent.db.get_value("Email Account", self.sender, "email_id")
	...
	def validate_report_count(self):
		count = traquent.db.count("Auto Email Report", {"user": self.user, "enabled": 1})

		max_reports_per_user = (
			cint(traquent.local.conf.max_reports_per_user)  # kept for backward compatibilty
			or cint(traquent.get_system_settings("max_auto_email_report_per_user"))
			or 20
		)
		...
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			...
			traquent.throw(msg, title=_("Report limit reached"))
		...
		if self.format not in valid_report_formats:
			traquent.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					traquent.bold(self.format), traquent.bold(", ".join(valid_report_formats))
				)
			)

	def validate_mandatory_fields(self):
		# Check if all Mandatory Report Filters are filled by the User
		filters = traquent.parse_json(self.filters) if self.filters else {}
		filter_meta = traquent.parse_json(self.filter_meta) if self.filter_meta else {}
		throw_list = [
	...
	def get_report_content(self):
		...
		report = traquent.get_doc("Report", self.report)

		self.filters = traquent.parse_json(self.filters) if self.filters else {}
		...
		# add serial numbers
		columns.insert(0, traquent._dict(fieldname="idx", label="", width="30px"))
		...
		if self.format == "HTML":
			...
			traquent.throw(_("Invalid Output Format"))
	...
	def prepare_dynamic_filters(self):
		self.filters = traquent.parse_json(self.filters)
	...
	def send(self):
		if self.filter_meta and not self.filters:
			traquent.throw(_("Please set filters value in Report Filter table."))
...
def send_daily():
	...
	enabled_reports = traquent.get_all(
		"Auto Email Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
	)

	for report in enabled_reports:
		auto_email_report = traquent.get_doc("Auto Email Report", report.name)
...
def send_monthly():
	...
	for report in traquent.get_all("Auto Email Report", {"enabled": 1, "frequency": "Monthly"}):
		traquent.get_doc("Auto Email Report", report.name).send()
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = None
				if doc_name and col.get("parent") and not traquent.get_meta(col.parent).istable:
					if traquent.db.exists(col.parent, doc_name):
						doc = traquent.get_doc(col.parent, doc_name)

				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = traquent.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
