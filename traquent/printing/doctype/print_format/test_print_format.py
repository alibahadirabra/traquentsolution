...
class TestPrintFormat(IntegrationTestCase):
	def test_print_user(self, style=None):
		print_html = traquent.get_print("User", "Administrator", style=style)
	...
	def test_export_doc(self):
		doc: "PrintFormat" = traquent.get_doc("Print Format", self.globalTestRecords["Print Format"][0]["name"])

		# this is only to make export_doc happy
		doc.standard = "Yes"
		_before = traquent.conf.developer_mode
		traquent.conf.developer_mode = True
		export_path = doc.export_doc()
		traquent.conf.developer_mode = _before
		...
		with open(exported_doc_path) as f:
			exported_doc = traquent.parse_json(f.read())
