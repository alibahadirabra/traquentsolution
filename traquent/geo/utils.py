...
def merge_location_features_in_one(coords):
	...
	for element in coords:
		geojson_loc = traquent.parse_json(element["location"])
		if not geojson_loc:
			continue
...
def return_location(doctype, filters_sql):
	...
	if filters_sql:
		try:
			coords = traquent.db.sql(
				f"""SELECT name, location FROM `tab{doctype}`  WHERE {filters_sql}""", as_dict=True
			)
		except traquent.db.InternalError:
			traquent.msgprint(traquent._("This Doctype does not contain location fields"), raise_exception=True)
			return
	else:
		coords = traquent.get_all(doctype, fields=["name", "location"])
	return coords
...
def return_coordinates(doctype, filters_sql):
	...
	if filters_sql:
		try:
			...
			traquent.msgprint(
				traquent._("This Doctype does not contain latitude and longitude fields"), raise_exception=True
			)
			return
	else:
		coords = traquent.get_all(doctype, fields=["name", "latitude", "longitude"])
	return coords
...
def get_coords_conditions(doctype, filters=None):
	...
	if not traquent.has_permission(doctype):
		traquent.throw(traquent._("Not Permitted"), traquent.PermissionError)
