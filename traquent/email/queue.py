...
from traquent.utils import cint, cstr, get_url, now_datetime
from traquent.utils.data import getdate
...
def get_emails_sent_this_month(email_account=None):
	...
	return traquent.db.count("Email Queue", filters=filters)
...
def get_emails_sent_today(email_account=None):
	...
	return traquent.db.sql(q, q_args)[0][0]


def get_unsubscribe_message(unsubscribe_message: str, expose_recipients: str) -> "traquent._dict[str, str]":
	...
	return traquent._dict(html=html, text=text)
...
def flush():
	...
	if traquent.are_emails_muted():
		msgprint(_("Emails are muted"))

	if cint(traquent.db.get_default("suspend_email_queue")) == 1:
		return
	...
	for row in email_queue_batch:
		try:
			email_queue: EmailQueue = traquent.get_doc("Email Queue", row.name)
			email_queue.send()
		except Exception:
			traquent.get_doc("Email Queue", row.name).log_error()
			...
			if (
				...
				traquent.throw(_("Email Queue flushing aborted due to too many failures."))


def get_queue():
	batch_size = cint(traquent.conf.email_queue_batch_size) or 500
