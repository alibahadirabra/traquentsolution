...
def add_bootinfo(bootinfo):
	...
	bootinfo.posthog_host = traquent.conf.get(POSTHOG_HOST_FIELD)
	bootinfo.posthog_project_id = traquent.conf.get(POSTHOG_PROJECT_FIELD)


@site_cache(ttl=60 * 60 * 12)
def site_age():
	try:
		est_creation = traquent.db.get_value("User", "Administrator", "creation")
...
def init_telemetry():
	...
	if hasattr(traquent.local, "posthog"):
		return

	if not traquent.get_system_settings("enable_telemetry"):
		return

	posthog_host = traquent.conf.get(POSTHOG_HOST_FIELD)
	posthog_project_id = traquent.conf.get(POSTHOG_PROJECT_FIELD)

	if not posthog_host or not posthog_project_id:
		return

	with suppress(Exception):
		traquent.local.posthog = Posthog(posthog_project_id, host=posthog_host)


def capture(event, app, **kwargs):
	init_telemetry()
	ph: Posthog = getattr(traquent.local, "posthog", None)
	with suppress(Exception):
		ph and ph.capture(distinct_id=traquent.local.site, event=f"{app}_{event}", **kwargs)
