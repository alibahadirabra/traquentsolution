import traquent
from traquent.desk.doctype.tag.tag import add_tag
from traquent.desk.reportview import get_stats
...
class TestTag(IntegrationTestCase):
	def setUp(self) -> None:
		traquent.db.delete("Tag")
		traquent.db.sql("UPDATE `tabDocType` set _user_tags=''")

	def test_tag_count_query(self):
		self.assertDictEqual(
			get_stats('["_user_tags"]', "DocType"),
			{"_user_tags": [["No Tags", traquent.db.count("DocType")]]},
		)
		...
		self.assertDictEqual(
			get_stats('["_user_tags"]', "DocType"),
			{"_user_tags": [["Standard", 2], ["No Tags", traquent.db.count("DocType") - 2]]},
		)
