...
class TestAutoAssign(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		traquent.db.delete("Assignment Rule")
	...
	def test_load_balancing(self):
		...
		for user in ("test@example.com", "test1@example.com", "test2@example.com"):
			self.assertEqual(
				len(traquent.get_all("ToDo", dict(allocated_to=user, reference_type=TEST_DOCTYPE))), 10
			)
		...
		for d in traquent.get_all(
			...
			traquent.db.delete("ToDo", {"name": d.name})
	...
	def check_assignment_rule_scheduling(self):
		...
		traquent.flags.assignment_day = "Monday"
		note = _make_test_record(public=1)
		...
		traquent.flags.assignment_day = "Friday"
		note = _make_test_record(public=1)
	...
	def test_assignment_rule_condition(self):
		...
		assignment_rule = traquent.get_doc(
		...
		expiry_date = traquent.utils.add_days(traquent.utils.nowdate(), 2)
		note1 = _make_test_record(expiry_date=expiry_date)
		note2 = _make_test_record(expiry_date=expiry_date)

		note1_todo = traquent.get_all(
			"ToDo", filters=dict(reference_type=TEST_DOCTYPE, reference_name=note1.name, status="Open")
		)[0]

		note1_todo_doc = traquent.get_doc("ToDo", note1_todo.name)
		self.assertEqual(traquent.utils.get_date_str(note1_todo_doc.date), expiry_date)

		# due date should be updated if the reference doc's date is updated.
		note1.expiry_date = traquent.utils.add_days(expiry_date, 2)
		note1.save()
		note1_todo_doc.reload()
		self.assertEqual(traquent.utils.get_date_str(note1_todo_doc.date), note1.expiry_date)

		# saving one note's expiry should not update other note todo's due date
		note2_todo = traquent.get_all(
			"ToDo",
			filters=dict(reference_type=TEST_DOCTYPE, reference_name=note2.name, status="Open"),
			fields=["name", "date"],
		)[0]
		self.assertNotEqual(traquent.utils.get_date_str(note2_todo.date), note1.expiry_date)
		self.assertEqual(traquent.utils.get_date_str(note2_todo.date), expiry_date)
		assignment_rule.delete()
		traquent.db.commit()  # undo changes commited by DDL
	...
	def test_submittable_assignment(self):
		...
		dt = traquent.get_doc("DocType", submittable_doctype)
		dt.is_submittable = 1
		dt.save()

		# create a rule for the submittable doctype
		assignment_rule = traquent.new_doc("Assignment Rule")
		...
		doc = traquent.new_doc(submittable_doctype)
		doc.save()
		doc.submit()
...
def clear_assignments():
	traquent.db.delete("ToDo", {"reference_type": TEST_DOCTYPE})


def get_assignment_rule(days, assign=None):
	traquent.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 1")
	...
	traquent.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 2")
...
def _make_test_record(
	...
	**kwargs,
	...
	doc = traquent.new_doc(TEST_DOCTYPE)
