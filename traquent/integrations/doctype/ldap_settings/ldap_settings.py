...
import traquent
from traquent import _, safe_encode
from traquent.model.document import Document
from traquent.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from traquent.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			traquent.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			traquent.throw(_("Invalid username or password"))
		except Exception as ex:
			traquent.throw(_(str(ex)))
	...
	def sync_roles(self, user: "User", additional_groups: list | None = None):
		...
		all_mapped_roles = {r.traquent_role for r in self.ldap_groups}
		matched_roles = {r.traquent_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups}
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if traquent.db.exists("User", user_data["email"]):
			user = traquent.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = traquent.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			traquent.throw(_("LDAP is not enabled."))
		...
		try:
			...
			traquent.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					if logout_sessions:
						from traquent.sessions import clear_sessions

						clear_sessions(user=user, force=True)
					traquent.msgprint(_("Password changed successfully."))
				else:
					traquent.throw(_("Failed to change password."))
			else:
				traquent.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			traquent.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	...
	user = ldap.authenticate(traquent.as_unicode(args.usr), traquent.as_unicode(args.pwd))

	traquent.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(traquent.local.login_manager):
			return False
