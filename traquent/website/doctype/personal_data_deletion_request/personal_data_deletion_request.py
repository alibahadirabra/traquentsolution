...
from traquent.desk.doctype.notification_settings.notification_settings import is_email_notifications_enabled
from traquent.model.document import Document
from traquent.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.website.doctype.personal_data_deletion_step.personal_data_deletion_step import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = traquent.get_hooks("user_data_fields")
	...
	def autoname(self):
		from traquent.model.naming import set_name_from_naming_options
		...
		domain = traquent.local.site.replace("_", "-")
		...
		traquent.utils.validate_email_address(self.email, throw=True)
	...
	def disable_user(self):
		user = traquent.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def send_verification_mail(self):
		...
		traquent.sendmail(
			...
			args={
				"email": self.email,
				"name": self.name,
				"host_name": traquent.utils.get_url(),
				"link": url,
			},
	...
	def notify_system_managers(self):
		...
		traquent.sendmail(
			...
			args={"user": self.email, "url": traquent.utils.get_url(self.get_url())},
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = traquent.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			traquent.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		traquent.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in traquent.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = traquent.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = traquent.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = traquent.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = traquent.get_doc("Personal Data Deletion Request", name)
	host_name = traquent.utils.get_url()
