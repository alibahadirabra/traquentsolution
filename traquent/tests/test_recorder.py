...
from traquent.recorder import normalize_query
...
class TestRecorder(IntegrationTestCase):
	...
	def wait_for_background_jobs(self):
		while any_job_pending(traquent.local.site):
			time.sleep(1)
	...
	def test_start(self):
		self.stop_recording()
		requests = traquent.recorder.get()
		self.assertEqual(len(requests), 1)

	def test_do_not_record(self):
		traquent.recorder.do_not_record(traquent.get_all)("DocType")
		self.stop_recording()
		requests = traquent.recorder.get()
	...
	def test_get(self):
		...
		request = traquent.recorder.get(requests[0]["uuid"])
	...
	def test_explain(self):
		traquent.db.sql("SELECT * FROM tabDocType")
		traquent.db.sql("COMMIT")
		traquent.db.sql("select 1", run=0)
		self.stop_recording()
	...
	def test_multiple_queries(self):
		...
		sql_dialect = traquent.db.db_type or "mariadb"
		for query in queries:
			traquent.db.sql(query[sql_dialect])
...
class TestRecorderDeco(IntegrationTestCase):
	def test_recorder_flag(self):
		...
		test()
		self.assertTrue(traquent.recorder.get())
