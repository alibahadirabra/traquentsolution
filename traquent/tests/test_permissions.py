...
from traquent.core.doctype.user_permission.user_permission import clear_user_permissions
from traquent.core.page.permission_manager.permission_manager import add, remove, reset, update
...
class TestPermissions(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		traquent.clear_cache(doctype="Blog Post")
		user = traquent.get_doc("User", "test1@example.com")
		user.add_roles("Website Manager")
		user.add_roles("System Manager")

		user = traquent.get_doc("User", "test2@example.com")
		user.add_roles("Blogger")

		user = traquent.get_doc("User", "test3@example.com")
		user.add_roles("Sales User")

		user = traquent.get_doc("User", "testperm@example.com")
		user.add_roles("Website Manager")
	...
	def setUp(self):
		...
		traquent.db.delete("User Permission")

		traquent.set_user("test1@example.com")

	def tearDown(self):
		traquent.set_user("Administrator")
		traquent.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def set_strict_user_permissions(ignore):
		ss = traquent.get_doc("System Settings")
		ss.apply_strict_user_permissions = ignore
		ss.flags.ignore_mandatory = 1
		ss.save()

	def test_basic_permission(self):
		post = traquent.get_doc("Blog Post", "-test-blog-post")
	...
	def test_select_permission(self):
		...
		self.assertRaises(traquent.PermissionError, post.save)

		permitted_record = traquent.get_list("Blog Post", fields="*", limit=1)[0]
		full_record = traquent.get_all("Blog Post", fields="*", limit=1)[0]
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = traquent.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		traquent.set_user("test2@example.com")
		names = [d.name for d in traquent.get_list("Blog Post", fields=["name", "blog_category"])]

		self.assertTrue("-test-blog-post-1" in names)
		self.assertFalse("-test-blog-post" in names)

	def test_default_values(self):
		doc = traquent.new_doc("Blog Post")
	...
	def test_user_link_match_doc(self):
		blogger = traquent.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		traquent.set_user("test2@example.com")

		post = traquent.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in traquent.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			traquent.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = traquent.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = traquent.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_standard_fields_manually(self):
		...
		fake_owner = traquent.db.get_value("User", {"name": ("!=", traquent.session.user)})

		d = traquent.new_doc("ToDo")
	...
	def test_dont_change_standard_constants(self):
		...
		self.assertRaises(traquent.CannotChangeConstantError, user.save)
	...
	def test_set_only_once(self):
		blog_post = traquent.get_meta("Blog Post")
		doc = traquent.get_doc("Blog Post", "-test-blog-post-1")
		doc.db_set("title", "Old")
		blog_post.get_field("title").set_only_once = 1
		doc.title = "New"
		self.assertRaises(traquent.CannotChangeConstantError, doc.save)
	...
	def test_set_only_once_child_table_rows(self):
		...
		doc = traquent.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = traquent.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		traquent.delete_doc("Blog Post", "-test-blog-post-title")
		...
		self.assertRaises(traquent.PermissionError, doc.insert)
	...
	def test_strict_user_permissions(self):
		...
		traquent.set_user("Administrator")
		traquent.db.delete("Contact")
		traquent.db.delete("Contact Email")
		traquent.db.delete("Contact Phone")
		...
		allowed_contact = traquent.get_doc("Contact", "_Test Contact For _Test Customer")
		other_contact = traquent.get_doc("Contact", "_Test Contact For _Test Supplier")

		traquent.set_user("test3@example.com")
		self.assertTrue(allowed_contact.has_permission("read"))
		self.assertTrue(other_contact.has_permission("read"))
		self.assertEqual(len(traquent.get_list("Contact")), 2)
		...
		self.assertTrue(len(traquent.get_list("Contact")), 1)
	...
	def test_contextual_user_permission(self):
		...
		posts = traquent.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = traquent.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		traquent.set_user("test2@example.com")
		self.assertEqual(traquent.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		doclist = [d.name for d in traquent.response.docs]
		self.assertTrue(doc.name in doclist)

		traquent.set_user("test2@example.com")
		self.assertRaises(traquent.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		traquent.set_user("test2@example.com")
		self.assertIn(doc.name, traquent.get_list("Blog Post", pluck="name"))
		...
		traquent.set_user("test2@example.com")
		self.assertNotIn(doc.name, traquent.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		self.assertRaises(traquent.PermissionError, traquent.delete_doc, "Blog Post", doc.name)

		traquent.set_user("test2@example.com")
		traquent.delete_doc("Blog Post", "-test-blog-post-title-new-1")
	...
	def test_clear_user_permissions(self):
		...
		# reset the user
		traquent.set_user(current_user)
	...
	def test_child_permissions(self):
		...
		self.assertRaises(traquent.PermissionError, doc.check_permission)
	...
	def test_select_user(self):
		...
		for gender in ("Male", "Female"):
			if traquent.db.exists("Gender", gender):
				continue

			traquent.get_doc({"doctype": "Gender", "gender": gender}).insert()

		# asssign gender to test users
		traquent.db.set_value("User", "test1@example.com", "gender", "Male")
		traquent.db.set_value("User", "test2@example.com", "gender", "Female")
		traquent.db.set_value("User", "test3@example.com", "gender", "Female")
		...
		traquent.set_user("test3@example.com")
		users = traquent.get_list("User", pluck="name")
	...
	def test_automatic_permissions(self):
		def assertHasRole(*roles: str | tuple[str, ...]):
			for role in roles:
				self.assertIn(role, traquent.get_roles())
	...
	def test_overrides_work_as_expected(self):
		...
		custom_role = traquent.new_doc("Role", role_name=traquent.generate_hash()).insert().name
		with self.set_user("Administrator"):
