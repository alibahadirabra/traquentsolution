...
class WebsiteTheme(Document):
	...
	if TYPE_CHECKING:
		...
		from traquent.website.doctype.website_theme_ignore_app.website_theme_ignore_app import (
	...
	def on_update(self):
		if (
			not self.custom
			and traquent.local.conf.get("developer_mode")
	...
	def is_standard_and_not_valid_user(self):
		return (
			...
			and not (traquent.flags.in_import or traquent.flags.in_test or traquent.flags.in_migrate)
		)

	def on_trash(self):
		if self.is_standard_and_not_valid_user():
			traquent.throw(_("You are not allowed to delete a standard Website Theme"), traquent.PermissionError)

	def validate_if_customizable(self):
		if self.is_standard_and_not_valid_user():
			traquent.throw(_("Please Duplicate this Website Theme to customize."))
	...
	def generate_bootstrap_theme(self):
		...
		folder_path = abspath(traquent.utils.get_files_path("website_theme", is_private=False))
		# create folder if not exist
		traquent.create_folder(folder_path)

		if self.custom:
			self.delete_old_theme_files(folder_path)

		# add a random suffix
		suffix = traquent.generate_hash(length=8) if self.custom else "style"
		file_name = traquent.scrub(self.name) + "_" + suffix + ".css"
		...
		if stderr:
			stderr = traquent.safe_decode(stderr)
			stderr = stderr.replace("\n", "<br>")
			traquent.throw(f'<div style="font-family: monospace;">{stderr}</div>')
		else:
			self.theme_url = "/files/website_theme/" + file_name

		traquent.msgprint(_("Compiled Successfully"), alert=True)
	...
	def delete_old_theme_files(self, folder_path):
		...
		for fname in os.listdir(folder_path):
			if fname.startswith(traquent.scrub(self.name) + "_") and fname.endswith(".css"):
...
def get_active_theme() -> Optional["WebsiteTheme"]:
	if website_theme := traquent.get_website_settings("website_theme"):
		try:
			return traquent.get_cached_doc("Website Theme", website_theme)
...
def get_scss_paths():
	...
	for app in traquent.get_installed_apps():
		for scss_file in scss_files:
			full_path = traquent.get_app_path(app, scss_file)
...
def after_migrate():
	...
	"""
	website_theme = traquent.db.get_single_value("Website Settings", "website_theme")
	if not website_theme or website_theme == "Standard":
		return

	doc = traquent.get_doc("Website Theme", website_theme)
