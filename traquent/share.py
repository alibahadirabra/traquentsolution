...
def add_docshare(
	...
	if share_name:
		doc = traquent.get_doc("DocShare", share_name)
	else:
		doc = traquent.new_doc("DocShare")
	...
	if traquent.get_cached_value("User", user, "follow_shared_documents"):
...
def remove(doctype, name, user, flags=None):
	share_name = traquent.db.get_value("DocShare", {"user": user, "share_name": name, "share_doctype": doctype})
...
def set_docshare_permission(doctype, name, user, permission_to, value=1, everyone=0, flags=None):
	...
	if not share_name:
		...
		share = traquent.get_doc("DocShare", share_name)
		if flags:
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = traquent.session.user
	table = traquent.qb.DocType("DocShare")
	query = (
...
def get_share_name(doctype, name, user, everyone):
	if cint(everyone):
		share_name = traquent.db.get_value(
...
def check_share_permission(doctype, name):
	...
	if not traquent.has_permission(doctype, ptype="share", doc=name):
		traquent.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), traquent.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(traquent.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		traquent.bold(reference_user), traquent.bold(_(doctype)), get_title_html(title)
	)
