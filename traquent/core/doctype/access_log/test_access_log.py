...
from traquent.utils import cstr, get_site_url
...
class TestAccessLog(IntegrationTestCase):
	def setUp(self):
		...
		generate_keys(traquent.session.user)
		traquent.db.commit()
		generated_secret = traquent.utils.password.get_decrypted_password(
			"User", traquent.session.user, fieldname="api_secret"
		)
		...
		self.file_name = traquent.utils.random_string(10) + ".txt"
		self.test_content = traquent.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = traquent.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = traquent.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(traquent.local.site) + new_private_file.file_url
