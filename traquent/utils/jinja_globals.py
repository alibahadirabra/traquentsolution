...
def inspect(var, render=True):
	from traquent.utils.jinja import get_jenv
...
def web_blocks(blocks):
	import traquent
	from traquent import _, _dict, throw
	from traquent.website.doctype.web_page.web_page import get_web_blocks_html
	...
	if not traquent.flags.web_block_scripts:
...
def include_script(path, preload=True):
	...
	if preload:
		...
		traquent.local.preload_assets["script"].append(path)
...
def include_icons(path, preload=True):
	...
	if preload:
		...
		traquent.local.preload_assets["icons"].append(path)
...
def include_style(path, rtl=None, preload=True):
	...
	if preload:
		...
		traquent.local.preload_assets["style"].append(path)

	return f'<link type="text/css" rel="stylesheet" href="{path}">'


def bundled_asset(path, rtl=None):
	from traquent.utils import get_assets_json
	from traquent.website.utils import abs_url
