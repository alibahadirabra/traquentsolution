...
def make(
	...
	**kwargs,
	...
	if doctype and name and not traquent.has_permission(doctype=doctype, ptype="email", doc=name):
		raise traquent.PermissionError(f"You are not allowed to send emails related to: {doctype} {name}")
...
def _make(
	...
	sender = sender or get_formatted_email(traquent.session.user)
	...
	if cint(send_email):
		if not comm.get_outgoing_email_account():
			traquent.throw(
				...
				exc=traquent.OutgoingEmailError,
			)
...
def add_attachments(name: str, attachments: Iterable[str | dict]) -> None:
	...
	for a in attachments:
		if isinstance(a, str):
			attach = traquent.db.get_value("File", {"name": a}, ["file_url", "is_private"], as_dict=1)
			file_args = {
...
def _mark_email_as_seen(name):
	try:
		...
		traquent.log_error("Unable to mark as seen", None, "Communication", name)

	traquent.db.commit()  # nosemgrep: after_response requires explicit commit
...
def update_communication_as_read(name):
	...
	communication = traquent.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
