...
class TestDefaults(IntegrationTestCase):
	...
	def test_user_permission_on_defaults(self):
		...
		old_user = traquent.session.user
		user = "test@example.com"
		traquent.set_user(user)

		perm_doc = traquent.get_doc(
			doctype="User Permission", user=traquent.session.user, allow="Language", for_value="en-GB"
		...
		traquent.delete_doc("User Permission", perm_doc.name)
		traquent.set_user(old_user)
	...
	def test_user_permission_defaults(self):
		...
		perm_doc = traquent.get_doc(
			doctype="User Permission", user=traquent.session.user, allow="Country", for_value="India"
		).insert(ignore_permissions=True)

		traquent.db.set_value("User Permission", perm_doc.name, "is_default", 1)
		set_global_default("Country", "United States")
		self.assertEqual(get_user_default("Country"), "India")

		traquent.db.set_value("User Permission", perm_doc.name, "is_default", 0)
		clear_user_default("Country")
		self.assertEqual(get_user_default("Country"), None)

		perm_doc = traquent.get_doc(
			doctype="User Permission", user=traquent.session.user, allow="Country", for_value="United States"
		).insert(ignore_permissions=True)
