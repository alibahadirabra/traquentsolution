...
def new_catalog(app: str, locale: str | None = None) -> Catalog:
	def get_hook(hook, app):
		return traquent.get_hooks(hook, [None], app)[0]
...
def get_po_dir(app: str) -> Path:
	return Path(traquent.get_app_path(app)) / PO_DIR
...
def get_method_map(app: str):
	file_path = Path(traquent.get_app_path(app)).parent / "babel_extractors.csv"
	if file_path.exists():
...
def generate_pot(target_app: str | None = None):
	...
	for app in apps:
		app_path = traquent.get_pymodule_path(app, "..")
...
def get_is_gitignored_function_for_app(app: str | None):
	...
	repo = git.Repo(traquent.get_app_source_path(app), search_parent_directories=True)
...
def migrate(app: str | None = None, locale: str | None = None):
	apps = [app] if app else traquent.get_all_apps(True)

	for app in apps:
		if locale:
			csv_to_po(app, locale)
		else:
			app_path = Path(traquent.get_app_path(app))
...
def csv_to_po(app: str, locale: str):
	csv_file = Path(traquent.get_app_path(app)) / "translations" / f"{locale.replace('_', '-')}.csv"
