...
import traquent
from traquent.email.doctype.email_queue.email_queue import SendMailContext, get_email_retry_limit
...
class TestEmailQueue(IntegrationTestCase):
	def test_email_queue_deletion_based_on_modified_date(self):
		...
		new_record = traquent.copy_doc(old_record)
		new_record.insert()

		EmailQueue.clear_old_logs()

		self.assertFalse(traquent.db.exists("Email Queue", old_record.name))
		self.assertFalse(traquent.db.exists("Email Queue Recipient", {"parent": old_record.name}))

		self.assertTrue(traquent.db.exists("Email Queue", new_record.name))
		self.assertTrue(traquent.db.exists("Email Queue Recipient", {"parent": new_record.name}))

	def test_failed_email_notification(self):
		subject = traquent.generate_hash()
		email_record = traquent.new_doc("Email Queue")
		email_record.sender = "Test <test@example.com>"
		email_record.message = textwrap.dedent(
			f"""\
		MIME-Version: 1.0
		Message-Id: {traquent.generate_hash()}
		...
		notification_log = traquent.db.get_value(
			"Notification Log",
	...
	def test_perf_reusing_smtp_server(self):
		...
		self.assertTrue(traquent.new_doc("Email Queue").get_email_account()._from_site_config)

		def get_server(q):
			return q.get_email_account().get_smtp_server()

		self.assertIs(get_server(traquent.new_doc("Email Queue")), get_server(traquent.new_doc("Email Queue")))

		q1 = traquent.new_doc("Email Queue", email_account="_Test Email Account 1")
		q2 = traquent.new_doc("Email Queue", email_account="_Test Email Account 1")
		self.assertIsNot(get_server(traquent.new_doc("Email Queue")), get_server(q1))
