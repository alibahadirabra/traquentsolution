...
class Note(Document):
	...
	if TYPE_CHECKING:
		from traquent.desk.doctype.note_seen_by.note_seen_by import NoteSeenBy
	...
	def validate(self):
		if self.notify_on_login and not self.expire_notification_on:
			...
			self.expire_notification_on = traquent.utils.add_days(self.creation, 7)
...
def get_permission_query_conditions(user):
	...
	return f"(`tabNote`.owner = {traquent.db.escape(user)} or `tabNote`.public = 1)"
