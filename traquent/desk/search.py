...
from traquent import _, is_whitelisted, validate_and_sanitize_search_inputs
from traquent.database.schema import SPECIAL_CHAR_PATTERN
from traquent.model.db_query import get_order_by
from traquent.permissions import has_permission
from traquent.utils import cint, cstr, unique
from traquent.utils.data import make_filter_tuple
...
def sanitize_searchfield(searchfield: str):
	...
	if SPECIAL_CHAR_PATTERN.search(searchfield):
		traquent.throw(_("Invalid Search Field {0}").format(searchfield), traquent.DataError)
...
def search_widget(
	...
	standard_queries = traquent.get_hooks().standard_queries or {}
	...
	if query:  # Query = custom search query i.e. python function
		try:
			is_whitelisted(traquent.get_attr(query))
		...
		except (traquent.PermissionError, traquent.AppNotInstalledError, ImportError):
			if traquent.local.conf.developer_mode:
				raise
			else:
	...
	if not meta.translated_doctype:
		_txt = traquent.db.escape((txt or "").replace("%", "").replace("@", ""))
...
def get_names_for_mentions(search_term):
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = traquent.utils.get_url_to_form(
...
def get_link_title(doctype, docname):
	...
	if meta.show_title_field_in_link:
		return traquent.db.get_value(doctype, docname, meta.title_field)
