...
from traquent.utils import cint, get_fullname
...
class DocShare(Document):
	...
	def get_doc(self):
		if not getattr(self, "_doc", None):
			self._doc = traquent.get_doc(self.share_doctype, self.share_name)
		return self._doc
	...
	def validate_user(self):
		if self.everyone:
			...
			traquent.throw(_("User is mandatory for Share"), traquent.MandatoryError)

	def check_share_permission(self):
		if not self.flags.ignore_share_permission and not traquent.has_permission(
			self.share_doctype, "share", self.get_doc()
		):
			traquent.throw(_('You need to have "Share" permission'), traquent.PermissionError)

	def check_is_submittable(self):
		if self.submit and not cint(traquent.db.get_value("DocType", self.share_doctype, "is_submittable")):
			traquent.throw(
				_("Cannot share {0} with submit permission as the doctype {1} is not submittable").format(
					traquent.bold(self.share_name), traquent.bold(self.share_doctype)
				)
			)
...
def on_doctype_update():
	...
	traquent.db.add_index("DocShare", ["user", "share_doctype"])
	traquent.db.add_index("DocShare", ["share_doctype", "share_name"])
