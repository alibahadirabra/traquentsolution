...
class TestScheduler(IntegrationTestCase):
	def setUp(self):
		...
		purge_pending_jobs()
		if not traquent.get_all("Scheduled Job Type", limit=1):
			sync_jobs()
	...
	def test_enqueue_jobs(self):
		traquent.db.sql("update `tabScheduled Job Type` set last_execution = '2010-01-01 00:00:00'")
	...
	def test_is_dormant(self):
		...
		self.assertTrue(is_dormant(check_time=add_days(traquent.db.get_last_created("Activity Log"), 5)))
		self.assertFalse(is_dormant(check_time=traquent.db.get_last_created("Activity Log")))

	def test_once_a_day_for_dormant(self):
		traquent.db.truncate("Scheduled Job Log")
		self.assertTrue(schedule_jobs_based_on_activity(check_time=get_datetime("2100-01-01 00:00:00")))
		self.assertTrue(
			schedule_jobs_based_on_activity(
				check_time=add_days(traquent.db.get_last_created("Activity Log"), 5)
			)
		)
...
def get_test_job(method="traquent.tests.test_scheduler.test_timeout_10", frequency="All") -> ScheduledJobType:
	if not traquent.db.exists("Scheduled Job Type", dict(method=method)):
		...
		job = traquent.get_doc("Scheduled Job Type", dict(method=method))
